# backend/scripts/firestore_to_bigquery_etl.py
#
# ETL script that syncs data from Firestore to BigQuery and creates analytics
# tables and views. Replaces cosmos_to_bigquery_etl.py.
...
class FullETLLoader:
    ...
    async def setup(self):
        ...
        try:
            ...
            if not connection_results.get("firestore", False):
    ...
    async def load_attempts_data(self, incremental: bool = False):
        """Load all attempts data from Firestore to BigQuery"""
        print(f"\n📊 Loading attempts data {'(incremental)' if incremental else '(full replace)'}...")

        try:
            result = await self.etl_service.sync_attempts_from_firestore(
    ...
    async def load_reviews_data(self, incremental: bool = False):
        """Load all reviews data from Firestore to BigQuery"""
        print(f"\n📝 Loading reviews data {'(incremental)' if incremental else '(full replace)'}...")

        try:
            result = await self.etl_service.sync_reviews_from_firestore(
                incremental=incremental,
                limit=None
            )
    ...
    async def load_assessments_data(self, incremental: bool = False):
        """Load assessment data (still via Cosmos until migrated to Firestore)"""
        print(f"\n📋 Loading assessment data {'(incremental)' if incremental else '(full load)'}...")

        try:
            # Assessments still come from Cosmos until that migration is complete
    ...
    async def load_user_profiles_data(self, incremental: bool = False):
        """Load user profiles data (still via Cosmos until migrated to Firestore)"""
        print(f"\n👤 Loading user profiles data {'(incremental)' if incremental else '(full load)'}...")

        try:
            # User profiles still come from Cosmos until that migration is complete
...
async def run_firestore_preview():
    """Preview Firestore data before loading"""
    print("🔍 Firestore Data Preview")
    print("-" * 50)
    ...
    try:
        ...
        # Count attempts using collection group
        attempts_docs = list(fs.client.collection_group('attempts').stream())
        print(f"📊 Total attempts in Firestore: {len(attempts_docs):,}")

        # Count reviews using collection group
        try:
            reviews_docs = list(fs.client.collection_group('reviews').stream())
            print(f"📝 Total reviews in Firestore: {len(reviews_docs):,}")
        except Exception as e:
            print(f"⚠️  Could not count reviews: {e}")

        # Show sample data
        sample_attempts = list(fs.client.collection_group('attempts').limit(3).stream())
        if sample_attempts:
            print(f"\n📋 Sample attempt fields:")
            sample_data = sample_attempts[0].to_dict()
            for field in sorted(sample_data.keys()):
                print(f"  - {field}")

    except Exception as e:
        print(f"❌ Firestore preview failed: {e}")


async def run_schema_migration():
    """Run one-time schema migration from cosmos_id to doc_id"""
    print("🔄 Running BigQuery schema migration (cosmos_id → doc_id)")
    print("-" * 50)
    ...
    try:
        ...
        results = await etl_service.migrate_schema_cosmos_to_firestore()
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Firestore to BigQuery ETL Script')
    parser.add_argument('--incremental', action='store_true',
                       help='Run incremental load instead of full load')
    parser.add_argument('--preview', action='store_true',
                       help='Preview Firestore data before loading')
    parser.add_argument('--migrate-schema', action='store_true',
                       help='Run one-time schema migration (cosmos_id → doc_id)')
    ...
    if args.preview:
        asyncio.run(run_firestore_preview())
