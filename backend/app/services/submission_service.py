...
class SubmissionService:
    ...
    async def _save_review(
        ...
        Attempt persistence is owned by
        CompetencyService.update_competency_from_problem, which saves the attempt
        (with primitive_type / eval_mode / success) and then reads it back to
        compute the blended competency score. Saving the attempt here as well
        would write two attempt docs per submission — double-counting every
        completion and inflating attempt-based credibility. So this method
        persists only the review.
        """
        try:
            student_id = user_context["student_id"]
            firebase_uid = user_context["firebase_uid"]
            ...
            logger.error(f"Error saving review to databases: {str(e)}")
