...
Document fields:
{
  ...
  velocity: {
    aggregate: 0.85,
    subjects: {
      math: { velocity: 0.78, earnedMastery: 42.09, adjustedExpected: 54.0 },
      reading: { velocity: 1.08, earnedMastery: 44.31, adjustedExpected: 41.0 },
      engineering: { velocity: 0.61, earnedMastery: 11.0, adjustedExpected: 18.0 }
    },
    lastSnapshotWeek: "2026-03-02"
  },
...
## 15. Mastery Velocity Metric

### 15.1 Purpose

Velocity is the single most important progress metric in Lumina. It answers the question every parent asks: "Is my child on track?" The metric compares the student's actual mastery progress against the expected progress based on where they are in the school year, expressed as a percentage.
...
### 15.2 The Pipeline Problem

A naive velocity calculation — `closedSkills / expectedClosedSkills` — is misleading because it ignores the review pipeline. Consider two scenarios at the midpoint of the school year:

**Student A:** Introduced 60 skills, closed 50, has 10 in review. Naive velocity: 50/60 = 83%.

**Student B:** Introduced 60 skills, closed 50, has 10 in review — but all 10 are at completion factor 0.75 (one review away from closing). Naive velocity: also 83%.

These students are not in the same position. Student B has 7.5 "earned" masteries sitting in the pipeline that will close within 2 weeks. The naive metric treats them identically.

The completion factor model solves this. Skills in the review pipeline represent partially completed mastery work, and their completion factors quantify exactly how much.

### 15.3 Earned Mastery

**Earned mastery** credits closed skills at 1.0 and in-review skills at their completion factor:

```
earnedMastery = Σ 1.0 for each closed skill
              + Σ completionFactor for each in-review skill
              + Σ 0.0 for each not-started skill
```

This is analogous to earned premium in insurance — it's the portion of the total curriculum investment that has been "earned" through actual student work, including work in progress.

**Example:**

```
Closed skills:     34 × 1.00 = 34.00
In-review skills:
  8 at CF 0.25     8 × 0.25 =  2.00  (just entered pipeline)
  5 at CF 0.50     5 × 0.50 =  2.50
  3 at CF 0.75     3 × 0.75 =  2.25
  2 at CF 0.67     2 × 0.67 =  1.34  (adverse development)
                              ------
Earned mastery:               42.09
```

### 15.4 Expected Mastery and Velocity Calculation

**Raw expected mastery** is the linear target based on time elapsed:

```
rawExpectedMastery = totalSkills × fractionOfYearElapsed
```

However, this creates a structural problem in the first 6 weeks of the school year. Even a perfectly performing student cannot have any closed skills until at least 6 weeks after their first introduction (the minimum review cycle is 0 + 2 + 4 + 6 weeks). A student who is doing everything right would show velocity well below 100% simply because the pipeline hasn't had time to flush.

**Pipeline-adjusted expected mastery** accounts for this structural lag by recognizing that skills introduced in the last 6 weeks cannot possibly be closed yet. Their expected contribution is their expected completion factor given their age:

```
For each week cohort of skills (grouped by when they were introduced):
  weeksInPipeline = (today - introductionDate) / 7
  
  if weeksInPipeline >= 6:
    expectedContribution = 1.0                    // should be closed by now
  elif weeksInPipeline >= 4:
    expectedContribution = 0.75                   // 3 of 4 sessions done
  elif weeksInPipeline >= 2:
    expectedContribution = 0.50                   // 2 of 4 sessions done
  else:
    expectedContribution = 0.25                   // just entered pipeline

adjustedExpectedMastery = Σ expectedContribution for all introduced skills
                        + 0.0 for skills not yet introduced but expected by now
```
...
This produces a metric that is fair from day one. A student who introduces skills on pace and passes every review on schedule will show velocity at or near 100% regardless of where they are in the school year.

### 15.5 Velocity Per Subject

Velocity is computed per subject and as a weighted aggregate:

```
velocity[math]        = earnedMastery[math] / adjustedExpected[math]
velocity[reading]     = earnedMastery[reading] / adjustedExpected[reading]
velocity[engineering] = earnedMastery[engineering] / adjustedExpected[engineering]

// Aggregate weighted by subject size
velocity[total] = Σ earnedMastery[all] / Σ adjustedExpected[all]
```

Per-subject velocity reveals imbalances the aggregate hides. A student at 100% aggregate might be at 140% in reading and 65% in math — the reading strength is masking the math deficit.

### 15.6 Velocity Decomposition — Why Is Velocity Where It Is?

A velocity number alone doesn't tell the parent what to do about it. The metric decomposes into three drivers:

**Introduction velocity** — Is the student introducing new skills on pace?

```
introductionVelocity = skillsIntroduced / expectedIntroduced
```

If this is low, the bottleneck is capacity — too many reviews are crowding out new skills.

**Pass-through velocity** — Of skills introduced, are they moving through the review cycle on schedule?

```
passThroughVelocity = avgCompletionFactor / expectedAvgCompletionFactor
```

If this is low, the student is failing reviews and accumulating adverse development. Skills are stuck in tight loops.

**Closure velocity** — Are skills actually closing, or sitting in late-stage review?

```
closureVelocity = closedSkills / expectedClosedSkills (adjusted for pipeline age)
```

If this is low but pass-through is fine, there may be a scheduling issue — reviews aren't being served on time.

The parent-facing display shows the headline velocity with the primary driver called out:

```
Math Velocity: 78%
  ↳ Primary driver: Pass-through (67%) — 4 skills in tight-loop recovery
```

### 15.7 Velocity Trend

A single velocity snapshot is less useful than the trend. The system stores weekly velocity snapshots and displays a sparkline or trend line showing the last 8 weeks:

```
Week:     -8    -7    -6    -5    -4    -3    -2    -1    Now
Velocity: 95%   92%   88%   85%   82%   78%   80%   79%   78%
Trend:    ━━━━━╲╲╲╲╲╲╲╲━━━━━━━━━━

Signal: Declining velocity, stabilizing in last 3 weeks
```

A declining trend is an early warning. A recovering trend after an override (e.g., pausing a subject or activating review-only mode) validates that the intervention worked.

### 15.8 Velocity Endpoint

```
GET /api/velocity/{studentId}
```
...
```json
{
  ...
  "aggregate": {
    "earnedMastery": 112.4,
    "adjustedExpectedMastery": 132.0,
    "velocity": 0.85,
    "trend": [0.95, 0.92, 0.88, 0.85, 0.82, 0.78, 0.80, 0.79, 0.85]
  },
  "subjects": {
    "math": {
      "totalSkills": 120,
      ...
      "inReviewEarned": 8.09,
      "earnedMastery": 42.09,
      "adjustedExpectedMastery": 54.0,
      "velocity": 0.78,
      "trend": [0.90, 0.88, 0.84, 0.80, 0.78, 0.75, 0.76, 0.77, 0.78],
      "decomposition": {
        "introductionVelocity": 0.88,
        "passThroughVelocity": 0.67,
        "closureVelocity": 0.82
      },
      "primaryDriver": {
        "component": "pass_through",
        "value": 0.67,
        "explanation": "4 skills in tight-loop recovery are depressing pipeline throughput"
      }
    },
    "reading": {
      "totalSkills": 80,
      ...
      "inReviewEarned": 4.31,
      "earnedMastery": 44.31,
      "adjustedExpectedMastery": 41.0,
      "velocity": 1.08,
      "trend": [1.02, 1.04, 1.05, 1.06, 1.07, 1.08, 1.08, 1.08, 1.08],
      "decomposition": {
        "introductionVelocity": 1.05,
        "passThroughVelocity": 1.02,
        "closureVelocity": 1.10
      },
      "primaryDriver": {
    ...
    "engineering": {
      ...
      "inReviewEarned": 3.0,
      "earnedMastery": 11.0,
      "adjustedExpectedMastery": 18.0,
      "velocity": 0.61,
      "trend": [0.85, 0.80, 0.75, 0.70, 0.68, 0.65, 0.63, 0.62, 0.61],
      "decomposition": {
        "introductionVelocity": 0.65,
        "passThroughVelocity": 0.88,
        "closureVelocity": 0.72
      },
      "primaryDriver": {
        "component": "introduction",
        "value": 0.65,
        "explanation": "Review burden from other subjects is limiting new engineering introductions"
      }
    }
  }
}
...
### 15.9 Velocity Visualization

The velocity metric integrates into the frontend as the headline number on the parent dashboard:

**Top-level display:** A large velocity gauge per subject with color coding:

- **120%+:** Blue — well ahead of pace
- **100-120%:** Green — on track or ahead
- **80-100%:** Yellow — slightly behind, manageable
- **60-80%:** Orange — significantly behind, action recommended
- **Below 60%:** Red — critically behind, intervention needed

**Trend sparkline** below each gauge showing the 8-week history.

**Decomposition callout** showing the primary driver with a one-sentence explanation and a suggested action (e.g., "Consider pausing engineering to free up review capacity for math").

**Trajectory integration:** The monthly projection chart (Section 12.2) should overlay velocity bands — showing the projected velocity at each future week. If current trends continue, when does velocity cross below 80%? Below 60%? This connects the current state to the forward projection.

### 15.10 Firestore Storage

Weekly velocity snapshots are stored for trend display:

```
Collection: studentCompetencyState/{studentId}/velocityHistory/{weekId}

Document fields:
{
  weekOf: "2026-03-02",
  aggregate: { earnedMastery: 112.4, expected: 132.0, velocity: 0.85 },
  subjects: {
    math: { earnedMastery: 42.09, expected: 54.0, velocity: 0.78,
            introVelocity: 0.88, passThroughVelocity: 0.67, closureVelocity: 0.82 },
    reading: { ... },
    engineering: { ... }
  }
}
...
Snapshots are written once per week (e.g., Sunday evening) by a scheduled Cloud Function. The velocity endpoint reads the current snapshot from live data and appends historical snapshots for the trend.

---

## 16. What BigQuery Still Handles
...
| Velocity accuracy | Pipeline-adjusted velocity within 5% of retrospectively computed true velocity |
| Velocity trend latency | Weekly snapshots computed and available within 1 hour of scheduled run |

---

## 18. Implementation Phases

### Phase 1: Weekly Planner + Review Engine + Velocity

- Implement `GET /api/plan/weekly/{studentId}`
- Implement `GET /api/velocity/{studentId}`
...
- Implement earned mastery and pipeline-adjusted velocity calculation
- Deploy weekly velocity snapshot Cloud Function
...
- Implement velocity dashboard with gauges, trend sparklines, and decomposition callouts
- Overlay projected velocity bands onto trajectory chart
