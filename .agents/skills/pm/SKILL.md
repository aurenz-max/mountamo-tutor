...
| Lifecycle follow-up queues and birth certificates | Per-primitive capability raises | `$add-eval-modes`, `$add-support-tiers`, `$add-structural-difficulty`, `$add-tutoring-scaffold`, `$add-sound`, `$add-spoken-judge`, `$add-voice-control`, `$migrate-primitive` | Follow the ladder in `my-tutoring-app/src/components/lumina/docs/PRIMITIVE_LIFECYCLE.md`. |
