...
**Lifecycle:** primitives are built in layers, not one pass. `/primitive` births at L0 (sound, measurable, single core mode) and ends with a birth certificate + follow-up queue; the add- skills raise it one layer each (`/add-eval-modes` L1 → `/add-tutoring-scaffold` L2 → `/add-support-tiers` L3 → `/add-structural-difficulty` L4 → `/add-sound`, `SPOKEN_INTERACTION_DOCTRINE.md`, `/add-voice-control` L5), with `/eval-test` closing every layer. Ladder + detection signals: `my-tutoring-app/src/components/lumina/docs/PRIMITIVE_LIFECYCLE.md`.
