...
## Current snapshot — reconciled 2026-08-10 (evening, full `/pm`) (**the tree is the story: nothing committed since 08-08 18:02 while the judged loop became a cross-subject CAPABILITY, its correction branch fired at last (build judge), and a pilot family got an invite gate. WIP re-ruled: pilot onboarding takes slot 2, science parks. Both ACTIVE pulls are USER sittings — the machine's best move is `/ship`.**)

| Lane | State | Pull now | Trusted as of |
|---|---|---|---|
| **🚢 SHIP THE TREE** | **TOP HYGIENE — last commit 2026-08-08 18:02; two days of work, ~65 modified + ~45 untracked files** | At least SIX slices, almost file-disjoint: **(1)** DI literacy ports 1–4 + DI-GREET-1 (`owns_opening`, backend) + `phonemeVoice` + `letterGroups` + the `verdictTimeoutMs` engine fix; **(2)** the generalized judged-script runner (`judgedScriptContract` + `useJudgedScriptRunner`) + `counting-board` + `push-pull-arena` pilots (incl. push-pull's kit migration); **(3)** pilot onboarding slice 1 (invite service + scripts + login form + the register grade write-through); **(4)** the CLOSED ordering lane's production fix (`gemini-curator-brief.ts` + the A/B deletion) — **a closed lane's fix riding uncommitted**; **(5)** docs/skills/pm (the `/add-spoken-judge` deletion, `SPOKEN_INTERACTION_DOCTRINE.md`, CLAUDE.md, this file, the qa registers); **(6)** the orphaned pair — **user ruling at `/ship` time: commit or revert.** Every slice's gates were green when its queue record was written; `/ship` re-verifies per slice. | 2026-08-10 |
| **🔝 JUDGED-LOOP FAMILY — DI modality on literacy + the generalized runner** | **ACTIVE 1 (user rulings 2026-08-09 + 2026-08-10 *"build the general schema and capability, try it on 2-3 subjects"*). Machine side done to the fence; the SPOKEN judge has never heard a wrong answer.** | **Pull = THE MIC SITTING (user-owned): #82 · #83 · #84 · #85-sitting-A · #86 · #87 — ANSWER WRONG ON PURPOSE, ~15 min total.** The #85 sitting-B drive proved the BUILD judge (dog corrected once, bug twice then capped; gesture anchor proven; letter-group fix held live) and found+fixed `verdictTimeoutMs` dead-while-mic-open (live for four ports; revert-bitten test). The spoken contract — what all six open rows run on — is still unproven against a deliberate error. Driving card: `qa/HANDOFF-di-mic-sitting-2026-08-10.md` (now points at #86/#87 too). **After the sitting:** wording fixes it flags → `/add-di-loop` thin wrapper → `picture-vocabulary` (named next literacy conversion) + `phoneme-explorer` portfolio call. **Filed residual, not fixed:** a stray utterance mid-build now costs up to 8s (`submitGestureAttempt` abandon rule — needs a bench or second drive). Queue: `qa/di/BACKLOG.md` item 16 (ports + the dated runner block). Detail: prior snapshot below. | 2026-08-10 |
| **🆕 Pilot onboarding (invite-only)** | **ACTIVE 2 — slot RULED this reconcile** (user-opened 08-10 above the fence; science parks to make it legal) | Slice 1 shipped, probe 6/6 live, uncommitted. **Pull = queue item 1: the lesson session record** (register the `session-summary` stub + stamp `session_id` on submissions) — machine-gated, and items 2–3 build on it. Browser gate on the deep-link signup form = **#88, filed this run — it gates sending invite #1** (criterion (e) is the exact K-planned-as-Grade-1 bug slice 1 fixed server-side). WATCH item 4 (`parent/link-student` unverified) before invite #2. Queue: `qa/pilot-onboarding/BACKLOG.md`. | 2026-08-10 |
...
| **⚠️ ORPHANED PAIR — `scaffoldAudit.ts` + `interpolateTemplate.ts`** | **NOBODY'S, day 2 — needs a ONE-LINE user ruling at `/ship` time: commit or revert** | Unchanged since 08-08; the session it belonged to never returned. 82 lines riding along in every `git status`. Item 16 has since filed the family-wide `/tutor-test` harness debt these files partially address, which argues COMMIT — but they are unmeasured and `/pm` has twice declined to rule. | 2026-08-10 |
