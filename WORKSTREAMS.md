...
| 🔝 **JUDGED-LOOP FAMILY** | **ACTIVE** | ✅ `sentence-analyzer` SHIPPED 08-17 — DI port 20, item 22 port **3/5**, all four modes, 30/30 judged. Two of its five biggest findings were UPSTREAM of the port; neither was findable without a live call. Mic #106. **➡️ Next: `/add-di-loop figurative-language-finder`** (port 4/5; `idiom` → item 23). **Alt:** 19h-i-b 9–11. | 08-17 |
| 🎙️ **Judged-loop human queue** | 7 mic rows + 2 | **#100** story-talk · **#101** word-workout · **#102** word-sorter · **#103** word-builder · **#104** text-structure-analyzer · **#105** genre-explorer · **#106** sentence-analyzer *(metalanguage; "noun" nested in "pronoun")*. Plus **#63** (BLOCKS code) and **#90**. Ports ship on gates + probe + `--di`. | 08-17 |
