...
| 🔝 **JUDGED-LOOP PORTS** | **ACTIVE** — `qa/di/BACKLOG.md` | Shipped: 18 math port, 31, **34**, **36 ✅ 09-07** (`explain_concept` — `concept_statement` BENCHED 0/32 false affirms, paraphrase 8/8; 6/6 fresh explain objectives `SUFFICIENT` when the block ships; `DI_PORTS` adapter closes #137's no-drive; mic #139; residual = fresh-draw yield ~60%, queued). Open: **33** `poetry-lab[rhyme_hunt]`; 30 owns 2 cap WARNs (+ a third witness: the ten-rod sitting's off-script drift). **35 RETRACTED**. **Next = `gas-laws-simulator` · `ph-explorer`.** | 09-07 |
| ↳ 🧮 **DI for older learners** | **NEW 09-07** — brief + pilot SHIPPED | User: *"modern di-stylized primitives for older learners… do the work"*. Brief = 7 concepts (artifact `8e67009d`). Pilot **`di-worked-procedure`** (talk-through subtraction, item **37**): the first pack that judges a MOVE — new `procedure_step` class (build-ahead), code-owned step chain, a stage that SCRIBES each affirmed step. tsc 0 · 33/33 · live gen 9 items/0 drops · plain + signature drives **PASS 9/9** · cap PASS (item 30 WARNs) · **bench 48/48, zero false affirms** (filler clause fix, re-bench 14/14); mic/eyes **#140**. **Second pack `di-deduction` SHIPPED same day (item **38**)**: rule + case judged on VERDICT + REASON, new `deduction` class (build-ahead), three code-built shapes (conclude β2.5 / deny β3.5 / cannot_tell β4.5), flash-latest truth review that TRIMS entities. tsc 0 · 35/35 · plain **6/6** · signature **4/4 + 2/2 cannot_tell** · cap PASS · **bench 67/67, zero false affirms** (harness bench-journey leak-span gap fixed); mic/eyes **#141**; no published G3–5 solve surface yet (`/curriculum-author`). BOTH UNCOMMITTED. **Next = the two class sittings, then carrying sibling / `di-error-hunt`.** | 09-07 |
...
| **Curriculum fit atlas** | **ACTIVE** | 12 themes, **3 built**: `letter-workshop` L0–L4, `you-and-me`, **`story-bridge` BORN 09-07** (judged gesture, DI drive PASS, K fit MATCH; 6 reqs ride `/add-eval-modes`). Queue `qa/curriculum-coverage/build-status.md` (+ `.json`). Owed: 2 mic sittings; P0 DSP-4/5 + PV-3 (DSP-4 gates `k-grammar-completion`). | 09-08 |
| **K Math fit atlas** | **ACTIVE** — pull P0 | 166 rows reviewed 09-07 (91/67/8); 20 pairs x 2 draws; 12 findings queued in EVAL_TRACKER (NS-3/4, TF-4, NB-1, OL-1, ASS-1, MFF-1, HC-4, CB-4, TS-4, CE-2, SS-6, CO-6). Queue = `qa/curriculum-coverage/math-k/` atlas Development tab; P0 `count-from-answer-leak`, `teen-numbers-ten-plus`. Executor `/eval-fix`. | 09-07 |
