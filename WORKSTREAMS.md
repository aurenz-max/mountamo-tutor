...
| 🧪 **Lesson Bench** | **ACTIVE**, `qa/lesson-bench/BACKLOG.md` | Items 13/17/19/20 SHIPPED. 21+22 WARN, fixes landed. **22(c): Q9 length now advisory-only** (estimates, not actuals) — scorer never sets `checks.Q9`, vitest 83/83. **23 (CVC-decoding): SUPPLY gap**, KC-orchestrator fix unconfirmed. **24 (kindergarten-subtraction): mirrors 21** — KC-1 (`knowledge-check` named-set drop, shared generator, NOT patched) + **DIMF-1 FIXED 09-05** (`di-math-facts` catalog line now excludes sign-identification objectives; H2 confirmed by headless resolver ×3; fresh package `…e1b7` judge PASS 1.00 — the first PASS on this topic; KC-1 still open on n=1); G1/Q8 equation-builder is a known dup of 21(d)/reader-fit 20a. **Pull = 19(a) calibrate**, then 23, 22(d), 21(b), 24(a). **#125, #134** open. | 09-05 |
