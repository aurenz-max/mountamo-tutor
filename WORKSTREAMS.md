...
| **Curriculum fit atlas** | **ACTIVE** | 35 no-candidate reqs → 12 themes, **3 built**: `letter-workshop` L0–L4 (`a7233f8e`), `you-and-me`, **`story-bridge` BORN 09-07** (L0+L2 judged gesture; headless DI drive PASS; K fit MATCH; 6 more reqs ride `/add-eval-modes`). Queue: `qa/curriculum-coverage/build-status.md` + `build-status.json` (atlas layer). Owed: both browser/mic sittings; P0 repairs DSP-4/5 + PV-3 (DSP-4 gates `k-grammar-completion`, 16 reqs). | 09-07 |
