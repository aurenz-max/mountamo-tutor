...
| **Curriculum fit atlas** | **ACTIVE** | K Language Arts: 191 reviews. `letter-workshop` trace/copy/write practice built; 284 tests + nine mode/tier browser flows pass, K fit MATCH (birth scope). Build queue: `qa/curriculum-coverage/build-status.md`; tutor scaffold, support withdrawal, and scoped structural tiers now pass; next: full live coaching/device review and copy/write calibration (feedback currently local-only). Repairs DSP-4/5 and PV-3 remain queued. | 09-07 |
