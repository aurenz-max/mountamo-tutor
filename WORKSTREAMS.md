...
- **Next:** item 1 — `spatial-scene` containment + two-reference prepositions (4-5 subskills,
  contract **C2 OPEN**). Take its own recommended split: `in` + `between` are buildable now
  (and **`in` inverts R11** — read it first), while `in_front_of`/`behind` needs a design
