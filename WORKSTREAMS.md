...
| 🔝 **JUDGED-LOOP PORTS** | **ACTIVE** — `qa/di/BACKLOG.md` | Shipped: 18 math port, 31, **34**. Open: **33** `poetry-lab[rhyme_hunt]`; 30 owns 2 cap WARNs. **35 RETRACTED** (was SELECTION, see lesson-bench 31). **36 NEW**: explain-verb math unassessable (20/22 rows), banked BEHIND declared order. **Next = `gas-laws-simulator` · `ph-explorer`.** | 09-07 |
...
| 🧪 **Lesson Bench** | **ACTIVE**, `qa/lesson-bench/BACKLOG.md` | 20-28 landed/open. 25 KC P0-P3 built, P4/P5 open, mic #135. 29(a) BUILT, re-judge OWED. 30: (c) CLOSED via di 34, (b) balance-scale open. **31 SHIPPED** fast-fact catalog fix (SELECTION not DI). **32 NEW** phoneme-explorer mis-pinned (MODE). **Pull = 29(a), 32, 26, 27, 30(b), 19(a).** | 09-07 |
