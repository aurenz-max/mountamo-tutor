...
| Direct Instruction family → session voice | ACTIVE | 14g `counting_next` handed off 08-05 (`qa/HANDOFF-di-14g-counting-next-2026-08-05.md`, runs in its own session). **Lane pull = VOICE TRANSPORT UNIFICATION — UN-PARKED 2026-08-05 (user re-pull: "we should be able to natively speak on e.g. foundation-explorer and have the tutor respond — that is not even enabled right now").** Charter `qa/voice-transport/CHARTER.md`; phases 1–2 first (calibration beat → session-wide turn authority with DI as a consumer), pilot = one non-DI primitive speaking natively (foundation-explorer) | Verified 08-05: mic-to-tutor streaming exists ONLY in the 4 DI packs + bench; standard primitives are one-way (tutor speaks, student taps/types). The DI engine stack the charter needs is all live-proven (turn authority, barge-in flush, resumption, catalog audioInput). Item 2 closed (Probe G 11/11, full Vitest 1,569/1,569). **Item 6 DEPRIORITIZED by user ruling 08-05** — imperfect free-form calibration accepted; NO hardcoded grade guardrails in retrieval (pure-IRT family ruling); standalone-only exposure. Item 9 Tier 2 stays demoted-but-queued. Focus per user 08-05: active development — primitives, DI packs, audio/spoken modalities are the foundation. |
...
queue decision under the 08-05 development-first ruling); DI lane: 14g handed off →
**voice-transport unification un-parked 08-05** (item 6 DEPRIORITIZED + item 9
Tier 2 demoted, both user rulings); support tiers **batch-3 evidence closure,
serial**.
