...
- **14h CLOSED 2026-08-04:** number-sequencer blend + scoped Grade-1 120 fidelity is
  ...
  **Next = 14j annotated-example**, then 14k/14l.
...
- **Queue:** `my-tutoring-app/qa/support-tiers/BACKLOG.md` — **AUTHORED `/pm` 2026-08-04** (top =
  next; the remaining 15 by primitive, literacy-first, with two coordination flags:
  hydraulics-lab is OWNED by its reimagining stream, timeline-builder crosses the reader-fit 14m
  sweep — resolver fix first, tiers after, one owning entry per register). The gap named by the
  08-03 reconcile is closed; batch 3 is now pullable cold. The ~59 not-yet-eligible generators
  (need `/add-eval-modes` first) stay un-enumerated by design — opening that frontier is its own
  decision.
