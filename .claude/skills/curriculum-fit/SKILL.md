...
4. **A MATCH is not an assessability signal — never let it pick a build pilot alone.** This skill answers *"does a curriculum home exist?"*. It is completely silent on *"does this primitive have anything to evaluate?"* — the description it embeds is marketing copy about the sim, and an exploration sandbox describes itself exactly like an assessable one. Measured 2026-08-21: the highest-confidence MATCH in the queue-A sweep, `ramp-lab` @ 0.867 / 5-of-5, turned out to be 1172 lines with **zero** occurrences of challenge / answer / correct / submit; 6 of the 11 primitives in its anchor unit were the same. **Before nominating a primitive for `/add-eval-modes` off a MATCH, grep its component for a solve surface** (`onComplete`, `isCorrect`, `solved`, a challenge/job array). No surface → the executor is `/add-structural-difficulty`, not `/add-eval-modes`. Say so in the report rather than handing the next session a pilot that cannot move.
