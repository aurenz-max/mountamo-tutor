...
| **target in a CONTENT-GENERIC generator** (topic may not be numeric at all; scope lives only in the topic text) | `createDiscretePool` over the full benched band, bespoke CONDITIONAL prompt copy | the full legal band (every value present — shuffle order alone is the entropy), topic authoritative for range AND for applicability ("ignore this section for a non-number topic") | `gemini-di-spoken-practice.ts` → `buildSeedSection` |

Three of these carry the load-bearing lessons:
...
- **di-spoken-practice (`buildSeedSection`)** — *the content-generic lesson.* The same generator serves "adding within 10" and "rhyming words", so neither a code classifier (regex-shaped) nor an unconditional pool is right: the seed section is CONDITIONAL IN THE PROMPT ("only if the topic involves numbers … ignore this completely otherwise") and the model reads applicability natively. The candidate set is the full benched band (1-20), so a narrow topic ("within 5") still finds all its legal values in the list after skipping the rest — the shuffle order alone is the entropy. Corollary: pooling the TARGET makes the model back-solve the rest of the item onto it (that is the point), so pair the pool with a consistency gate that DROPS an item disagreeing with its own stimulus (`findArithmeticMismatches`) — never patch the answer field, because other generated fields (the correction) state the claimed answer.
