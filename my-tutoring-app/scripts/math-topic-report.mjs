...
const out=resolve(process.argv[2]??'qa/lesson-planner/math-topic-ab/k-2-3-first-run');
...
const aggregate=[...new Set(rows.map(r=>r.caseId))].flatMap(caseId=>['production','rich','lean'].map(arm=>{
...
const lines=['# Math topic comparison: exact streams',''];
for(const r of rows){
