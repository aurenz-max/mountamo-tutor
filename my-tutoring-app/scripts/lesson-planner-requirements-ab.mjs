...
import { familyEvidenceHopper as pairHopper } from './lib/lesson-planner-family-evidence.mjs';
import { objectiveContract, requirementsRequest, compileRequirementManifest, inspectTargetScope } from './lib/lesson-planner-requirements.mjs';

const root = process.cwd();
const argv = process.argv.slice(2);
const outArg = argv.indexOf('--out');
const out = resolve(root, outArg >= 0 ? argv[outArg+1] : `qa/lesson-planner/requirements-ab/${new Date().toISOString().replace(/[:.]/g, '-')}`);
...
if (!argv.includes('--run')) { console.log(JSON.stringify({ cases, repetitions: 2, arms: ['experimental'], out })); process.exit(0); }
...
try {
  ...
  const sourcePaths = ['scripts/lesson-planner-requirements-ab.mjs', 'scripts/lib/lesson-planner-family-evidence.mjs', 'scripts/lib/lesson-planner-family-search.mjs', 'scripts/lib/lesson-planner-requirements.mjs', 'scripts/lib/lesson-planner-pairs.mjs', 'scripts/lib/lesson-planner-pilot.mjs', 'scripts/lib/lesson-planner-discovery.mjs', 'scripts/lib/lesson-planner-topic.mjs',
  ...
  save(join(out, 'protocol.json'), { cases, repetitions: 2, grade: 'kindergarten', fixtureSource: 'curriculum-ab', catalogHash: hash(catalog.UNIVERSAL_CATALOG),
    ...
    caveats: ['Fixed objectives; topic-to-objective authoring not tested.', 'Exploratory 10-lesson follow-up, historical comparator only. Protected family retrieval, all family modes, explicit evidence requirements and supported base-ten numberRange binding change together. No generator or judge changes.', 'Two concurrent lessons, per-lesson parallel hydration.', 'API transport timeout 120 seconds in both arms.', 'Coverage judge is blinded to arm, not a substitute for human review.'] });
  ...
  const schedule = existsSync(schedulePath) ? JSON.parse(readFileSync(schedulePath, 'utf8')) : cases.flatMap((caseId, index) => [1,2].flatMap(rep => {
    const arms = ['experimental']; // Exploratory follow-up; compare to the frozen prior run, not a fresh randomized control.
    ...
    await context.run(rec, async () => {
      try {
        ...
        if (task.arm === 'production') {
          ...
          rec.objectiveContract = objectiveContract(input);
          const request = requirementsRequest(capture.capturedRequest, rec.discovery.selected, rec.objectiveContract);
          ...
          const native = compileRequirementManifest(rec.plan, rec.discovery.selected, rec.objectiveContract);
        ...
        rec.scopeInspection = inspectTargetScope(rec.components, rec.objectiveContract);
