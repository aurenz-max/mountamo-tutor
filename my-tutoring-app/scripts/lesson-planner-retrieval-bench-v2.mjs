...
import { familyEvidenceHopper as familyHopper, groupedCards } from './lib/lesson-planner-family-evidence.mjs';

const root = process.cwd();
const out = resolve('qa/lesson-planner/retrieval-bench/family-v2');
...
save('protocol.json', { cases, model: 'gemini-embedding-001', dimensions: 768,
  policy: 'Unchanged flat pair baseline vs protected semantic families plus lexical/fused complements, all family modes, and existing evidence queries. Family metadata is shared in the prompt; task count is not held equal. No manifest generation or scorer changes. Existing consideration labels plus three explicit task regression probes, frozen before execution.',
  sourceHashes: Object.fromEntries(['scripts/lib/lesson-planner-pairs.mjs','scripts/lib/lesson-planner-family-search.mjs','scripts/lib/lesson-planner-family-evidence.mjs','scripts/lesson-planner-retrieval-bench.mjs'].map(p => [p,hash(readFileSync(p,'utf8'))])) });
...
try {
  ...
  for (const c of cases) for (const arm of ['flat','family']) {
    ...
    const row = { id:c.id,arm,fixtureHash:hash(fixture),catalogHash:hash(catalog.UNIVERSAL_CATALOG),latencyMs:Math.round(performance.now()-started),
      ...
      taskCount:retrieval.selected.length,familyCount:found.size,promptCharacters:JSON.stringify(arm === 'flat' ? retrieval.selected : groupedCards(retrieval.selected)).length,retrieval };
    save(filename,row);
