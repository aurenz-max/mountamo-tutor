...
import { compilePlan } from './lib/lesson-planner-compile.mjs';

const root = process.cwd();
const argv = process.argv.slice(2);
const outArg = argv.indexOf('--out');
const out = resolve(root, outArg >= 0 ? argv[outArg+1] : `qa/lesson-planner/production-ab/${new Date().toISOString().replace(/[:.]/g, '-')}`);
...
try {
  ...
  const sourcePaths = ['scripts/lesson-planner-production-ab.mjs', 'scripts/lib/lesson-planner-compile.mjs', 'scripts/lib/lesson-planner-pilot.mjs', 'scripts/lib/lesson-planner-discovery.mjs', 'scripts/lib/lesson-planner-topic.mjs',
  ...
  save(join(out, 'protocol.json'), { cases, repetitions: 2, grade: 'kindergarten', fixtureSource: 'curriculum-ab', catalogHash: hash(catalog.UNIVERSAL_CATALOG),
    ...
    caveats: ['Fixed objectives; topic-to-objective authoring not tested.', 'Production default has no duration parameter; experimental retains its 15-minute target.', 'Two concurrent lessons, per-lesson parallel hydration.', 'API transport timeout 120 seconds in both arms.', 'Coverage judge is blinded to arm, not a substitute for human review.'] });
    ...
    await context.run(rec, async () => {
      try {
        ...
        if (task.arm === 'production') {
          ...
          input.generalPurposeCandidateIds = [...catalog.CORE_CATALOG, ...catalog.ASSESSMENT_CATALOG].map(c => c.id);
          rec.discovery = await prepareSemanticDiscovery(input, ai, join(root, 'qa/lesson-planner/discovery-cache'));
          rec.neighborhood = objectiveNeighborhood(input, 10, rec.discovery);
          const planInput = { ...input, candidates: rec.neighborhood.candidates };
          const response = await ai.models.generateContent({ model: 'gemini-flash-latest', contents: buildPrompt(planInput, 'mode-cards'),
            config: { responseMimeType: 'application/json', responseSchema, temperature: 0.3, thinkingConfig: { thinkingLevel: 'LOW' }, maxOutputTokens: 10000 } });
          rec.plan = JSON.parse(response.text);
          rec.planValidation = validatePlan(rec.plan, planInput);
          manifest = flatten.enrichManifestWithLayout(compilePlan(rec.plan, { ...frozen.fixture, verb: frozen.objective.verb }, catalog.UNIVERSAL_CATALOG), [frozen.objective]);
        }
