...
mkdirSync(join(out,'lessons-text-only'),{recursive:true});
...
for(const name of ['log','warn','error','debug','info'])console[name]=(...args)=>appendFileSync(join(out,'lessons-text-only/runtime.log'),format(...args).replaceAll(process.env.GEMINI_API_KEY,'[REDACTED]')+'\n');
...
try{
 ...
 const retrieval=read('teacher-text-bench/summary.json');
 save('lessons-text-only/protocol.json',{topic,objectives,arms:['descriptions','exemplars','hybrid'],selection:'One chooser call per arm. Union of top 4 task results for sound/blend-print/spell queries, plus identical foundation/fast-fact/knowledge-check support pool. Same source-reviewed capability metadata in all arms.',
  caveats:['Exploratory one build per arm, no statistical quality claim.','Objectives are tester-authored decomposition of the user topic.','Review is model/source-assisted, not human certification.','Target-field inspection is not full lesson coverage evaluation.'],sourceHashes:{runner:hash(readFileSync('scripts/literacy-exemplar-text-lesson-bench.mjs','utf8')),catalog:hash(catalog.UNIVERSAL_CATALOG)}});
 const sharedBrief=await service.generateIntroBriefing(topic,'kindergarten');
 sharedBrief.objectives=objectives;
 save('lessons-text-only/shared-brief.json',sharedBrief);
...
State limitations and uncovered requirements honestly. Intent must explicitly preserve the five-letter set and short-i scope for reading/spelling. Do not add a or other letters as targets. Pictures or narration may contain other words; the constraint concerns target letters and words used for phonics practice. JSON only.`;
  ...
  save(`lessons-text-only/${arm}.json`,record);say({phase:'planned',arm,tasks:plan.activities.map(a=>a.taskId)});return record;
 }));
 for(const r of plans){
  ...
  pkgMod.parseLessonPackage(r.package);
  r.packageValidated=true;
  save(`lessons-text-only/${r.arm}.json`,r);
  if(!r.package.error)save(`lessons-text-only/${r.blindId}.package.json`,r.package);
