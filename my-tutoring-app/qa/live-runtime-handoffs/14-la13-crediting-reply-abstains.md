...
| di-worked-procedure (C6, 09-26) | a column step whose task or scene names the whole problem ("Sixty-nine minus fifty-four. Look at the ones column…"; an object label "the problem 92 − 75…"; a `printedProblem` fact), credited "nine minus four is five" | refused 3/3 on replay each (`uncertain_or_invalid` / `unsupported`); the same exchange with the column-only task credits 3/3. Worked around in the binding (no problem numbers reach the observer); the observer has no notion of one step of a larger printed problem **Part 2 (09-26):** binding workaround kept; probe `column_fact` still lands below the gate (0.60) and resolves through `confirm_credit`. | [C6 report](../tutor-reports/workspace-rollout-C6-2026-09-26.md) |
| di-worked-procedure (C6, 09-26) | confirming turn "That's right, you nailed the regrouping step and are ready to subtract!" after a `confirm_credit` at correct 0.88 | `unsupported` 3/3 on replay; scoping the key or a fact to "this item is only the decision" changes nothing (0/3 each). Candidate 1 below **Part 2:** still `none` on replay; wording c1c kept on a user ruling; 26/27 step credits now clear the gate on the first reply in connected runs; a non-crediting confirmation reopens the item instead of stalling. | `di-worked-procedure-w1-subtract_regroup-audio-2026-09-26.json` |
| di-word-problem-setup (C6, 09-26) | a checked-correct build, then "Great job placing the starting number of shells in the big spot!" | transition advance 0.87 < 0.9, `uncertain_or_invalid`; the drive stalled on a solved hands step **Part 2: fixed** — a checked-correct response settles on finished feedback (6/6 builds advanced in connected runs). | `di-word-problem-setup-w1-build_family-audio-r1-2026-09-26.json` |
| di-word-problem-setup (C6, 09-26) | family credited (correct 0.94), then "You got it right by saying five plus box equals seven!" and "…let's keep going!" | `transition_uncertain` (advance 0.57, then 0.64): a held success that never advances. Same step framing as candidate 1 **Part 2: fixed** — a confirming reply credits at the gate too (probe 3/3 `correct/advance`). | `di-word-problem-setup-w1-build_family-audio-2026-09-26.json` |
