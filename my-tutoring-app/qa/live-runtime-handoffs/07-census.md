...
| `math/ThreeDShapeExplorer.tsx` | live + ordinary lessons | **workspace only 09-25** (batch C4): `withWorkspaceOnly`, `useWorkspaceRunner`; every item is one spoken answer (solid name, flat or solid, a property count or verdict, a face shape, a riddle). `threeDShapeExplorerScript.ts` stays for its truth-table gates, the DI drive plan and pure tests. [Report](../tutor-reports/workspace-rollout-C4-2026-09-25.md) |
| `calendar/CalendarExplorer.tsx` | live + ordinary lessons | **workspace only 09-25** (batch C4): two surfaces, both `withWorkspaceOnly`. The grid (plain shape) runs on `useWorkspaceProgress`: a date or option pick plus Check is the activity's check (`commitCheck`); Next, the three-miss answer reveal and the improvised tutor sends are gone. The spoken day/month chain runs on `useWorkspaceRunner`. Four runner- and click-era mount suites moved into `CalendarExplorer.workspace.test.tsx`. `calendarExplorerScript.ts` stays for the DI drive plan and pure tests. [Report](../tutor-reports/workspace-rollout-C4-2026-09-25.md) |
| `physics/PushPullArena.tsx` | live + ordinary lessons | **workspace only 09-25** (batch C4): `withWorkspaceOnly`, `useWorkspaceRunner`; every item is one spoken word computed from the sim. observe is ready for an answer only after the learner presses Go; predict and compare now run the push once the answer is credited (the runner ran it when an attempt opened, an emission the workspace does not have). `pushPullArenaScript.ts` stays for its build gates, the DI drive plan and pure tests. [Report](../tutor-reports/workspace-rollout-C4-2026-09-25.md) |
| `biology/HabitatDiorama.tsx` | live + ordinary lessons | **workspace only 09-26** (batch C5): `withWorkspaceOnly`, `useWorkspaceRunner`; observe, predict and defend are one spoken choice from the screen; connect (tap the living thing the relationship leads to) and restore (tap the zone) are checked by the activity (`commitGesture`). A payload with no askable challenge stays the ungraded exploration diorama. The runner-mock stage suite moved into `HabitatDiorama.workspace.test.tsx`. `habitatDioramaScript.ts` stays for its build gates, the DI drive plan and pure tests. [Report](../tutor-reports/workspace-rollout-C5-2026-09-26.md) |
| `chemistry/MatterExplorer.tsx` | live + ordinary lessons | **workspace only 09-26** (batch C5): `withWorkspaceOnly`, `useWorkspaceRunner`; every item is one spoken answer computed from the object (state, what it does in a cup, whether a change goes back, or a secret object's state from clues). The tier's rule clause rides in the scene facts below hard; mystery never names the object to the tutor before credit. A payload with no askable challenge stays the ungraded exploration shelf. `matterExplorerScript.ts` stays for its build gates, the DI drive plan and pure tests. [Report](../tutor-reports/workspace-rollout-C5-2026-09-26.md) |
| `chemistry/StatesOfMatter.tsx` | live + ordinary lessons | **workspace only 09-26** (batch C5): `withWorkspaceOnly`, `useWorkspaceRunner`; every item is one spoken answer computed from the substance table (the state its particles show, the state it will reach or the change it goes through, which of two melts first or stays solid). The beaker ramps to the new temperature once the answer is credited; the tier's rule clause rides in the scene facts below hard. A payload with no askable challenge stays the ungraded particle sim. `statesOfMatterScript.ts` stays for its build gates, the DI drive plan and pure tests. [Report](../tutor-reports/workspace-rollout-C5-2026-09-26.md) |
| `astronomy/SolarSystemExplorer.tsx` | live + ordinary lessons | **workspace only 09-26** (batch C5): `withWorkspaceOnly`, `useWorkspaceRunner`; every item is one spoken planet name computed from the bodies on screen (classify accepts any member). A tap is looking: it opens the body card from Grade 2, never answers. The identify and pair spotlight now paints when the item opens (the runner held it until the ask was spoken; W1 has no `present`). Labels stay withheld on identify. The runner-mock stage suite moved into `SolarSystemExplorer.workspace.test.tsx`; the explore face stays pinned by reader-fit. `solarSystemScript.ts` stays for its build gates, the DI drive plan and pure tests. [Report](../tutor-reports/workspace-rollout-C5-2026-09-26.md) |
...
| `direct-instruction/DiSpokenPractice.tsx` | live + ordinary lessons | **workspace only 09-26** (batch C6): on `DiTeachingStage`; every mode is one spoken answer judged against a key that carries the pack's alternates, word menu, `acceptRule`, `signatureError`, and for `explain_concept` the one idea judged on meaning (`diSpokenPracticeWorkspace.ts`). Tap-to-hear, the runner and its Pip pose are gone. `diSpokenPracticeScript.ts` stays for the generator gates, the DI drive plan and the concept bench. [Report](../tutor-reports/workspace-rollout-C6-2026-09-26.md) |
