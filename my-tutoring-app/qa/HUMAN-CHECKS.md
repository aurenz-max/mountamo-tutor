...
> **116 rows open.** **Newest:** #165 Live transcript refusal-phrase ear check; #148 Ramp Lab investigation MIC + tablet; #147 Bar Model graph explanation MIC ? #146 Story Ribbon tablet + MIC + sound · #145 Story Bridge
> tablet + MIC · #144 You & Me role-switch MIC · #143 Letter Workshop manuscript/touch/audio ·
> #142 ten-frame teen numbers. Next free ID is **#166**.

### #165 🎤 — **Gemini Live: is "I'm just a language model and can't help with that." SPOKEN at the end of a resumed count, or transcript-only?** Both `gemini-3.8-live` and production `gemini-3.1-flash-live-preview` ended the resumed turn with the output transcript `Twenty-oneI'm just a language model and can't help with that.` in 2/3 standalone runs each (byte-identical). Two Gemini transcribers emit the phrase at different points and the WAV has continuous speech energy to the end, so a machine could not settle it. **Listen:** `qa/tutor-reports/audio/gemini-3.8-live-resumed-count-refusal-tail-2026-09-15.wav` (control with a clean ending: `…-resumed-count-clean-2026-09-15.wav`). If spoken → child-facing defect on both models, report to Google; if transcript-only → strip the phrase before the DI sentinel scan and the misconception `observed` string builder. Report: `qa/tutor-reports/live-model-ab-gemini-3.8-live-2026-09-15.md` §5. di item 43(d).

### #164 🎤👀 — **`ten-frame` operate: does saying the starting number back become a saved observation, and does the next operate activity show two neighbouring items with the same first number and a different second?** Implemented 2026-09-14, machine-verified with fictional evidence: mounted capture on the real runner, real distiller 8/8, planner 16/16, registry generation 33/33, delivery replay with a Python-signed packet ([report](misconception/ten-frame-2026-09-14.md)). Signed in, on a real microphone, in a Kindergarten `OPS001-02-D` lesson with `ten-frame` operate: (a) on at least three of five take-away items say the number the frame STARTED with ("seven" for seven take away three), let the tutor correct you, answer right, and finish; (b) My Progress shows a suspected ten-frame observation whose evidence quotes what was heard with each item's start and number taken away; (c) the next operate activity on the same skill has two neighbouring items with the same start and a different number taken away (for example eight take away three, then eight take away five), the tutor says both, and no answer is printed; (d) on a subtraction double (eight take away four) say "four": the tutor affirms it; (e) teaching judgment: does the same-start pair make the child's answer change, or does it read as a repeat? Owner: user.

### #163 🎤👀 — **`counting-board` take_away / add_more / count_on: does saying the number from before the change become a saved observation, and does the next activity show two boards that start the same but change by different amounts (or a count-on with one more)?** Implemented 2026-09-14, machine-verified with fictional evidence: mounted capture on the real runner, real distiller 8/8, planner 20/20, registry generation, delivery replay with a Python-signed packet ([report](misconception/counting-board-2026-09-14.md)). Signed in, on a real microphone, in a Kindergarten `COUNT001-02-E` lesson with `counting-board` take_away: (a) on at least three of five boards take the objects away, then say the number that was on the board BEFORE taking any ("seven" for 7 take away 3), let the tutor correct you, answer right, and finish; (b) My Progress shows a suspected counting-board observation whose evidence quotes what was heard with each board's start and change; (c) the next take_away activity on the same skill has two neighbouring boards with the same start and different changes (for example 7 take away 1, then 7 take away 3), the tutor's asks say those changes, and nothing on screen prints a count; (d) repeat (a)–(c) for add_more (`COUNT001-02-F`) saying the start before adding; (e) in a `COUNT001-02-G` count_on lesson say the basket's number back on three boards; the next count_on activity has one board with exactly one object beside the basket; (f) teaching judgment: does the same-start pair or the one-more board make the child's answer change, or does it read as a repeat? Owner: user.

### #161 👀 — **`number-tracer` sequence: does a child's handwriting get judged fairly, does writing the number after the run become a saved observation, and does the next activity show one run twice with the gap moved?** Implemented 2026-09-14, machine-verified with synthetic drawings and fictional evidence: real-app drives in all four modes, real distiller, planner and registry generation, authenticated store → delivery → generation PASS with cleanup ([report](misconception/number-tracer-2026-09-14.md), [grader fix](eval-reports/number-tracer-2026-09-14.md)). On a tablet with a finger, signed in, in a Kindergarten `COUNT001-01-D` lesson with `number-tracer` sequence: (a) no dotted guide, start dot or arrow appears for the hidden number at any tier; (b) a correct but untidy numeral is accepted, and a clearly different numeral is rejected with feedback naming what it looks like (never "nothing was drawn"); (c) on at least three of five runs write the number after the last shown number (3, 4, ?, 6 → 7), correct it, and finish; My Progress shows a suspected number-tracer observation listing those readings; (d) the next sequence activity on the same skill shows the same four numbers on two neighbouring items with the gap in different places; (e) neither the screen nor the tutor names the hidden number or mentions the observation; (f) copy and write checks take about 1-3 s longer than before — say whether that wait is acceptable. Known limits: sessions never show 10 or teens at K (NT-11); the tutor channel is unverified (NT-9). Teaching effectiveness and transfer are not claimed. · OPEN

### #160 👀 — **`area-model`: do a tens × tens slip and an area-for-perimeter slip become saved observations, and does the next activity show the matching contrast?** Implemented 2026-09-13, machine-verified with fictional evidence: real distiller, planner and registry generator draws, authenticated store → delivery → generation PASS with cleanup ([report](misconception/area-model-2026-09-13.md)). Signed in: (a) in a Grade 4 `NBT004-06-d` lesson with `area-model` find_area, type the tens × tens cell with one zero missing (30 × 40 as 120) in at least three of five models, correct it, and finish; My Progress shows a suspected area-model observation whose evidence lists those cells; (b) the next find_area activity on the same skill has one model whose largest and smallest cells use the same fact (for example 23 × 32: 20 × 30 and 3 × 2); (c) in a Grade 4 `MEAS004-04-b` perimeter lesson, enter length × width on the first try for most rectangles; the next perimeter activity places two rectangles with equal area and different perimeters next to each other; (d) nothing on screen or in the tutor's speech mentions the observation; (e) whether either contrast reads as a contrast to a child. Known limits: the submitted score still reads 100 when one cell per model was wrong (AM-3); about 1 in 20 perimeter sessions has no legal equal-area pair and stays unchanged. Teaching effectiveness and transfer are not claimed. · OPEN

### #159 👀 — **`number-line` jump: does counting the start as a hop become a saved observation, and does the next jump activity pair a zero-anchored jump with the same hop from another start?** Implemented 2026-09-13, machine-verified with fictional evidence: real distiller, planner and generator draws, authenticated store → delivery → generation PASS with cleanup ([report](misconception/number-line-jump-2026-09-13.md)). Signed in, in a Grade 1 `OPS001-03-a` (count on within 20) lesson with `number-line` jump at medium: (a) on two jumps tap the tick one hop short (for 8 + 3, tap 10) and confirm Check marks it wrong, then correct it and finish; (b) My Progress shows a suspected number-line observation whose evidence lists the first landings; (c) the next jump activity on the same objective has a jump starting at 0 (or subtracting down to 0) beside a jump with the same hop size from another start; (d) no hint counts the hops out loud or names the landing. Teaching effectiveness and transfer are not claimed.

### #158 👀 — **`fraction-circles` compare: does a tapped "more slices is bigger" pattern become a saved observation, and do the next activity's same-numerator pairs help a Grade 3 child?** Implemented 2026-09-13, machine-verified with fictional evidence: real distiller, planner 27/27, registry generations, authenticated store → delivery → two targeted draws PASS with cleanup ([report](misconception/fraction-circles-2026-09-13.md)). Signed in, in a Grade 3 `NF001-06-b` lesson at medium, play `compare` and on the first try pick the circle cut into more slices on at least three of five pairs (for example 1/8 over 1/4), then finish. Check: (a) My Progress shows a suspected fraction-circles observation whose evidence lists the first choices; (b) the next `compare` activity in the same skill contains two pairs sharing a numerator (for example 2/4 vs 2/6), with instructions matching the circles; (c) nothing on screen or in the tutor's speech mentions the observation; (d) whether the two pairs read as a contrast to a child. Known limits: two first-try misses in five pairs saves nothing (shared 60% gate); untouched pairs may still show fifths, tenths or twelfths at Grade 3. Teaching effectiveness and transfer are unverified. · OPEN

### #157 🎤👀 — **`base-ten-blocks` read_blocks as an observation SOURCE: does a spoken bare-count answer become a saved observation, and does the next blocks activity pair equal counts of different sizes?** Implemented 2026-09-13, machine-verified with fictional evidence: real distiller 5/5, real planner/generator 12/12, authenticated capture → profile → delivery → two targeted draws PASS with cleanup ([report](misconception/base-ten-blocks-origin-2026-09-13.md)). Signed in, in a Grade 4 `NBT004-01` lesson at medium, play `read_blocks` on a real microphone and answer the WORTH steps with the bare count ("four" for four thousand) until the tutor moves on, on at least two mats, so the activity fails. Check: (a) My Progress shows a suspected observation from base-ten-blocks whose evidence quotes what ASR heard; (b) the next `read_blocks` activity in the same skill contains two mats with the same count of different block sizes; (c) the child never sees the observation text; (d) a strong later run does not mark it resolved. Machine gates cannot check the ASR text, whether the paired mats read as a contrast to a child, or teaching effectiveness. Also try the pattern the average hides: say the bare count once on EVERY worth step, then the right answer after each correction; that run passes at 67% but must still save an observation (first-response 50%). Known limit: a single missed worth step across three mats saves nothing.

### #156 👀 — **`fraction-bar` build after a saved role-swap observation: does the consecutive contrast (3/4, then 4/5) actually help a Grade 3 child keep numerator and denominator apart?** Implemented 2026-09-13, machine-verified: 19/19 real applicability draws, authenticated store → delivery → generation PASS with cleanup ([report](misconception/fraction-bar-2026-09-13.md)). In a Grade 3 NF001-03 lesson, play `build` once choosing the bottom number for the numerator on two fractions and shading the denominator's count once, so the session saves an observation; then launch another `build` activity in the same skill. Check: (a) two consecutive fractions share a number in swapped roles and both numbers appear in each choice row; (b) nothing on screen or in the tutor's speech mentions the saved observation; (c) whether the pair, placed back to back, is noticeable to a child at all, given most ordinary sessions already contain one by chance. Teaching effectiveness and transfer are unverified. · OPEN

### #155 🎤👀 — **`base-ten-blocks`: the concrete half of place value, spoken — can a G2 child say what a size of block is WORTH, and predict a trade before making it?**

Judged loop shipped September; this slice gave it tests, a drive adapter and its FIRST headless drives (2026-09-12). Machine report: [base-ten-blocks-di/README.md](base-ten-blocks-di/README.md). 99 tests across four suites, `typecheck:lumina` 0, six real generated sessions (24 challenges / 48 items, 0 dropped, 0 pack-gate issues), and **three headless drives all PASS with 0 findings** — read_blocks plain 8/8, regroup plain 8/8 including all four hands items, and `--di-wrong signature` **10/10**, which is the one that matters: the two signature errors are mirror images on the same mat and the judge discriminated in both directions. The drives found four defects no machine gate could see, all fixed: an ask that stated its own answer on any mat with an empty receiving place, verdict lines invisible to the shared span parser, broken subject-verb agreement at a count of one, and a sign-off that described the wrong activity.

In **Math Primitives Tester → Base-Ten Blocks**, drive both modes on a real microphone. **(a) THE `-ty`/`-teen` EAR, which is why this row exists.** `place_value_word` is accepted-build-ahead on the **#63** sitting and its known near-pair is exactly this pack's answer material: say "forty" and "fourteen" as a G2 child would and check what ASR returns. A drive sends TEXT and cannot see this. **(b) The mirror-image pair by ear.** Answer the count with the VALUE ("forty" where "four" was asked), then the value with the bare COUNT ("four" for "forty") — both must be refused, and the correction is where "one ten-stick is worth ten, so four of them are worth forty" gets taught. **(c) A one-block mat**, where the pair is "ten" (the value) against "one" (the count) — the hardest discrimination in the pack, and the place the grammar was broken. **(d) The trade, with a finger.** Tap the asked-for block and watch it split; does the 900 ms commit fire before a child changes their mind, and is the 1500 ms wrong-trade hold long enough to notice the mat changed? Tap the WRONG size and confirm the correction; use "Put the blocks back" mid-turn and confirm the tutor re-coaches without a verdict. **(e) Fourteen blocks on one column** — a traded mat holds 14 ten-sticks or 17 ones cubes; check it does not reflow illegibly at tablet width and that the animation reads as "one block became ten". **(f) The prediction, with pauses** — "ten… and four more… fourteen": one turn under the close, or three? **OPEN — text-driven Live tests do not test acoustics, ASR, touch ergonomics or animation.**

### #153 ? **Fraction Circles: Touch the Fraction ? tablet, audio, and mixed-session handoff**

Implemented 2026-09-12. [Report](fraction-touch/README.md). 27 focused frontend tests, 3 backend calibration tests, typecheck 0, five real generator scenarios pass. Live DI drive: 5 wrong touches corrected, 5 right touches affirmed, hands-hold silent; no findings. In Math Primitives ? Fraction Circles ? Touch the Fraction: inspect circle size/partitions on a narrow screen; intentionally touch wrong then right; retry twice through the correction cap; replay the ask; rapid-tap while a verdict is pending. Confirm no printed fraction labels, answer-position hints, or duplicate commits. Test touch+build handoff and one final aggregate. **OPEN:** browser automation was unavailable; the headless drive does not verify real touch or audio pacing.

### #154 🎤👀 — **`number-bond`: persistent counters and a spoken turn on every mode — does a child actually build, split, and answer by hand and voice the way the six modes now expect?** Implemented 2026-09-11/12. Machine report: [number-bond-split-and-say/README.md](number-bond-split-and-say/README.md). 87 focused tests across six suites pass; six real generated sessions / 18 source challenges / 66 runtime turns, no contract issues. In Math Primitives Tester select Number Bond and try all six modes. (a) Split and Say (`decompose`) — split five into two and three, answer the highlighted part, then move a counter directly between parts to make the next pair; verify incomplete motion, Undo, Bring back together, a duplicate pair, and an empty part. (b) Ten and Ones — for fourteen, try six/eight, repair it to ten/four, answer "four"; try the ten on either side. (c) Related Facts — join three and four, answer "four", move the same blue group away, answer "three"; a replay or correction must preserve the committed model. (d) Missing Part — inspect the cover visually and with a screen reader; answer both independently and after requesting counters; the cover must never leak through focus or animation. (e) Build Equation — join then submit `7 - 4 = 3` and confirm action-specific coaching; accept `4 + 3 = 7` and `7 = 3 + 4`; repeat with a separation and an equal-part bond. (f) Fact Family — complete four unequal-part transforms and two equal-part transforms; confirm prior equations stay visible and reversed equality doesn't inflate coverage. Across modes: confident wrong answers, help questions, silence, replay, capped/modelled responses, source resets, and mixed sessions. **OPEN — text-driven Live tests do not test animation, touch ergonomics, or live microphone judging.**

### #152 🎤👀 — **`balance-scale`: the weight workshop's six touch+voice modes — does a child actually match, compose, share, and connect physical moves to equations by ear and hand?** Implemented 2026-09-11, superseding the click-only equation port (`equality`/`equality_hard` first landed as a smaller pilot, then the full six-mode workshop). Machine reports: [balance-workshop/README.md](balance-workshop/README.md) (current), [balance-equality/README.md](balance-equality/README.md) (superseded pilot, historical). 97 tests across 10 suites pass; 18 real generated sessions / 54 challenges / 225 script steps, no contract issues; `typecheck:lumina` 0. In Math Primitives Tester select Balance Scale and try all six modes: (a) `equality` — tap weights to match the unnumbered block, watch the 900ms auto-commit and the gather animation, say the sum, then answer the separate "what's the left weight" question; try breaking a near-match before it settles. (b) `equality_hard` — build a match, say its total, build a genuinely different combination, say that total too. (c) `one_step` — add to a known load until balanced, say the added weight then the missing part. (d) `one_step_hard` — distribute units among identical opaque parcels, say each group's weight and one parcel's weight. (e) `two_step_intro` — separate known weight from both pans, name what remains, then share it. (f) `two_step` — the same physical moves build equation lines; confirm a line appears only after its action and that x stays hidden until after the group-weight turn. Across modes: undo/reset, overshoot, a capped hand turn's attributed tutor demonstration. **OPEN — text-driven Live tests do not test acoustics, animation, or child pacing.**

### #151 🎤👀 — **`syllable-clapper`'s two new acts: can a five-year-old's blended multisyllable word and their compound leftover actually be heard?** Shipped 2026-09-12 (`qa/di/BACKLOG.md` item 40) machine-verified: `typecheck:lumina` 0 in every touched file, 77/77 tests across 3 suites, 3 revert-bites, live probe 7 draws / 3 acts / 0 pack-gate issues, and **6 headless drives all PASS with 0 HIGH** — blend plain 8/8, blend signature 8/8 (the parts said straight back, refused every time), delete plain 8/8, delete signature 8/8 (the whole word echoed, which contains the answer, refused every time), count_parts regression 8/8, `--di-cap` 0 HIGH + the known 18c pair. Three things need an ear, and the first is the reason this row exists. **(a) NEW ANSWER MATERIAL for a benched class.** `short_spoken_word` is benched on short CVC words; `blend_syllables` answers with "kangaroo", "watermelon", "helicopter". `--di` sends TEXT, so it cannot see whether ASR returns a K child's blended 3-4 syllable word intact, or whether a child who blends slowly ("kan… ga… roo") arrives as ONE turn under the 500 ms close or splits into three. Raise `silenceCloseMs` pack-scoped if it splits — the di-sentence-reading precedent. **(b) Does the chant actually teach the blend?** In the Literacy Primitives Tester pick Syllable Clapper → Put the Parts Together. Listen to the tutor's "but … ter … fly": are the beats separated enough for a child to hear three pieces, and does the joined word ever leak into the ask? Then answer with the parts said back separately — it must refuse. **(c) The deletion ask under a real ear.** Pick Take a Part Away. "Say starfish without star" — does a five-year-old produce "fish", or repeat the whole word? Say the removed part ("star") and confirm the refusal, then say "fish". Known delivery drift to watch: the tutor dropped the "…" separators in 2 of 8 blend CORRECTIONS ("la dy bug"), which is the earned model losing its beats. · OPEN

### #150 🎤👀 — **`number-sequencer`: child microphone and card-arrangement acceptance across all six modes.** Implemented 2026-09-11; machine report: [number-sequencer-di/REPORT.md](number-sequencer-di/REPORT.md). In Math Primitives Tester select Number Sequencer and try all six modes. (a) Before/after and fill/decade gaps: deliberately say a neighbor, then the missing number; only that blank fills, and the tutor advances without Check/Next. (b) Count-from: echo the starting number, then give the next number; repeat with backward counting, pausing and speaking quickly. Does one-number-per-turn pacing feel natural? (c) Spot-error: say the replacement first (must refuse), then the printed wrong number (must affirm); no card is pre-marked. (d) Order-cards: stop after one card or build a reversed order; a three-second stillness pause must submit the wrong/incomplete arrangement. Speech alone must not advance this hands turn. Check removal/rearranging and the last train holding during affirmation. (e) Stop/global mic toggle and reconnect, then resume; check the end summary and that carried items receive no success credit. (f) Try 21-120, especially fourteen/forty and backward counting across 100; this extends the existing **#63** acceptance material, not a claim of a newly benched class. **OPEN — text-driven Live tests do not test acoustics or child pacing.**

### #149 🎤👀 — **`di-word-problem-setup`: the first hands+voice math pack — does a child PLACE the big number and then SAY the family, and does the placement close at a sensible moment?** Shipped 2026-09-10 (brief concept 4, `qa/di/BACKLOG.md` item 39) machine-verified: `typecheck:lumina` 0, 54/54 pack + stage tests, live generation 0 drops / 0 gate issues in all three modes, every story and cue read by eye; headless `--di` plain PASS 12/12, `--di-wrong signature` PASS 12/12 (the upside-down family drew the "big number goes last" branch 3/3, the story-verb operation refused 3/3), `--di-cap` PASS (item-30 WARNs), `--di-bench` **zero false affirmations in every refuse bucket** (56/57; the one miss was a hesitant fragmented family, accept clause tightened and re-benched — see item 39). Four things need a finger and an ear. **(a) The placement.** In Developer Tools → Direct Instruction → "Set Up the Story", start a `build_family` run: tap (or drag) one of the three amount chips to the arrowhead. Does the 1.8 s stillness commit fire before a child changes their mind, or long after they have stopped? Does "the biggest number I see" (Jen's 12 in "Tom has 8 more than Jen") get placed by a G2 child, and does the correction ("the big number is the whole amount, the one that has it all") land? **(b) The family, spoken.** After the affirmation the tutor says only "Say the family." — does a child produce "twelve plus eight equals box" from the how-to-play alone, or need the model once? Say it in pieces with pauses ("twelve… and then eight… is… box") — does it arrive as ONE turn under the 500 ms close, or split? **(c) The scribe.** Does the chip visibly lock at the arrowhead on the affirmation, do the family slots fill only after the family is said, and does the bar model read as the story? Tablet width. **(d) `find_big_number` at G1.** Place + "how many?" with no family step — is that enough scaffold for a G1 child to work it? [Birth](eval-reports/di-word-problem-setup-birth.md) · [Curriculum fit](curriculum-fit/di-word-problem-setup-2026-09-10.md). · OPEN

### #148 — **`ramp-lab`: controlled investigations and spoken evidence.** In Developer Tools → Engineering → Ramp Lab, select `plan_fair_test`, then `explain_from_trials`, using a Grade 3/elementary profile. Change two settings and repair the plan; confirm controls lock, prediction precedes trials, and the notebook remains readable on tablet. On a real microphone, try a correct qualitative comparison, correct numbers, reversed comparison, fabricated measurement, single observation, slogan, and off-task speech. Include a mid-sentence pause; check the hands-on tutor → judged speech handoff and next-challenge reset. Two failed corrections must finish without success credit. Automated browser flow and semantic checks are recorded in the [build report](eval-reports/ramp-lab-2026-09-10.md). Real microphone, child comprehension, tablet touch, and by-ear pacing remain unverified. · OPEN

### #147 ? **`bar-model`: K spoken graph explanations and related-survey comparison.** In Math Primitives Tester, drive `say_what_it_shows` and `compare_two_graphs` on a real microphone. Try a natural correct paraphrase, reversed comparison, bare number, and off-task response. Confirm no premature totals, uninterrupted count-along feedback, wrong-answer correction cap without credit, and a fresh next item. Inspect matching morning/afternoon rows at tablet width; drive one blend for tutor handoff. Automated evidence: 26 tests, 24 fresh new-mode challenges, 4 routing cases, 36 existing K regression challenges; zero new TypeScript errors against baseline. Audio meaning and browser layout remain unverified. [Report](eval-reports/bar-model-explanations-2026-09-09.md). ? OPEN

### #146 — **`story-ribbon`: do three-card planning, open retelling, tense judging, privacy-safe connection prompts, and procedural sound hold together on a real tablet and microphone?** Drive all five modes at least once. Swap cards at phone and tablet widths; confirm the canonical order is not revealed before a verdict, Hear Again does not leak event sentences, and the finished story remains visible through feedback. In each tense mode, try one fluent correct and one fluent wrong-tense account. In `story_to_experience`, decline a personal example and offer an imagined one; the tutor must accept the boundary and judge the explained connection. Listen for selection/cancel/swap/verdict/replay levels with mute and volume controls. Automated evidence: 27/27 post-fix cue alignments, pinned/mixed/blended/tier runs, 1,000 structural constructions, 30/30 focused tests, tutor probes, and typecheck pass. [Birth](eval-reports/story-ribbon-birth.md) · [L1](eval-reports/story-ribbon-evalmodes-2026-09-08.md) · [L5](eval-reports/story-ribbon-sound-2026-09-08.md). · OPEN

### #145 — **`story-bridge`: can a five-year-old hold two read-alouds in mind, tap the intended far-shore character, and understand the evidence reveal?** On a tablet, verify shuffled character placement, hit targets, anchor-side alternation, Hear Again, bridge animation, and that the two supporting story excerpts appear only after the verdict. Use the microphone for one correct and one plausible wrong match; check that tutor speech neither names nor describes the answer before the tap. Headless DI drive and five generation runs passed. [Birth](eval-reports/story-bridge-birth.md) · [Eval](eval-reports/story-bridge-2026-09-07.md). · OPEN

### #144 — **`you-and-me`: do speaker/actor role switches survive real child speech without turning pronoun practice into imitation?** Exercise `describe_action` and `describe_independent_action` at easy and hard. Say a fluent wrong-perspective sentence, a bare pronoun, an echoed named scene, and a valid paraphrase. Verify the current speaker/actor cues withdraw by tier, correction does not overlap the next microphone turn, and `myself`/`yourself` stays bound to the actor. Automated mode, tier, tutor, and sound checks passed; real browser/microphone acceptance is still missing. [Birth](eval-reports/you-and-me-birth.md). · OPEN

### #143 — **`letter-workshop`: are all 52 manuscript references, touch/stylus tolerances, spoken cues, and provisional feedback usable by a child?** Review every uppercase/lowercase form and stroke convention; then try trace, copy, and write with finger and stylus at easy/medium/hard. Confirm independent writing hides the form, audio names the correct case, retries preserve earlier attempts, and near-miss feedback is helpful rather than falsely precise. Copy/write must remain excluded from adaptive mastery until scoring is calibrated and backend evidence replay is verified. Automated mode/tier/browser-flow tests and live variety draws passed. [Birth](eval-reports/letter-workshop-birth.md) · [Modes](eval-reports/letter-workshop-modes-2026-09-07.md). · OPEN
...
### #142 — **ten-frame teen numbers (`build_teen`, `decompose_teen`) and number-bond `ten_and_ones`: does a twenty-cell board work under a five-year-old's finger, and does the scattered group read as one group?** Three things need eyes and one needs a finger. (a) **The double frame at K.** Every other K mode is a single 2×5 frame; these two render twenty 56px cells side by side. Does the board still fit a tablet without the counters shrinking below a comfortable tap target, and does the top frame read as *one full ten* rather than as ten separate things? (b) **The scatter.** `decompose_teen` seeds 11-19 counters at pseudo-random cells across both frames on purpose — seeded top-frame-first, "flip the full row" would solve the item without counting. Does a scattered group read as ONE group a child can count through, or does it read as noise? At **nineteen** one frame is unavoidably full (geometry: 19 over two frames of ten) and the item degrades to spotting it — worth seeing whether that lands as easy or as confusing. (c) **The teen ask, spoken.** The tutor says "The top frame is full. That is ten. Your turn — make fourteen." Does a K child act on it, and does the stillness commit fire at a sensible moment while they are still counting? Headless: 60/60 pack cases, 35/35 stage cases, 550/550 math suite, six live generator draws clean (18/18, 18/18, 16/16 content checks). No live tutor session and no mic were driven. `qa/primitive-contracts/ten-frame-check-2026-09-08.md`; contract R10. · OPEN
...
**2026-09-12 follow-up:** the bounded opportunity pilot now has a reviewed Grade 4 scope, `NBT004-01-b`, for four-digit compare. Real generation and Firestore transactions passed with synthetic observations. Use that Grade 4 scope for the eventual authenticated sitting; Grade 3 remains incompatible. Local signing, deployed client read/write denials and real authenticated capture through signed Next issuance now pass. Submission retry safety and the mounted microphone/canonical-submission journey remain open. This row and #63 remain **OPEN**; no live microphone or full learner journey is claimed. [Current report](misconception/place-value-opportunities-2026-09-12.md).

**Dedicated tester ready:** Lumina > Developer Tools > Misconception Loop pins Grade 4 / compare / medium and the reviewed curriculum IDs. Use a designated test account: completion updates real learning records. Inspect capture outcome, saved phase evidence, and generation adaptation. The user's 67% activity reached both review stores but exposed discarded diagnosis evidence; its machine regression is now fixed. A saved place-name/value hypothesis can drive content adaptation but does not receive digit-worth resolution certification. Automated LLM/store/generation harness passes; complete joined live teaching acceptance is still OPEN. See [current handoff](HANDOFF-learning-observations-2026-09-12.md). #113/#63 remain human-owned.
...
### bar-model picture graph choices and capture - 2026-09-13 - OPEN

In a Grade 3 lesson with `bar-model` picture_graph at medium: (1) every choice row shows four numbers including the row's icon count, and nothing on screen marks which is right; (2) pick the icon count on two graphs, then the right total, and confirm My Progress shows a new bar-model observation after the session; (3) start another picture_graph lesson on the same objective and confirm one graph shows 5 icons (total 25) and another 1 icon (total 5). Note whether the numbered axis under the icons lets you read totals without the key (queued eval-fix). Teaching effectiveness is not claimed. [Report](misconception/bar-model-picture-graph-2026-09-13.md)

### #162 lesson launch carries the signed observation packet - 2026-09-14 - OPEN

Sign in as a learner who holds an active server-delivered hypothesis (e.g. run the number-tracer sequence at K and write the successor of the last shown number on two runs, then wait for the capture to save). Launch a lesson on the same subskill from the curriculum browser or daily plan. Confirm in DevTools that the `/api/student-profile/generation-context` response carries `learningObservations` and that the `/api/lumina/build-stream` request body forwards it; the generated number-tracer block should carry `learningAdaptation.source === 'saved-observation'` and the neighbouring runs should show the same four numbers with the gap moved. Then launch with the backend stopped: the lesson still generates, unadapted. Report: qa/misconception/launch-packet-delivery-2026-09-14.md.
