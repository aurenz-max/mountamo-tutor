...
### 20. K TAG-SOURCE AUDITS — the two verdicts written to give `affordances.reader` a source (2026-09-05; reports `qa/reader-fit/{curator-brief,equation-builder}-PRE-2026-09-05.md`)
Both were short read-only audits run by `/add-affordances`, not fix loops: curator-brief was picked in 16/16 Bench lessons and equation-builder 5x at K, and neither had a reader verdict anywhere in the tree. Tags are applied (`curator-brief: none`, `equation-builder: developing`). What is left:
- **20a. equation-builder — make it PRE-fit, do NOT floor it (WRONG-BAND @ PRE).** The build target is stated ONLY in an English sentence (`instruction`, rendered as prose at `EquationBuilder.tsx:1133`; two of five K challenges spell the numbers as WORDS), the true/false answer surface is the words **True**/**False** behind a second **Check** tap (`:945` `:957` `:965`), and `missing-result` / `missing-operand` / `balance-both-sides` commit through a numeric `<LuminaInput>` (`:1002`). There is no `isPreReaderGrade` gate, no `LuminaReadAloud`, and no read-aloud `aiDirective` — the scaffold ladder's own level 1 is *"Read the equation out loud."* Fix direction (age-friendly, never a band floor — the catalog still claims K-2 on purpose): a PRE-READER READ-ALOUD directive that says `instruction` verbatim; a PRE render turning the pills into glyph buttons that commit on tap; a K generator rule that states the target as a numeral; route the typed modes above K. Executor: `/reader-fit equation-builder --fix`.
- **20b. equation-builder — phantom-object instructions at K.** Every `true-false` instruction opens on objects that are not on screen: *"Look at the apples!"*, *"Check the stars!"*, *"Count the blocks!"*, *"Inspect the balloons!"* The stage is symbolic tiles only. Generator fix; not a reading-load issue. Executor: `/eval-fix equation-builder`.
- **20c. curator-brief — PRE chrome trim (rules 4 + 7).** READY on the reading axis (it measures nothing; every section auto-voices on change and the `lg` read-aloud button is there), but at PRE the child's field carries the header meta row (subject / grade / "Prepared for" / estimated time), the six-tab strip, a *"Use left/right arrow keys"* hint, and a mindset + "Pro tip" footer that is the only student-facing prose `[READ_SECTION]` never voices. Hide them below Grade 1 (arrows only) or voice the footer. Cosmetic at a block with no success condition, hence queued. Executor: `/reader-fit curator-brief --fix`.

### 19. K MATH PILOT LEFTOVERS — from the Lesson Bench item-15 reader verdicts (2026-09-05; reports `qa/reader-fit/{number-tracer,number-sequencer,hundreds-chart,fast-fact}-PRE-2026-09-05.md`)
All four now carry `reader` tags (`none`; hundreds-chart `identify_pattern` → `developing`). Two gaps were fixed in the same slice (hundreds-chart ORIENT beat; fast-fact pre-reader picture options). What is left is not reading load:
- **19a. number-sequencer — typed numerals at K (rule 6).** `count_from` / `before_after` / `fill_missing` / `decade_fill` answer through `<input type="number">` in the train cars (`NumberSequencer.tsx` ~:636, :786, :852); a five-year-old must summon a keyboard and type a digit. `order_cards` is already tap. Fix direction: at `gradeBand === 'K'` replace each blank input with a tap-a-numeral picker bounded by the challenge's `rangeMin..rangeMax` (the local window contract R4 already gives the bounds) — the ten-frame item-12 shape. Contract-first (`docs/contracts/number-sequencer.md` R4/R7). Executor: `/reader-fit number-sequencer --fix`. Tag already says `answers: type` so the curator sees it.
- **19b. fast-fact — start gate + chrome at K (rule 7)** and a browser glance at the new large picture buttons. "Ready to practice? / Start / Take your time" is a start-gate paragraph; streak, accuracy %, "1 / 10", subject/grade badges sit in the child's field. `gradeBand` reaches the component as a free string ('K-2'), so the gate needs `isPreReaderGrade`-style detection or a lesson-grade prop (flashcard-deck 9d did auto-start). Executor: `/reader-fit fast-fact --fix`.
- **19d. length-lab — the `order` picker chip is a 4px swatch plus a name.** `OrderingWorkspace` renders every object as `colour dot + object name`, while the length bars sit above it. The colour does link chip to bar, which is why the K floor came down (`k-band-floor-2026-09-08.md`), but a proportional mini-bar in the chip would make it readable on its own. `item.length` is already in scope. Executor: `/reader-fit length-lab --fix` (15 min).
- **19e. time-sequencer — K gets no perception anchor when no support tier is pinned.** The sun-position strip lives on the easy tier, so a session that pins no `difficulty` shows bare picture cards. Unchanged from before the 09-08 fix (the clock anchor was equally tier-only), but at the pre-reader band it may be the wrong default: consider `showSkyCue = true` at K whenever the tier is absent. Needs a ruling, not a patch. Report: `time-sequencer-PRE-2026-09-08.md`.
- **19f. time-sequencer — eight unresolvable contextKeys.** `events`, `correctOrder`, `event`, `correctPeriod`, `referenceEvent`, `relation`, `schedule`, `targetTime` are declared in the catalog and absent from `aiPrimitiveData`, so their RUNTIME STATE lines are dropped (tutor-test HIGH ×8, pre-existing). The tutor cannot name a card or describe the sky cue it now depends on. Executor: `/tutor-test time-sequencer --fix`.
- **19c. number-tracer — `[NEXT_ITEM]` names the answer in sequence mode.** The moment sends `digit: N` where N is the missing number, with "Introduce briefly" — a spoken-leak invitation. Drop `digit` from the moment text for `sequence` (it stays in the bag as tutor reference). Chrome (grade badge, counter, `%` badge) → systemic K-stage row. Executor: `/reader-fit number-tracer --fix` (10 min).
