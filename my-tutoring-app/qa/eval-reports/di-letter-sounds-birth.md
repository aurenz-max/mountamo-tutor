...
| ~~4~~ | ~~`/add-structural-difficulty`~~ | **L4 DONE 2026-08-03** | Shipped the ruled item-set composition axis exactly: easy = unique continuants with no complete confusable pair; medium = continuants + ≥1 short vowel, still no complete pair; hard = `m/n` + `f/v` together when the 3–6 item capacity permits. `first_sound_in_word` preserves its continuant-only identity, so medium honestly saturates at easy there while hard remains a real step. One resolver drives prompt + deterministic post-rotation enforcement; count/menu/mode never change. 17/17 new tests, 2,048-set stress, 10 fail on revert, live sweep 7/7. Report: `di-letter-sounds-structural-difficulty-2026-08-03.md`. |
