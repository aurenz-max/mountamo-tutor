...
| ✓ | `/add-structural-difficulty` | L4 shaped | **DONE 2026-08-04** — operand selection now follows within-five → crossing-five → crossing-ten from one `resolveProblemShape`; prompt describes and code enforces. Objective/eval-mode pools remain caps: the current within-ten K/G1 catalog honestly saturates hard at crossing-five; crossing-ten appears only when an explicit pool already reaches twenty. Variance runs inside the structural window before fallback (10/17 non-vacuity), answers rebuild from final operands, live `/eval-test` 10/10. Missing-addend remains a separate future identity, never a tier. Report: `qa/eval-reports/di-math-facts-structural-difficulty-2026-08-04.md`. |
