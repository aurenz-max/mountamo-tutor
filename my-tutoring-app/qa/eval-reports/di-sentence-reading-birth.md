...
| 4 | `/add-structural-difficulty` | L4 shaped | **NEXT RUNG — unblocked, L3 landed.** The axis is **already built and measured**: sentence LENGTH (3-8 words), carried per challenge as `wordCount` and per session as `meanSentenceWords`. Levers: word count band, sight-word density (`sightHeavy`), and vowel-mixing (single-vowel vs mixed-vowel sentences — the purity distinction the generator now enforces). **Hard constraint: the 8-word benched ceiling is not a difficulty knob** — raising it requires a new bench sitting, not an L4 decision. |
