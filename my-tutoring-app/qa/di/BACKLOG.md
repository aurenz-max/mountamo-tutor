> **Architecture direction, 2026-09-19:** The user directed a sunset of the highly scripted tutoring approach. [LA-14 retirement handoff](../live-runtime-handoffs/07-sunset-scripted-tutoring.md) owns the migration. Exact-script, sentinel and forced-correction requirements below describe legacy maintenance; they are not requirements for new tutor/JEV workspaces. Preserve domain constraints, response validity, accessibility and unresolved data/experience findings. Keep unmigrated consumers working until their replacement passes the handoff gates. No backlog item or human check is closed by this notice.

> 2026-09-19 item 44: **THE THIRD BRANCH — a judged turn may now be a QUESTION, not just right or wrong** (USER REVIEW of a live counting-board session; voice half DONE, uncommitted). The session: a five-year-old said "can you help me" and received the wrong-answer correction line, then received the same line again, verbatim. Root cause is structural, not prompt drift — the two-branch law makes every reply the affirmation or the correction, so a child who ASKS is scored as having answered wrong. `_QUESTIONS_BLOCK` in `lumina_tutor.py` ("outranks every scripted beat") is unreachable on this path: the judged loop never opens a student turn to the tutor. An OFF-TASK branch existed in exactly 2 of 17 packs (`rhymeStudioScript`, `pictureVocabularyScript`). SHIPPED in the shared loop: `SentinelPair.help` + `HELP_OPENER` + `withHelpBranch()` (OPT-IN per pack — `DI_SENTINELS` is unchanged, so no shipped pack starts classifying a stray "Good question."); `scanForSentinel` -> `'helped'`; `LoopJudgment` gains `'helped'` with `isLearnerVerdict()` to keep it out of evidence; the reducer resolves the attempt, emits `misses: 0` and CLEARS the miss streak (answering a question is the tutor on script, not drift); the runner adds a `helped` branch that touches no `correctness`, no correction count and no `observation`, re-arms the stimulus and sends NO cue (the clause re-asks in-band, exactly as a correction does). Shared clause `helpBranch(reAsk)` in `judgedScriptContract.ts` — one quoted opener, a hard ONE-sentence cap, mandatory return to the ask, because an uncapped third reply channel is what the 2026-08-15 cap drill proved stalls a run. `findSentinelCollisions` now covers the help opener. PILOT: counting-board's 7 voice kinds. The 10 packs that consume `useJudgedSpeechLoop` directly declare the branch INERT (one guarded line each) — the compiler found all 10, which is why the union was widened rather than cast. Gates: `typecheck:lumina` 0, full tsc 770 = baseline, full vitest 6902 passed / 0 failed (10 new tests). **NOT VERIFIED AT RUNTIME — needs a mic sitting, HUMAN-CHECKS #168.** QUEUED (a) the GESTURE half, which is the user's actual repro: `give_me_n` and `subitize_perceptual` set `listenForVoice = false` (`useJudgedScriptRunner.ts` `listenForVoice`), so the voice bracket is HELD for the whole item and the child's words never reach Gemini at all — a help turn there needs the bracket handed over WITH its instruction attached, and `emitClose` currently couples `sendActivityEnd()` to `onTurnClose` (`useLiveVoiceTurns.ts`). The open question is Live API semantics: does a text cue close a turn while a manual activity is open, or must `activityEnd` fire first and race the cue? Settle it on a drive, not by reading. Executor `/add-di-loop`. The doctrine this must not regress is in the `listenForVoice` docblock: letter-spotter 2026-08-13 fabricated a `[LSP_TAP]` message and read it aloud when handed a turn it had no branch for. (b) sweep `withHelpBranch` + `helpBranch` across the other 16 packs AFTER the counting-board drive — pilot-then-sweep, the clause and the sentinel ship together or not at all. (c) `moveOnCue`/`giveContract`/`perceptualContract` carry no help branch yet by design (gesture items, blocked on (a)).
> 2026-09-15 item 43: SHIPPED — `gemini-3.8-live` is the DEFAULT on the user's ruling (empty-script cue demoted to a watch item; rollback = one constant / `LUMINA_LIVE_MODEL`). Earlier the same day: A/B evaluated, not yet switched. Wins continuity (resumes where cut 2/3, never mentions the drop 0/3 vs 2/3) and tool honesty (calls 3/3 vs fabricates 2/3); blocker = empty-script celebrate cue draws no speech 4/6 (3.1 0/10). SILENT function-response stream = silent context channel on both models (design ruling owed). Report `qa/tutor-reports/live-model-ab-gemini-3.8-live-2026-09-15.md`.
> 2026-09-22 item 18: QUEUED from LA-14 S5 ([report](../tutor-reports/di-drill-deletion-2026-09-22.md)). The four spoken DI packs now run only on the teaching workspace (`DiTeachingStage` + `useTeachingEvaluation`), which submits no `observation`/`learningResponses` and no Tier-A packet. di-math-facts is a declared misconception source whose drill shipped a `diDiagnosisEvidence` packet and silent `meanResponseMs`; both are gone (lessons and Pulse had already lost them when the packs bound the workspace 09-19..22). Wire committed workspace attempts (learner transcript + the tutor's completed feedback per item) into the shared evidence path once, in the stage, for all four packs. Executor `/add-misconception-loop`.
> 2026-09-15 item 18: alias migration **Phase D (science/history/other) DONE** (uncommitted, on top of uncommitted A–C). The last 11 packs on `observation`; no pack uses the alias any more. 15 verdict-worded fallbacks reworded. Gesture observations now record the move: knowledge-check reads a new `tappedIdRef` (the tap was render state), habitat-diorama a new `committedRef` (nothing held the committed edge or zone), cause-effect-chain records card text instead of ids. Challenges state the stimulus (knowledge-check screen description, periodic-table and push-pull asks, choices, clues, starting state, both ramp trials). push-pull-arena and calendar-explorer submitted no student work before; knowledge-check submits per-problem responses; ramp-lab carries them through the investigation result. `typecheck:lumina` 0; Phase D suites 54 files / 748 tests; habitat-diorama stage test covers both gestures and the submit; full vitest 519 passed / 3 skipped; no census (no declared sources). [Report](../misconception/observation-alias-phase-d-other-2026-09-15.md). NEXT: Phase E — delete the alias from the contract, the runner's legacy branch, the census fallback, the runner tests and the skill doc; full census rerun `RUN=alias-final`. Executor `/add-misconception-loop`.
> 2026-09-15 item 18: alias migration **Phase C (DI) DONE** (uncommitted, on top of uncommitted A and B). The 5 DI packs on `observation`, every host submits `learningResponses` (di-spoken-practice submitted no student work at all before); 5 verdict-worded fallbacks reworded; di-word-problem-setup now lists the number cards with their values and names every placed card, not just the big-amount slot. `typecheck:lumina` 0; DI suites 27 files / 290 tests; new stage-test case for the placement observation and submit; full vitest 519 passed / 3 skipped; census `alias-di` vs run2: all 4 hypotheses hold, di-dice-roll not driveable as before (one cold-import timeout on the first evidence run; rerun 5/5). [Report](../misconception/observation-alias-phase-c-di-2026-09-15.md). NEXT: ship A+B+C, then Phase D (science/history/other, 11 packs), executor `/add-misconception-loop`.
> 2026-09-15 item 18: alias migration **Phase B (literacy) DONE** (uncommitted, on top of uncommitted Phase A). All 18 literacy packs on `observation`, every host submits `learningResponses`; 29 verdict-worded fallbacks reworded; challenges now carry the letters, picture menus, sentence and labels, story, excerpts, passage (clipped), page parts, scene labels, pictured events and compared story lines. Behaviour fix: letter-spotter logged confusion pairs inside the observation, which under `observation` would fire on affirmed attempts; pairs now come only from corrections. `typecheck:lumina` 0; Phase B suites 61 files / 1,257 tests; `LetterSpotter.capture.test.tsx` extended (student work + confusion-pair case, both mutation-checked); full vitest 519 passed / 3 skipped; census `alias-literacy` vs run2: all 7 hypotheses hold (phoneme-explorer more specific), sentence-analyzer abstains as before, read-aloud-studio not driveable as before. [Report](../misconception/observation-alias-phase-b-literacy-2026-09-15.md). NEXT: ship A+B, then Phase C (DI, 5 packs), executor `/add-misconception-loop`.
> 2026-09-15 item 18: alias migration **Phase A (math) DONE** (uncommitted). All 11 math packs on the one `observation` callback; challenges state the stimulus (objects and ask, pictures shown, graph rows, bond and counters, story, drawn shape with size/colour/rotation, scale state); no verdict words remain. **Gap found:** the runner's `summary.learningResponses` never reached `studentWork` outside four packs, so right answers were still not submitted; every Phase A host now submits them (bar-model and spatial-scene through the per-challenge result), plus this morning's four capture-repair packs; recipe step 5 added to the handoff. `typecheck:lumina` 0; Phase A suites 46 files / 703 tests; new `math/SortingStation.capture.test.tsx` (fails without the submit line); full vitest 519 passed / 3 skipped; census `alias-math` vs run2: sorting-station hypothesis unchanged, compare-objects abstains as before, fraction-circles not voice-driveable as before. [Report](../misconception/observation-alias-phase-a-math-2026-09-15.md). NEXT: ship Phase A, then Phase B (literacy, 18 packs), executor `/add-misconception-loop`.
> 2026-09-15 item 18: QUEUED — migrate the 45 remaining judged packs from the deprecated `diagnosisObservation` alias to the one `observation` callback, in five phases by family (A math 11, B literacy 18, C direct instruction 5, D science/history/other 11, E delete the alias); 18 of the 45 are declared sources, 41 `observed` strings in 33 packs name a verdict and are reworded first, every `challenge` states the stimulus; census per phase against run2/run4. Executor `/add-misconception-loop`. [Handoff](../HANDOFF-observation-alias-migration-2026-09-15.md).
> 2026-09-15 item 18: **capture repairs for the four census abstains DONE** (uncommitted). ordinal-line states the line order, decodable-reader quotes the story sentence and the cards, 3d-shape-explorer names the solid with its face and surface counts, picture-vocabulary names the picture; all four moved to the one `observation` callback with factual fallbacks. Census run4 with real generation and the real distiller: ordinal-line now yields "counts ordinal positions from the back of the line" (was abstain); the other three still abstain, now on the fixture (decodable-reader's harness decoy is the passage's longest word by design, 3d-shape's `alternateShape` mixes face names and nearby solids, picture-vocabulary's "a thing" is a deliberate placeholder) rather than on missing facts. `typecheck:lumina` 0; the packs' 8 suites / 203 tests pass. [Report](../misconception/judged-capture-repairs-2026-09-15.md). QUEUED: (a) `/add-misconception-loop` alias migration — 67 files still on `diagnosisObservation`, 25 verdict-worded `observed` strings to reword first; (b) optional census-only single-rule fixtures for the three fixture abstains; (c) drive adapters for di-dice-roll and read-aloud-studio.
> 2026-09-15 item 18: judged-evidence handoff **slices 2–4 DONE** (uncommitted, on top of the uncommitted slice 1). (2) one pack `observation(item, { heard, verdict })` callback: the runner keeps every evidence-carrying attempt as `learningResponses` and corrections as diagnosis observations; `responseObservation` deleted, `diagnosisObservation` a deprecated corrections-only alias; counting-board, ten-frame, base-ten, place-value migrated; census run3 over them 4/4 gate, 4/4 hypotheses. (3) `evaluation/evalModeKey.ts` at the evaluation boundary: manifest pin → catalog mode → unique challenge type → kept with one dev warning; written back into `metrics.evalMode`; table test over every catalog challenge type (13 primitives carry a type shared across modes, listed in the report). (4) `service/generation/adaptationStep.ts` (`plannedMode`, `adaptationTaskFor`, `planAdaptation`, `stampAdaptation`, `LearningAdaptation<M>`) ports all ten consumers; status rule = always stamp, `no-focus` → `insufficient-capacity`; fences 31 files / 182 tests unchanged; ten-frame signed-packet replay 2/2 targeted, controls unadapted, 0 backend calls. `typecheck:lumina` 0, full vitest 513/513 files, tsc 0 under lumina. [Report](../misconception/judged-evidence-slices-2-4-2026-09-15.md). QUEUED: (a) `/add-misconception-loop` migrate the 71 alias packs to `observation` after auditing 25 verdict-worded `observed` strings, then delete the alias; (b) component-level catalog mode reports for the rule-4 primitives (knowledge-check, fact-file, number-line plot_point, …); (c) slice-1 capture gaps and drive-adapter gaps unchanged.
> 2026-09-14 item 18: judged-evidence handoff **slice 1 DONE** (uncommitted). The runner assembles every judged primitive's evidence (`hooks/judgedRunEvidence.ts`: `firstResponseScore`, every item's first wrong attempt under the 12-phase cap, the pack's `evidenceSummary(items)`); counting-board/ten-frame builders and the base-ten patch deleted; six more runner callers submit it (balance-scale ×2, read-aloud-studio, calendar-explorer, di-spoken-practice, push-pull-arena). Census (`scripts/misconception-harness/judged-evidence-census.mjs`: real generation → mounted pack → real distiller on :3000, 23 sources): gate fires 20/20 where it never fired before; 14 hypotheses name the fixture's signature miss; 2 abstains correct (sentence-analyzer, compare-objects); 3 not driveable. `typecheck:lumina` 0, tsc 770 for this slice's files, full vitest green outside a concurrent session's untracked pip surface tests. [Report](../misconception/judged-evidence-census-2026-09-14.md). QUEUED: (a) `/add-misconception-loop` capture repair — phases omit the stimulus the error is about: ordinal-line (line order), decodable-reader (story sentence), 3d-shape-explorer (solid shown), picture-vocabulary (picture named); (b) `/tutor-test` drive adapters + `HarnessAnswers` for di-dice-roll and read-aloud-studio; (c) cvc-speller / phonics-blender / sound-swap self-gate on `accuracy < 60` (`useJudgedSpeechLoop`, not the runner); (d) slices 2–4 of the handoff, executors unchanged.
> 2026-09-14 item 18: QUEUED from the ten-frame slice — [handoff](../HANDOFF-judged-evidence-and-adaptation-wiring-2026-09-14.md). (1) `/add-misconception-loop`: the judged runner's `summary.diagnosisEvidence` has no `firstResponseScore` and keeps the LAST 12 phases, so capture rarely fires for the 20 declared judged sources that submit it; move first-response scoring and phase selection into the runner, delete the counting-board/ten-frame/base-ten workarounds, run a distiller census from each pack's `HarnessAnswers`. (2) `/add-misconception-loop`: one `observation` callback in place of `diagnosisObservation`/`responseObservation`. (3) `/eval-fix`: normalize the submitted eval mode to a catalog key in `usePrimitiveEvaluation` (CNB-3, TF-6 class). (4) `/add-misconception-loop`: shared generator adaptation step; the planned mode currently comes from three different sources across ten consumers.
> 2026-09-14 item 18: `ten-frame` misconception loop implemented for `operate` (capture repair across all eight types + one consumer, uncommitted). Voice items were described as "subtract on a frame of 10", right answers were not kept, three wrong first answers in five still submitted 80, and `metrics.evalMode` sent `add`/`split` instead of the catalog `operate`/`decompose` (TF-6, fixed). Now per-item facts (split with its code verdict), `learningResponses`, `firstResponseScore`. Move `contrast_same_first_number_different_second` (7−1 then 7−3; 8+3 then 8+5), K–2, same-operation neighbours only. Reading compiled output found subtraction doubles (8−4) giving the judge a contradictory contract ("the correct answer is four" / "four taken away is NOT the answer"): judge fixed in `discriminationFor`; whether to keep doubles is TF-5. Distiller 8/8, planner 16/16 + 4/4 positive, generation 33/33, exploratory 2/2 abstain, replay with a Python-signed packet 2/2 adapted / four controls unadapted / 0 backend calls. Chance 0% in real K subtraction and Grade 1 baselines, 4 of 6 in K addition within five. [Report](../misconception/ten-frame-2026-09-14.md), HUMAN-CHECKS **#164**. QUEUED: (a) `/misconception-test ten-frame`; (b) TF-5 `/eval-fix` (doubles hide the number-taken-away error; pedagogy call); (c) no legal move for make_ten (partner counts already adjacent 3/3), subitize (ascending sweep 4/4), build, split, teen modes.
> 2026-09-14 item 18: `counting-board` misconception loop implemented for `take_away`/`add_more` and `count_on` (capture repair + two consumers, uncommitted). Capture described every counted board as "count N" (a 7-take-away-3 board read "count 7, expected four"), kept no right answers, and three wrong first answers in five still submitted 80; now per-mode board facts, `learningResponses`, `firstResponseScore`. Moves `contrast_same_start_different_change` (7−1 then 7−3; 4+1 then 4+3) and `count_on_exactly_one_more` (five in the basket and one bear). add_more's baseline change now avoids a repeated neighbouring start (contrast was present by chance in 12/20 seeds); take_away's step-off no longer draws a change of 4. Distiller 8/8, planner 20/20 + 4/4 positive, generation 44/44 + take_away rerun 23/23, exploratory 4/4 abstain, replay with a Python-signed packet 2/2 adapted / four controls unadapted / 0 backend calls. Chance 0% in real baselines; simulated take_away 26-58% when the model repeats sizes. [Report](../misconception/counting-board-2026-09-14.md), HUMAN-CHECKS **#163**. QUEUED: (a) `/misconception-test counting-board`; (b) ~~`/eval-fix` CNB-2 compare larger group always first (blocks a compare move), CNB-3 evalMode `count_all`/`group_count` vs catalog `count`/`group`~~ DONE 09-14 ([report](../eval-reports/counting-board-compare-side-evalmode-2026-09-14.md)); a compare move is now buildable (`/add-misconception-loop`); CNB-4/CNB-5 filed in EVAL_TRACKER; (c) no legal move for count, recount_moved, give_me_n, subitize, group.
> 2026-09-14 item 18: observation delivery moved to a SIGNED LAUNCH PACKET (USER DIRECTION: frontend pipeline, backend = storage + ownership + scope resolution; uncommitted). `/generation-context` now returns `learningObservations: {payload, signature}` (owner's stamped hypotheses + each objective's live published scope); it rides `useExhibitSession` -> build-stream -> `withGenerationRequest`, verified once; `learningObservationPacket.ts` owns the format, per-task join, ordering and evidence bounds. Generation makes NO backend call; the two signed read routes and six per-primitive authenticated probes are deleted; receipt issuance is the only signed write left. Verified: backend 78/78 across the observation set, `learningObservationPacket.test.ts` + eight ported delivery suites, `typecheck:lumina` 0, number-tracer replay with a Python-signed packet 2/2 adapted / 0 backend calls / four controls unadapted, generic `misconception_authenticated_smoke.py` PASS with cleanup (against a fresh :8001; the shared :8000 `uvicorn --reload` did not reload). [Report](../misconception/launch-packet-delivery-2026-09-14.md), HUMAN-CHECKS **#162**. QUEUED: (a) USER RULING whether retest receipt issuance should also leave the generation path (signed token checked at submission; consumer `submission_service.py`); (b) harness runner + fixtures (handoff steps 1-2) unchanged.
> 2026-09-14 item 18: `number-tracer[sequence]` misconception loop implemented (capture repair + consumer, uncommitted). The grader could not support capture: vision read a transparent PNG as blank (NT-5), scaled geometry accepted a different numeral (NT-6), easy/medium painted the answer's guide (NT-7), neighbouring answers repeated (NT-8), and the window call ran away in 7/26 draws (NT-13) — all fixed via `/eval-fix` and driven in the real app ([eval report](../eval-reports/number-tracer-2026-09-14.md)). Capture: every check recorded, judge `writtenAs`, evidence only when a drawing reads as another number, `firstResponseScore`, metrics `evalMode` (NT-14). Move `contrast_gap_positions_in_one_run` (same run, gap moved) at K/Grade 1 sequence. Distiller 5/5, planner 12/12 + 2/2 positive, registry generation 24/24 + exploratory 3/3 abstain, authenticated `nt-http-qa-e9c1f11e…` PASS with cleanup; chance 0-0.1% and capacity miss ≤0.4% in wide windows (narrow windows: contrast already present 19-75%). [Report](../misconception/number-tracer-2026-09-14.md), HUMAN-CHECKS **#161**. QUEUED: (a) `/misconception-test number-tracer`; (b) NT-9 `/tutor-test` (sequence answer reaches the tutor), NT-10 USER RULING on write's instruction showing the numeral, NT-11 `/topic-fidelity` K ceiling 9, NT-12 `/add-support-tiers` sequence; (c) `/add-misconception-loop` handwriting-family move, after NT-10. Harness handoff REVISED: a no-login delivery-replay tier (`scripts/misconception-harness/replay-delivery.mjs` + `backend/scripts/project_learning_observation.py`, piloted on number-tracer) replaces per-primitive authenticated probes; one generic authenticated smoke remains for shared-code changes.
> 2026-09-13 item 18: QUEUED — shared misconception probe harness, executor `/add-misconception-loop`. Each consumer copies ~200-line Node and ~200-line Python probes that differ only in fixtures; six of seven Node probes still use the eval-test `remediationFocus` tap, which declared consumers strip, so reruns would report positives unadapted. Plan: one Node runner (registry generator path, fixed outcome classes, chance and capacity rates) + one Python signed-in runner + a fixture file per primitive; port area-model and number-line first, then delete the copies. [Handoff](../HANDOFF-misconception-probe-harness-2026-09-13.md). `/add-misconception-loop` SKILL.md rewritten the same day around this slice's findings (registry probe path, per-mode move descriptions, score-visibility check, 12-phase evidence limit, outcome classes and rates).
> 2026-09-13 item 18: `area-model` misconception loop implemented for `build_model`/`find_area`/`multiply` and `perimeter` (capture repair + consumer, uncommitted). Area-model recorded no responses, and a model with one wrong cell of four still scored 100, so capture could never fire; it now keeps a factual response ledger and opts into `firstResponseScore`. Catalog source + consumer declaration (no service edit). Moves: `contrast_same_fact_across_places` (one model whose largest and smallest cells use the same fact at different places, 23 × 32: 600 and 6) and `contrast_equal_area_perimeters` (two consecutive rectangles with equal area, different perimeters). Final real run: distiller 4/4, planner 10/10, registry generation 30/31 (miss = Gemini 503); authenticated `am-http-qa-ccd668ba…` PASS with cleanup. Two repairs came out of failed runs (kept): evidence exceeded the store's 12 phases, and the grid move was described only for two-digit factors, so the planner abstained 4/4 in build_model. [Report](../misconception/area-model-2026-09-13.md), HUMAN-CHECKS **#160**. QUEUED: (a) `/eval-fix` AM-3 score 100 with a wrong cell, AM-4 factor-mode answers not unique, AM-5 build_model 1 × 1 models (EVAL_TRACKER); (b) `/misconception-test area-model` station inventory not run; (c) `/add-misconception-loop` factor mode records evidence but has no move, blocked on AM-4; (d) `/topic-fidelity area-model` Grade 3 `NBT003-05-c` and Grade 4 `NBT004-06-a/b` ask for one-digit factors that build_model (11-25) and multiply (3-digit × 2-digit) do not draw; (e) harness: the eval-test `?remediationFocus=` tap cannot reach declared consumers, and `probe-bar-model-applicability.mjs` still relies on it.
> 2026-09-13 item 18: observation delivery is catalog-declared on the frontend (uncommitted). The six `componentId ===` branches in `geminiService.ts` are one generic branch reading `ComponentDefinition.learningObservations` (`{ eligible(config), retest? }`); `learningObservationServer.ts` takes the scope's subject from `config.objectiveSubject` (stamped by the manifest flatten from the lesson's curriculum subject) and names no primitive or subject; the place-value retest is a catalog `retest` declaration (`placeValueRetest`, a pure compiler) served by the same module, so `placeValueOpportunityServer.ts` is gone and the signed request scope lives in `generationRequest.ts`. A client-supplied `remediationFocus` no longer reaches a server-delivered consumer. Verified: `typecheck:lumina` 0, vitest 95/95 across the generation + remediation + manifest sets (new: catalog dispatch through the real fraction-bar generator; retest suite ported), authenticated probes PASS with cleanup for fraction-bar (shared delivery), place-value (receipt issued) and number-line. The four non-place-value probes had been stale since the backend ruling (captures posted without `delivery: 'server'`, so no `scope_context`) and now relay it. [Report](../misconception/catalog-observation-consumers-2026-09-13.md). QUEUED: (a) USER RULING still owed on a retest consumer also taking shared-scope observations (blocks-origin → chart); (b) bar-model and fraction-circles probes patched, not rerun.
> 2026-09-13 item 18: `number-line[jump]` misconception loop implemented (capture repair + consumer, uncommitted). Three repairs came first: jump grading accepted a landing one hop short (tolerance = snap step), so the targeted error was graded correct; sessions always scored 100, so capture never fired (shared opt-in `firstResponseScore` gate, `isDiagnosableFailure`, used by four consumers); and LLM hints named in-between landings or the landing (NL-3, fixed in code). Move `contrast_start_positions` pairs a jump anchored at 0 with the same hop from another start; shared delivery pinned to Grade 1 `OPS001-03-a`/`OPS001-04-a` at medium. Real distiller, planner 6/6, generation 8/8 + 15/15 + 18/18, authenticated `nl-obs-qa-dbddafe9…` PASS with cleanup. [Report](../misconception/number-line-jump-2026-09-13.md), HUMAN-CHECKS **#159**. QUEUED: (a) `/eval-fix` NL-2 `plot_point` accepts one tick off; (b) `/misconception-test number-line` station inventory not run; (c) `/add-misconception-loop` plot-mode sparse-label variant is a second move, not built.
> 2026-09-13 item 18: `fraction-circles[compare]` misconception loop implemented (capture repair + consumer, uncommitted). Compare only advanced after a correct answer, so it always scored 100 and never captured; it now records every response and opts into `firstResponseScore`. `misconceptionScope: 'skill'`, `SERVER_DELIVERED_SOURCES` member, shared delivery gated to five reviewed same-numerator-compatible subskills; `contrast_same_numerator_denominators` rewrites one fraction in at most two unstructured pairs within grade-legal denominators. Real distiller, planner 27/27, registry generations, authenticated run PASS with cleanup (two earlier unadapted authenticated runs kept: missing signing key, and one unexplained). [Report](../misconception/fraction-circles-2026-09-13.md). QUEUED: (a) `/topic-fidelity` Grade 3 compare draws use fifths, tenths, twelfths and wholes (3-5 pool wider than the Grade 3 standard); (b) `/misconception-test` failed touch_fraction sessions now reach the distiller, unprobed; (c) HUMAN-CHECKS #158.
> 2026-09-13 item 18: `bar-model[picture_graph]` misconception loop implemented (capture repair + consumer). Bare icon count is always a choice; first-choice selection evidence with firstResponseScore; `contrast_icon_count_and_row_value` via the shared planner and shared delivery at Grade 3 MEAS003-04-a. 14/14 real planner draws and an authenticated distill→store→delivery→generation run pass, cleanup verified. [Report](../misconception/bar-model-picture-graph-2026-09-13.md). QUEUED: (a) `/eval-fix` picture graphs print a numbered axis under icon rows, so totals are readable without the key; (b) `/add-number-pool-service` or `/eval-fix` unadapted picture_graph targets converge (all four = 20; one draw asked about the same row four times); (c) `/eval-fix` key emoji can differ from the targeted category; (d) `/add-misconception-loop` other option modes record selections but emit no evidence.
> 2026-09-13 item 18: fraction-bar consumer SHIPPED (uncommitted), retargeted identify → build (identify is unit fractions; no role contrast is possible). Shared source-agnostic delivery is now live: `learningObservationServer.ts` → signed `/learning-observation-context` (SERVER_DELIVERED_SOURCES producers, write-time scope). Also fixed: signed routes returned 403 whenever uvicorn lacked the exported key. Real 19/19 applicability draws; authenticated run PASS with cleanup. [Report](../misconception/fraction-bar-2026-09-13.md). Open: 6 of 10 adapted draws were already-targeted (low content yield, needs a dosage decision); `/misconception-test fraction-bar` not run; teaching effectiveness unverified.
> 2026-09-13 item 18: `base-ten-blocks[read_blocks]` is now an observation SOURCE (was consumer only). Catalog `misconceptionScope: 'skill'`; factual correction evidence kept whatever the average; backend `SERVER_DELIVERED_SOURCES` policy (canonical scope stamped, prose kept off the client manifest, no score/tag resolution); blocks consumes the shared `/learning-observation-context` delivery (Grade 4 read_blocks medium, no longer pinned to one subskill); `gradeBand` code-owned after live draws labelled Grade 4 as `2-3` and silently dropped the move. Real distiller 5/5, planner 12/12 (run 1 failures kept), authenticated `pvc-http-qa-cee522b2…` PASS with cleanup. [Report](../misconception/base-ten-blocks-origin-2026-09-13.md), mic **#157**. First-response gate adopted the same day: bare count on every worth step, right after one correction (67%, first-response 50%) now diagnoses; real distiller 4/4. QUEUED, executor `/add-misconception-loop`: (b) USER RULING needed on chart generation receiving blocks-origin observations; (c) `regroup` has no evidence or capability.
> 2026-09-12 item 18: shared LLM applicability replaces generation regex for Place Value and Base Ten. Fourteen real cases and authenticated two-primitive verification pass. Retest certification remains separate. [Report](../misconception/shared-learning-applicability-2026-09-12.md). Ordinary Blocks generation repeated mats in two unadapted probes; that existing variety limitation remains open.
...
### 43. 🎛️ **SHIPPED 2026-09-15 — LIVE MODEL A/B: `gemini-3.8-live` IS THE DEFAULT (user ruling same day: "I wouldn't let a states-of-matter blocker prevent this… we move forward with this model"). Rollback = one constant in `lumina_tutor.py` or `LUMINA_LIVE_MODEL=gemini-3.1-flash-live-preview`.** Report: [live-model-ab-gemini-3.8-live-2026-09-15.md](../tutor-reports/live-model-ab-gemini-3.8-live-2026-09-15.md). Lever kept: `LUMINA_LIVE_MODEL` process-env override in `lumina_tutor.py` (default unchanged).

**Measured (3 runs each unless noted).** Continuity: both PASS the one-drop `lesson-resume-continuity` gate 3/3; 3.8 resumes a cut count where it stopped 2/3 (3.1 restarts from "one" 3/3) and never mentions the disconnection (3.1 tells the child "my connection dropped" 2/3, which the steering text forbids). Async tools: 3.8 calls a NON_BLOCKING tool 3/3 and speaks the result unprompted 1.1s after a WHEN_IDLE response; 3.1 calls it 1/3 and FABRICATES the tool's answer 2/3. A `will_continue` + `SILENT` function-response stream is a working silent context channel on BOTH models (3/3 quiet, 3/3 state reaches context, WHEN_IDLE reaction 3/3). Style: 3.8 ~18 words/turn, ends-with-? 0.24–0.30 vs 3.1 ~26 / 0.50. **Blocker:** on the states-of-matter journey's empty-script cue `[ANSWER_CORRECT] Student answered "" for "". Celebrate: ""`, 3.8 drew no real speech 4/6 (2 turns never closed — U+200B transcript, no audio, no turn_complete for 2 min; 2 empty ~1s turns); 3.1 0/10. Not reproducible in a short isolated session (0/10 each), so it is the long production session.

**Queue, in order.**
- **(a) WATCH ITEM (was the switch gate; flipped on the ruling)** — executor `/tutor-test`: add an EMPTY-scripted-line beat to the states-of-matter journey (it is the harness's generic template today; production cues like ReactionLab's `Celebrate: "${narration}"` can also be empty) and require speech 6/6 on 3.8, OR add a backend guard that never forwards an empty scripted line. The `--di-bench` half is DONE: `concept_statement` on 3.8 = 0/56 false affirms, 48 agreed vs 3.1's 49 (report §4). The default is flipped; this row watches the empty-script behaviour in real lessons.
- **(b) DESIGN — silent state channel (CTX-1 successor), user ruling owed:** a NON_BLOCKING `watch_activity` tool called once at session start, state streamed as function responses (`will_continue=True`, `SILENT`), events as `WHEN_IDLE`. Works on the production model today; replaces "state attached to the next message that asks for a turn". Not built.
- **(c) Harness fixes** — executor `/tutor-test`: (1) `LUMINA_FAULT_DROP_EPISODES>1` cascades because `[SESSION RESUMED]` is cue-classified and re-arms the drop (3.1 run 1 took 9 drops, re-introduced the activity 8×); arm once per SESSION, not per cue. (2) `RESUME_REGREET_FORBID` lacks "connection" / "dropped" / "blinked" — 3.1 said them 2/3 and passed. (3) Two same-day drives of one component overwrite each other's report; a `--report-tag` is owed (worked around by copying).
- **(d) Ear check → HUMAN-CHECKS #165:** the output transcript `Twenty-oneI'm just a language model and can't help with that.` at the end of a resumed count, 2/3 on EACH model; WAVs in `qa/tutor-reports/audio/`.

### 42. 🔢 **SHIPPED 2026-09-11/12 — `number-bond` GOES HANDS+VOICE: persistent counters replace dot-hint scaffolding, and all six modes get a spoken turn.** Filed retroactively 2026-09-12 (`/ship`) — code, tests and the evidence writeup predate this entry; the queue record is the piece that was missing.

**What shipped.** A fixed set of movable counters becomes the learning object for every challenge type, not a removable hint: `decompose` and `ten-and-ones` ("Split and Say") move counters between the whole and its two parts, settle on a 1.2s hold, then the student speaks the highlighted part — the equation appears only after the spoken affirmation. `missing-part` covers the unknown region opaquely; an optional counter workspace is available on request rather than shown by tier. `related-fact` keeps stable red/blue counter groups through join → say missing addend → separate → say remainder. `build-equation` has the student choose and perform a join or separation, then construct the matching equation — a true fact describing a DIFFERENT action gets relationship-specific coaching, not a bare wrong mark. `fact-family` requires every distinct related form (4 for unequal parts, 2 for equal parts — reversing `=` no longer inflates the count). Construction and speech outcomes combine per pair using the lower score, so a correct spoken answer can never erase an unsuccessful construction. Files: `numberBondSplit.ts` (Split and Say model), `numberBondModes.ts` (the other five challenge types' model/response phases), `SplitAndSayBoard.tsx`, `NumberBond.tsx` rewritten, `numberBondScript.ts`, `gemini-number-bond.ts` rewritten per-mode, catalog `math.ts`, oracle `number-bond.ts`.

**Gates.** 87 focused tests across six suites pass (immutable counter movement, pair novelty, bounded split/teen cases, dynamic spoken targets, action recognition, arithmetic-vs-action diagnosis, equality reversal, unequal/equal family coverage, logical-outcome aggregation, mounted tap/drag/keyboard paths, reveal timing, optional-support attribution, mixed-mode continuity, DI scripts, the stateful headless drive, content oracles). Six real generated sessions / 18 source challenges / 66 runtime turns across decompose and ten-and-ones at easy/medium/hard, no contract issues. Changed Number Bond files have no TypeScript diagnostics.

**Residuals.** Live acceptance open: browser animation, touch ergonomics, and live microphone judging are not established by the mocked-runner tests. No claim of calibrated difficulty for the redesigned interaction — existing evaluation-mode identities and priors are unchanged. Full writeup: [number-bond-split-and-say/README.md](../number-bond-split-and-say/README.md).

### 41. ⚖️ **SHIPPED 2026-09-11 — `balance-scale` BECOMES A SIX-MODE TOUCH+VOICE WEIGHT WORKSHOP, replacing the click-only equation port.** Filed retroactively 2026-09-12 (`/ship`) — code, tests and the evidence writeup predate this entry; the queue record is the piece that was missing.

**What shipped.** All six existing eval modes (`equality`, `equality_hard`, `one_step`, `one_step_hard`, `two_step_intro`, `two_step`) move from click-only equation-solving to a physical weight workshop with a separate spoken interpretation per stage: match-and-add, compose-the-same-weight-two-ways, complete-a-known-load, share-among-identical-parcels, separate-then-share, and connect-those-moves-to-equations. Unknown parcels are identical opaque containers; known weights show their quantities. An exact hand solution held for 900ms auto-commits through the shared judged runner; the challenge score is the minimum of a mode's spoken-outcome stages, so a correct inference can never erase an unsuccessful addition/composition answer. Numeric targets stay code-built from the existing equation pool — the manifest still supplies no numbers. `equality`/`equality_hard` first shipped as a smaller pilot (match/add/infer only); the full six-mode workshop superseded it same day. Files: `balanceEqualityModel.ts`/`balanceEqualityScript.ts`/`BalanceScaleEquality.tsx` (superseded pilot, kept for the historical fixtures), `balanceWorkshopModel.ts`/`balanceWorkshopScript.ts`/`BalanceScaleWorkshop.tsx` (current), `BalanceScale.tsx` routing, `gemini-balance-scale.ts` rewritten per-mode, catalog `math.ts` (`audioInputByMode` on all six modes), new `primitiveAudioInput.ts` resolver this pilot motivated.

**Gates.** 97 tests across 10 focused suites pass (weight conservation/arithmetic, every generated mode's solvability, drag/tap, undo/reset, settled-match cancellation, independent spoken outcomes, generation, shared-runner audio routing). 18 real generated sessions / 54 challenges / 225 script steps across all six modes at easy/medium/hard, no contract issues. `typecheck:lumina` 0. No claim of calibrated difficulty for the redesigned interaction — existing adaptive IDs/priors are retained unchanged.

**Residuals.** Live acceptance open: browser tilt/gather animation, hand-turn timing, and spoken-turn transitions are unverified by the mocked-runner tests. Mic/eyes row **#152**. Full writeup: [balance-workshop/README.md](../balance-workshop/README.md) · superseded pilot: [balance-equality/README.md](../balance-equality/README.md).

### 40. 🔤 **SHIPPED 2026-09-12 — `syllable-clapper` GETS A LADDER: three ACTS where there were three word LENGTHS.** User ruling 2026-09-11 (*"is there a way to enhance with the voice modality?"* → blend + delete, `/add-eval-modes`).

**The defect the slice fixes.** The 2026-08-16 port (item 17) shipped `evalModes` of `easy` / `medium` / `hard`. Those are WORD LENGTHS: one act — hear a word, say how many parts — registered as three skills. It was not cosmetic. `resolveEvalModes` blends 2-3 modes when an objective spans them, so a K phonological-awareness lesson could resolve to `easy + medium`, a "blend" of two word lengths, which is not a curriculum decision; meanwhile the three acts such a lesson is actually made of had nowhere to be declared. Length moved to `config.difficulty` beside the ask scaffolds already there, and the modes became the acts: **`blend_syllables` β1.5 · `count_parts` β2.5 · `delete_compound` β3.5** (βs unchanged in magnitude from the shipped registry; the three legacy band keys are kept read-only in `problem_type_registry.py` so historical attempts still resolve). Also fixed: `metrics.evalMode` was ABSENT, so every attempt filed as `'default'` and the priors were never reached — the same defect the three older-learner packs carried.

**⭐ THE CHANT CHANGES SIDES, and it is the cleanest statement this lane has that a leak rule is about the ANSWER and never about a form of words.** `syllableClapperScript` bans a chanted ask in its strongest terms — saying "but … ter … fly" hands the count over, three beats IS three, and item 17's headline finding was the click era using the chant AS the easy-tier scaffold. On `blend_syllables` that same chant is the legitimate QUESTION: the child is not asked how many parts there are, they are asked to join them. So the banned scaffold becomes the stimulus, and the joined word becomes the thing that may never be said early — `stimulusFor` pushes the CHANT on this act, because pushing the word would hand the answer to the tutor as runtime state, outside every cue the leak oracle reads. It also sits BELOW counting on the ladder: blending is the easier act, and the pack had no rung under its own floor.

**⭐ THE LIVE PROBE RENAMED A MODE AND BOUGHT A POSITIVE ORACLE — both findings no amount of reasoning produced.** The act was built as `delete_syllable`. Its first draw returned (a) `peanut → pe|anut`, removePart "pe", residue "anut": two parts, they spell the word, the residue is one of them and it is sayable letters — **every structural gate passed**, and the ask it would have produced is "say peanut without pe", whose answer is a nonword; and (b) `dragonfly → dragon|fly`, `honeybee → honey|bee` — good compound-deletion items and bad syllable splits, since "dragon" is two beats. Both were fixed by believing the content: the mode is `delete_compound`, its parts are WORDS not syllables, and both halves are gated against a **positive** oracle (`COMPOUND_PART_WORDS`, ~140 words). A positive list is the wrong tool almost everywhere — word-workout's real-word oracle is used negatively for exactly that reason — and the right tool here, because the K-appropriate compound supply is small and closed. True syllable deletion stays unbuilt on purpose: "banana without ba" is "nana", and a judged loop cannot score a nonword honestly.

**Files.** NEW `syllableClapperModes.ts` (the first LITERACY adopter of `diModeContract` — one declaration feeds the catalog's `evalModes`, the generator's prompt/schema docs, the judged response classes and the step sequence); `syllableClapperScript.ts` rewritten per-act (asks, affirms, corrections, judging contracts, near-miss clauses, harness answers, gates); `SyllableClapper.tsx` (act badge, act prompt line, per-act reveal, per-act metrics + `evalMode`); `gemini-syllable-clapper.ts` (task enum, `removePart`/`residue`, `resolveEvalModes` replacing the legacy `resolveEvalModeConstraint`, band-as-tier); catalog `literacy.ts`; `diDrivePlan.ts` docblock; backend priors.

**Gates.** `typecheck:lumina` **0 errors in every touched file** (the gate's 4 reds are a concurrent session's untracked `baseTenScript.ts` / `BaseTenBlocksDi.tsx`) · full `tsc` 0 in touched files · **77 tests across 3 suites** (54 di-script, 23 stage, 8 tier) · full vitest **5803 passed** (the 4 fails are the same concurrent lane) · **3 revert-bites** (the blend stimulus channel → 2 fail; the deletion shape gate → 1 fail, and it needed a case the PART WINDOW does not already catch; the positive oracle → 1 fail) · **live probe 7 draws / 3 acts / 4 topics, 0 pack-gate issues** — deletion re-probed after the fix at 23/24 kept, every residue a real word.

**Drives — 6 sessions, 0 HIGH.** `blend_syllables` plain **PASS 8/8 refused + 8/8 affirmed**; `blend_syllables` signature **PASS 8/8** — ⭐ this is the act's sharp test and its ask MANUFACTURES it: the chant hands the child the answer's own pieces, so the fluent wrong answer is those pieces said straight back, and nothing but whether they were JOINED separates it from the right answer. `delete_compound` plain **PASS 8/8**; `delete_compound` signature **PASS 8/8** — the whole word echoed ("starfish" for "say starfish without star"), which CONTAINS the residue as a substring, refused every time. `count_parts` regression **PASS 8/8** on a fresh topic. `--di-cap` (blend): **0 HIGH, 2 WARN = the known-open 18c pair** (the verbatim correction this contract deliberately commands, and the capped item that asks then withdraws); **no `di-no-verdict`**.

**Residuals, none blocking.** (a) **`short_spoken_word` on 2-4 syllable words is new answer material for this class** — it is benched on short CVC words, and blending's answers are "kangaroo" and "watermelon"; `--di` sends text, so whether ASR gives a K child's blended multisyllable word back intact is unmeasured. Mic/eyes row owed. (b) The tutor dropped the "…" separators in 2 of 8 blend CORRECTIONS ("la dy bug"), a delivery drift on the earned model, not a verdict failure — orthography is the strong lever and it held 6/8. (c) The drive report name is keyed by wrong-kind + date, so the blend signature report was overwritten by the deletion one; its verdict was read before that happened. Item 19's residual (a), still a one-line harness fix. (d) `delete_compound`'s tier ladder is word FAMILIARITY, not length — a compound has two words in it at every tier. (e) **`/curriculum-fit` owed on all three modes** — the catalog claim changed shape and the old homes were written for a counting-only pack. (f) `isolate_part` ("say just the first part of rabbit") remains unbuilt: its answers are frequently nonwords, so it needs a new response class and a `--di-bench` sitting first — queued behind that sitting.

### 39. ➡️🗣️ **SHIPPED 2026-09-10 — `di-word-problem-setup`, THE THIRD "DI FOR OLDER LEARNERS" PACK AND THE FAMILY'S FIRST HANDS+VOICE MATH PACK: the child PLACES the big number at the arrowhead, then SAYS the number family.** Routed from item 37 residual (e) via [HANDOFF-di-word-problem-setup-2026-09-07.md](../HANDOFF-di-word-problem-setup-2026-09-07.md); user ruling 2026-09-10 (*"build di-word-problem-setup next with the big_number step as the hybrid pilot"*). Mic/eyes row **#149**. Birth report: [di-word-problem-setup-birth.md](../eval-reports/di-word-problem-setup-birth.md).

**What shipped.** Connecting Math Concepts number families for one-unknown addition/subtraction stories. Steps: `classify` (spoken, `closed_set_choice`) → **`big_number` (HANDS — one of three amount chips placed at the arrowhead, stillness close 1.8 s, verdict computed in code by `bigNumberVerdictCue`, class `manipulation`)** → `family` (spoken, NEW class **`equation_statement`**, accepted-build-ahead) → `operation` (`closed_set_choice`) → `solve` (number word). Three eval modes = three step lists: `find_big_number` β2.5 (place + work) · `build_family` β3.5 · `classify_and_build` β4.5; backend mirror in. Nine code-built story frames (`diWordProblemPlan.ts`: comparison ×3, change ×4, part-whole ×2), a seeded number pool, and the family/misplaced/subtraction/incomplete forms all from code; Gemini emits only THEMES (two names, a plural noun, a gain/loss verb pair) plus an answer-free wrapper. The stage SCRIBES: the chip locks at the arrowhead on affirm, the family slots fill when the family is said, the sign and working line when the operation is said, the box when it is solved; a move-on draws the step carried in amber. Files: `diWordProblemPlan.ts`, `diWordProblemScript.ts`, `DiWordProblemSetup.tsx`, `gemini-di-word-problem-setup.ts`, `service/qa/di/wordProblemBench.ts`, `DI_PORTS` adapter (with `gestureVerdictCue`), catalog `di.ts`, tester tab, `RESPONSE_CLASSES.equation_statement`, three new `OpenSetBucket`s (`big-number-misplaced`, `operation-not-family`, `family-incomplete`), three test suites.

**Design calls.** (1) **The hybrid is at the PACK level, not inside one item.** The runner already had gesture items and voice items; this pack SEQUENCES them inside one story — hands for POSITION (which amount is the whole), voice for the FAMILY — the 2026-08-13 ruling exactly. A within-item "do, then say" (mic opens after the stillness close) was NOT built: it touches the shared runner and the harness for no gain over the sequenced form. (2) **A session runs ONE mode** — the step list is the how-to-play. (3) **Every quantity ≥ 2** and **the answer word is refused if any token of it appears in the story** (hyphens split) — the second is exactly the harness leak scan, so the gate and the scan agree. (4) Change-story chips are "what Sam started with / has now / gave away", never "at the start / now".

**Machine evidence (2026-09-10).** `typecheck:lumina` 0 · plan 18/18, script 26/26, stage 10/10 (placement arms stillness, the commit hands the runner the `[WPS_BIG]` cue, remove clears, affirmation locks/hides/draws, move-on carries dim), contract + drive-plan 193/193, direct-instruction folder 359/359 · affordances tagged · live generation, one draw per mode: 0 drops, `packGateIssues: []`, every story read by eye (two content fixes from the eye-read: answers of ONE, change-story labels). **Headless drives** (`qa/tutor-reports/di-word-problem-setup-live-di-*.md`): `--di` plain **PASS 12/12 both directions, hands-hold SILENT 3/3** · `--di-wrong signature` **PASS 12/12** — the upside-down family drew the `misplaced` branch 3/3, the story-verb operation refused 3/3, the wrong-way number refused 3/3, "the biggest number I see" placement corrected 3/3 · `--di-cap` PASS-with-WARNs — the move-on carried the family verbatim ("Good try. Three plus box equals thirteen. Do you add or subtract?"); the 2 WARNs are item 30's family shape · `--di-bench` (7 stimuli, 57 probes): **ZERO false affirmations in every refuse bucket** (`big-number-misplaced` 6/6, `operation-not-family` 4/4, `family-incomplete` 6/6, `echo` 2/2, `wrong-verdict` 4/4, `wrong-number` 2/2, `off-task` 14/14), `valid-canonical` 7/7, `valid-paraphrase` 11/11, **1 missed valid** — the hesitant fragmented family "um, twelve… and then eight… is, box" drew the incomplete branch. Fix shipped same slice: the accept clause now states a family said in pieces with "um" / "and then" / "is" between the parts is whole, with the item's own fragmented form as the example; **re-bench on `wpsb-cmp-big-family` 12/12 agreed, the fragmented family affirmed, zero false affirms, 0 missed valid** (first attempt died with ws 1012 — a concurrent session's backend save; the relaunch is the record).

**Curriculum fit (`/curriculum-fit di-word-problem-setup`, [report](../curriculum-fit/di-word-problem-setup-2026-09-10.md)).** G1 `find_big_number` **MATCH** (`OPS001-07-b` larger-value comparison 0.818 5/5; `OPS001-06-b` unknown addend) — an honest home the pack over-serves. G2 `build_family`: the honest homes exist (`OPS002-01-c` *"by selecting the correct operation … translating scenarios into equations"*, `OPS002-02-c` *"distinguish take-away and compare prompts"* — the only G1–4 subskills whose stated act IS setup) but rank 3–4 behind G2 two-step problems. G3 `classify_and_build`: **curriculum gap** — no one-step add/sub word-problem subskill is published at G3 (nearest: three-digit solve, two-step mult/div). G4: wrong-fit (multiplicative comparison). **Two attribution defects found and FIXED this slice:** (a) no `_PRIMITIVE_TO_SUBJECT` override — a live G3 session scoped to LANGUAGE_ARTS (Character Analysis 0.708); `di-worked-procedure` and `di-word-problem-setup` → MATHEMATICS with a test (`test_curriculum_retrieval_subject_scope.py` 2/2; `di-deduction` left on the family default on purpose); (b) `metrics.evalMode` absent on all three older-learner packs → `eval_mode: 'default'`, β priors never reached; `evalMode: data.challengeType` added to all three. Also the three eval-mode descriptions now say "one-step addition or subtraction word problem" (the report's reworded-description diagnostic lifted G2 `find_big_number` to a 0.806 match).

**Residuals, in order.** (a) **The `equation_statement` sitting** — bench-clean on its first sitting; a second on a fresh draw or a user ruling promotes it to `benched`. (b) **Mic/eyes #149** — the 1.8 s stillness under a real finger; the fragmented family under the 500 ms voice close (raise `silenceCloseMs` pack-scoped if it splits — the di-sentence-reading precedent). (c) **G3/G4 home** — author a G3 one-step setup subskill (`/curriculum-author`, CCSS 3.OA.8 one-step-in-two-step) or trim the catalog claim to G1–G2 — a user ruling. (d) `medium` vs `hard` tiers identical (only `easy` re-reads the story) — `/add-support-tiers`. (e) Two-step stories, three-quantity part-whole, multiplication families — `/add-structural-difficulty`. (f) A distractor fourth chip (an amount that belongs to no slot) — `/add-eval-modes`. (g) Within-item "do, then say" — only if a later pack needs the SAME item to carry both; the runner addition is scoped in the birth report. (h) `di-correction-verbatim-repeat` = item 30, a doctrine call, unchanged.

### 38. 🔎 **SHIPPED 2026-09-07 — `di-deduction`, THE SECOND "DI FOR OLDER LEARNERS" PACK: a RULE card and a CASE card, and the child SAYS what follows and how they know, judged aloud on meaning.** Routed from item 37 residual (e) via [HANDOFF-di-deduction-2026-09-07.md](../HANDOFF-di-deduction-2026-09-07.md); the brief ranked it second because `deduction` is the one class that spans science, social studies and ELA inference (G3–5). Mic/eyes row **#141**.

**What shipped.** Item 37 judges a MOVE; this one judges a DEDUCTION — a VERDICT plus the REASON that reaches it from a rule, judged on meaning like `concept_statement` ("it's got eight legs so it's not one" is a full `deny` answer). NEW class **`deduction`** (accepted-build-ahead on the item-37 precedent). Three case shapes = three eval modes, all code-built from ONE generated rule (`diDeductionPlan.ts`): `conclude` β2.5 ("A beetle is an insect." → "a beetle has six legs"), `deny` β3.5 ("A spider does not have six legs." → "no, not an insect, because…"), `cannot_tell` β4.5 ("This animal has six legs." → "can't tell — other things have six legs too"); mixed = a rule worked through all three, the Corrective Reading format. Backend mirror in. The stage: rule card, case card, the conclusion written under them on affirm (carried in amber on a move-on), verdict pills lighting on affirm only. Files: `diDeductionPlan.ts`, `diDeductionScript.ts`, `DiDeduction.tsx`, `gemini-di-deduction.ts`, `service/qa/di/deductionBench.ts` (the class key: 67 probes over 6 cases), `DI_PORTS` adapter, catalog `di.ts`, tester tab, `RESPONSE_CLASSES.deduction`, three new `OpenSetBucket`s (`affirmed-consequent`, `wrong-verdict`, `verdict-no-reason`). ~1,250 production lines under ~360 of comments and ~370 of tests.

**Two design calls that differ from the handoff, both recorded in the class notes.** (1) **The `cannot_tell` subject is ANONYMOUS** ("This animal lays eggs."), not a named lookalike: against a child, "A turtle lays eggs. Is a turtle a bird?" makes "no, it's a reptile" a TRUE answer the contract would have to refuse; the named lookalike moved to the FIRM-UP ("a turtle lays eggs too, and a turtle is not a bird"), which is the DI form ("An animal lays eggs. Is it a bird?" → "Maybe."). One cannot_tell case per rule by construction; a rule with no lookalike (insects / six legs) builds none — refused, not vague. (2) **Verdict-with-no-reason is REFUSED** (two-branch law, ONE way) via its own "My turn: how do you know? …" branch ahead of the contrast; "no, because it's a spider" routes there too; the accept clause names the short forms ("nope, no six legs").

**Generator = Fork B with code rails + a TRUTH REVIEW.** flash-lite emits up to 4 rules as {category, plural, property ×3 forms, kindNoun, members, nonMembers, lookalikes}; code prints the rule ("All A P." — a "some/most" rule cannot arrive), supplies articles, gates structure (sayable ≤3-word entities, none in two lists, none in the rule text, a negation that negates). A separate flash-latest review gates TRUTH per rule. **Mid-slice fix:** the first review dropped whole rules on pedantry ("robins have scales on their feet"; a pillow as a feather lookalike) → yield 0/4 in two modes; it now returns `ruleTrue` + `badEntities[]` and TRIMS, re-sanitizing — yield after ≈ 2 rules / 4–6 cases per draw (birds+amphibians, insects+birds, fish+squares seen live).

**Machine evidence (all 2026-09-07).** `typecheck:lumina` 0 · 35/35 (`diDeductionPlan.test.ts`, `diDeductionScript.test.ts`: `checkPackGates` [] on the mixed session AND a six-deny single-shape session, easy tier, `checkDiCatalogEntry` [], leak scan over every ask with the rule sentence + how-to-play exempt, span order pinned per shape 4/5/6) · affordance ledger 204/204 · live generation via `/api/lumina/tutor-test?di=1`: 0 drops, `packGateIssues: []`, every cue read by eye. **Headless drives** (`qa/tutor-reports/di-deduction-live-di-*.md`): `--di` plain **PASS 6/6 both directions** · `--di-wrong signature` **PASS 4/4** (mixed; the echo and the bare "no" refused) and **2/2 on `cannot_tell`** — "yes, because it lives in water" drew the "rule does not work backwards" branch verbatim · `--di-cap` PASS-with-WARNs: the move-on carried the conclusion verbatim ("Good try. An ant is an insect, so an ant has six legs. Same rule. …"); the 3 WARNs are item 30's family shape · **`--di-bench` 67/67 agreed, ZERO false affirmations in every refuse bucket, 0 missed valid, 0 no-verdict** (`affirmed-consequent` 5/5 · `verdict-no-reason` 8/8 · `wrong-verdict` 11/11 · `echo` 6/6 · `adjacent-concept` 3/3 · `off-task` 12/12 · paraphrase 12/12). The report's HIGH was a HARNESS gap: `build_di_bench_journey` hard-coded `leak_exempt_span: ""` on its ask/re-anchor beats while the drive journey wires the port's span, so every rule-reading `conclude` ask scored as a leak the plain drive had already cleared — fixed in `run_tutor_live.py` (20/20 harness tests), re-bench `ddb-insects-c0-conclude` 11/11 with no HIGH. Two `di-verdict-embellished` WARNs are the contrast slot carrying a five-word child quote; one re-anchor of ~70 drew a correction instead of the re-ask (family noise).

**Curriculum fit (`/curriculum-fit di-deduction`, retrieval probe G3–5).** Science G3/G4: `match` (τ 0.6, best 0.72/0.71) but the neighbors are inherited-traits / life-cycle / classify-materials sorting; G5 science unpublished (probe widened to K–4). Literacy G3/G4: `abstain` (diffuse); G5 `match` on LA002-05 "Reasons & Evidence" (0.71). Social studies: no retrieval domain. **No published G3–5 subskill has rule-based deduction as its solve surface** — the pack maps via the generation path today; the nearest honest homes are G5 RI "reasons & evidence" and G3 science classification as APPLY blocks. Executor for a real home: `/curriculum-author` (a "reason from a stated rule" subskill in G3 science / G4 RI).

**Residuals, in order.** (a) **The `deduction` sitting** — the fixture ships and is bench-clean; a second sitting on a fresh draw (the concept-class precedent) or a user ruling promotes it to `benched`. (b) **Mic #141** — the deny sentence's mid-answer pause (raise `silenceCloseMs` if it splits) and whether "can't tell" survives being taught once. (c) **`unrelated` shape** ("A ladybug is red. Does the rule tell you…?") as a second `cannot_tell` case source — needs a fourth generated list; `/add-eval-modes`. (d) **Two-rule chains / syllogisms** = `/add-structural-difficulty` on this pack. (e) `medium` vs `hard` tiers identical (only `easy` re-reads the rule every ask) — `/add-support-tiers`. (f) **`di-error-hunt`** (concept 6) reuses `deduction` — build after (a). (g) Three of four drives died with ws **1012** because a CONCURRENT session was saving `backend/*.py` (uvicorn `--reload`), and one because the harness file itself sits under the watch — memory `feedback_uvicorn-reload-kills-live-drives`.

### 37. 🧮 **SHIPPED 2026-09-07 (same session as the brief) — `di-worked-procedure`, THE FIRST "DI FOR OLDER LEARNERS" PACK: a multi-digit subtraction the child works OUT LOUD one column at a time, each STEP judged where it happens.** User thread: *"we need some new modern di-stylized primitives that are intended for older learners… not just 'repeat the word' but rather do the work"* → design brief (artifact `8e67009d`, seven concepts) → *"these are fantastic… lets jump in"* = the build-ahead ruling. Mic/eyes row **#140**.

**What shipped.** The K-2 packs judge a WORD; this one judges a MOVE. Two step kinds: `decide` ("Start in the ones column. Tell me what you do." → the child regroups or subtracts cleanly; NEW class **`procedure_step`**, accepted-build-ahead) and `subtract` ("Now subtract the ones." → the difference; benched `number_word_to_20`). Two eval modes (`subtract_no_regroup` β2.0, `subtract_regroup` β3.5; backend mirror in). The step chain is CODE-OWNED (`diWorkedProcedurePlan.ts`): Gemini writes only the answer-free wrapper + a width hint; named problems in the objective win; grade sets the default width (≤G2 two-digit, G3+ three-digit); `hard` on three digits = two regroups. The stage SCRIBES: every affirmed step writes its marks (strike / digit above / 1 beside / difference), a move-on writes the tutor's step in dim amber so "Now subtract the ones" never refers to a thirteen the page never showed. Files: `diWorkedProcedurePlan.ts`, `diWorkedProcedureScript.ts`, `DiWorkedProcedure.tsx`, `gemini-di-worked-procedure.ts`, `service/qa/di/workedProcedureBench.ts` (the class key: 45 probes over 6 column shapes), `DI_PORTS` adapter, catalog `di.ts`, tester tab, `RESPONSE_CLASSES.procedure_step`, six new `OpenSetBucket`s.

**Four content gates, all "true by construction" so the judge is never asked what the bench cannot see:** (1) no zero difference in any column ("zero" is unbenched); (2) no borrowing across a zero (the G3 cascade rung — queued below); (3) **the flipped column never lands on the right digit** — for `bottom − top = 5` the flip equals the regrouped result (8−3 = 13−8 = 5), so 53 − 28 — the brief's own mock — is REFUSED and the brief now shows 52 − 28; (4) same width both numbers.

**Machine evidence.** `typecheck:lumina` 0 · 33/33 (`diWorkedProcedurePlan.test.ts`, `diWorkedProcedureScript.test.ts`: `checkPackGates` [] on a 3-problem session incl. the easy tier, `checkDiCatalogEntry` [], leak scan over every ask with the problem statement exempt, family span order pinned ask/affirm/specific/general) · live generation via `/api/lumina/tutor-test?di=1`: 3 problems → 9 items, 0 drops, `packGateIssues: []`, every cue read by eye (`wp-probe.json`). **Headless drives, all 2026-09-07 (`qa/tutor-reports/di-worked-procedure-live-di-*.md`):** `--di` plain **PASS 9/9 both directions**, no findings · `--di-wrong signature` **PASS 9/9** — every flipped column ("eight minus one is seven") and every forgot-to-decrement ("seven minus five is two" on a lent tens) REFUSED, every right move affirmed · `--di-cap` PASS-with-warnings: the capped regroup moved on with the carry line spoken verbatim ("Good try. We regroup: six tens, fifteen ones. Now subtract the ones."); the 2 WARNs are `di-correction-verbatim-repeat` = **item 30's family-wide shape, a third witness, not a pack edit** · `--di-bench` (the `procedure_step` key, 54 probes / 6 column shapes): **48/48 scored agreed, ZERO false affirmations in every refuse bucket, 0 missed valid** — the 6 misses were all the filler probe "umm" drawing NO VERDICT (the tutor re-asked instead of correcting; the concept-statement bench had the same intermittent miss). Fix shipped same slice: every fallback clause now names "only a filler sound like um or hmm" — re-bench on `wpb-52-28-c0-decide` **14/14 agreed, off-task 2/2**. The class is bench-clean on this evidence; it stays `accepted-build-ahead` until a second sitting on a fresh draw (the concept-class precedent) or the user rules it benched.

**Residuals, in order.** (a) **The `procedure_step` sitting** — the fixture ships; `--di-bench` scores it; zero false affirms in `flipped-column` / `forgot-decrement` / `no-move` / `unneeded-regroup` / `wrong-landing` is the bar. (b) **Voice-turn close for a sentence-length answer** — if #140(a) finds the pause after "I can't" splits the turn, raise `silenceCloseMs` pack-scoped (the di-sentence-reading precedent). (c) **The cascade rung** (borrow across zero, 300 − 148) = `/add-structural-difficulty` on this pack, with its own step ("the tens are zero, so I go to the hundreds first"). (d) **Addition with carrying** = the sibling pack (`carry` step: "three plus eight is eleven, write one, carry one") — same runner, same scribe, a second step grammar; ~the cost of this slice. (e) **The remaining six brief concepts** — next by leverage: `di-deduction` (the one cross-subject class) and `di-word-problem-setup` (reuses `procedure_step`). **Handoffs written 2026-09-07:** [HANDOFF-di-deduction-2026-09-07.md](../HANDOFF-di-deduction-2026-09-07.md) — paste-ready prompt, the four code-built case shapes, the `deduction` class and its bench buckets, the seven gates, and the carrying-sibling alternative · [HANDOFF-di-word-problem-setup-2026-09-07.md](../HANDOFF-di-word-problem-setup-2026-09-07.md) — the CMC number-family pack: five judged steps (classify / big number / family / operation / solve), code-owned story frames with a theme-only LLM window, the new `equation_statement` class and its buckets. The two are independent and can run in parallel sessions. (f) `medium` vs `hard` support tiers are identical in this pilot (only `easy` states digits) — `/add-support-tiers` owns the split. (g) `/curriculum-fit di-worked-procedure` not yet run — G2 NBT.5/NBT.7 and G3 NBT.2 are the expected homes.

### 36. 🗣️ **OPENED 2026-09-07 (`/lesson-coverage` — grade-1 math operations pass) — "EXPLAIN"-VERB MATH OBJECTIVES HAVE NO PRODUCIBLE ASSESSMENT PATH. 20/22 explain-verb objective rows in the full `evals.jsonl` land `ASSESSED_INDIRECTLY`/`INSUFFICIENTLY`; the only 2 `SUFFICIENT` are DI spoken blocks (`di-spoken-how-many`, `spoken-practice`).** Layer: SUPPLY — matches the routing doctrine directly ("a K-2 production verb needs a SPOKEN mode → `/add-di-loop`"). Not a new sighting — three prior citations never filed as their own item: (1) `qa/lesson-bench/BACKLOG.md` item 21(c) — kindergarten-addition; `comparison-panel`'s own misconception gates (`obj3-start-vs-end`) already argue the concept but aren't counted as assessment evidence (open question: digest gap vs. real supply gap). (2) item 26(a)+(b), 2026-09-05 grade-1 sweep, 10/10 packages: the curator correctly reaches for `di-spoken-practice` on explain-verb objectives, but on abstract "explain the rule/meaning" prompts (no closed named-set target) the generator returns `items: []` (2/10: `…ah5w` obj2, `…f00i` obj3) and silently falls back to MC/TF or an adjacent-skill block elsewhere (10/10 off-target: `equation-builder[true-false]`, `knowledge-check`, `number-bond[missing_part]`…). (3) item 34's shipped `compare_choice` fix explicitly EXCLUDED this shape ("open-ended verbal EXPLANATION — a different, harder problem"). Reconfirmed live 2026-09-07 on `…kbg3` obj3 and `…xr3w` obj3 — same categories still hold at current `HEAD f7d32f47`.
**Design, not a patch — corrected 2026-09-07 after reading `spokenPracticePlan.ts`/`judgedScriptContract.ts`: NOT a new primitive, a new task identity on `di-spoken-practice` (same shape as `compare_choice`, item 34), but the first genuinely open-ended one.** Every existing task (`visual_naming`/`read_aloud`/`say_answer`/`count_and_say`/`compare_choice`) grounds one item to a SINGLE `expectedAnswer` token checked against `sourceQuote`; "explain why" has no single token — the target is a proposition. New shape needed: (1) a `explain_concept` task in the `TASKS` enum; (2) item content = `stimulusPrompt` + 2-3 code-validated `acceptableConcepts` (grounded paraphrase anchors, not model-invented — same "LLM emits scope, code builds structure" split as everywhere else); (3) the runtime judge does NOT need new machinery — DI doctrine already holds "word-matching is the reporting channel, not the judge," the live tutor already makes semantic calls guided by the hand-authored script — what's new is the SCRIPT shape in `diSpokenPracticeScript.ts` (model-then-elicit branches written for "did the utterance touch concept X," not "did they say word Y"). This IS a new response class (first free-form, multi-phrasing one the family has had) — DI standing gate #1 applies: a ~30-min hand-rolled bench sitting BEFORE any wiring. Executor: `/add-eval-modes di-spoken-practice`, scoped with a design handoff doc first (same shape as `HANDOFF-di-spoken-practice-comparative-2026-09-06.md`), bench sitting as the first gate. Check `comparison-panel`'s digest mapping (item 21c) before designing — the concept-argument evidence may already exist and only need counting, which would shrink the ask.
Gate: bench sitting passes → `/lesson-coverage confirm` on `…ah5w` (obj2) and `…f00i` (obj3) — `items: []` → non-empty; category improves past `ASSESSED_INDIRECTLY`; then a fresh grade-1 sweep ×3 per `rerun`.
**Sequencing: filed behind the declared queue order — `gas-laws-simulator` / `ph-explorer` stay Next; this is evidence banked for when the queue turns to it, not a reorder** (user call, 2026-09-07).
**Handoff written 2026-09-07 (design reviewed against the code, nothing wired):** [HANDOFF-di-spoken-practice-explain-2026-09-07.md](../HANDOFF-di-spoken-practice-explain-2026-09-07.md). Three corrections to the note above, so the build does not start from them: (1) anchors are NOT code-groundable — nothing in the sources prints a paraphrase — so the split is code-validated STRUCTURE + the existing flash-latest REVIEW call for meaning (the plan+review authority split, one layer down); (2) "model-then-elicit" is already the correction branch — modeling the concept in the ASK would be the leak, so the new script surface is the concept-anchored judging clause + an affirm that restates the concept sentence; (3) **the bench is unrunnable today: `di-spoken-practice` has no adapter in `DI_PORTS`**, so the sitting starts by writing one (`build`/`benchBuild`/`answersFor`), which also closes compare_choice's "no live drive" residual (#137). 21(c) checked: `gates` is absent from the digest's `ITEM_KEYS` (real, one-line gap) but the gate is TRUE/FALSE recognition — counting it never lifts an explain verb past `ASSESSED_INDIRECTLY`; file separately. Class: `concept_statement` (blocked → benched), bench buckets incl. `valid-paraphrase` (must affirm ≥80%) and `negated-keyword` (must refuse — the bucket that fails a word-matching judge). Mode `explain_concept`, β 4.0. Census: 22 explain/describe rows in `evals.jsonl`, 2 sufficient.

**✅ SHIPPED + BENCHED 2026-09-07 (same day; branch `ship/2026-08-10-judged-loop`, uncommitted).** `concept_statement` cleared the family's THIRD machine-scored class bench — [run-2026-09-07-concept-statement.md](../di-bench/run-2026-09-07-concept-statement.md): 6 `--di-bench-item` sittings over 4 stimuli across BOTH sub-shapes, **0 false affirmations in 32 hard REFUSE probes, `valid-paraphrase` 8/8 (floor 80%)** — every anchor-free paraphrase ("they match", "it jumps by two", "it takes turns, one color then the other", "it's just the little cubes stuck together") affirmed and every negated-keyword probe ("they are not the same", "it's not plus two, it's plus one") refused; the planted echo-plus-idea case ("red, blue, red, blue, over and over") affirmed while the bare read-back refused. Misses (none blocking): "it means equal" 1/2, `um` → no verdict 3/4 (re-ask, not a verdict), one ten-rod sitting drifted off script (embellished affirms, a false-completion claim, two improvised re-teaches with no sentinel — session variance; the re-run held 14/14). Class record flipped `blocked → benched`; `validateJudgedScriptPack` now admits it. **Shipped:** `explain_concept` on all five touch points (task enum + planner prompt/review, `MODE_SHAPE`/`HOW_TO_PLAY`, item `conceptStatement`, catalog eval mode β 4.0, `problem_type_registry.py`), the concept-anchored judging clause + concept-restating affirm (§4), `findConceptDefects` (7 structural reasons) + one per-session flash-latest meaning review, **and the `DI_PORTS` adapter** (`build`/`benchBuild`/`answersFor` for all five modes — #137's "no live drive" residual closed for free; it also caught a birth defect: the contract's imperative "Then wait for the learner." tripped `findPerformedStageDirections` on every cue since 2026-08-11 — now the fact form). **Pilot** (`scripts/probe-di-spoken-practice-explain.mjs`): selection 2/2 (both frozen draws route unpinned to `explain_concept`), refusal ✓ (a procedure ships nothing), resolution **3/6 → 6/6** over four runs, each step a gate that was refusing good items — over-ask `count+2` with pooled survivors, one re-read of an `unsupported` plan under a pin, `normalizeConceptAnchors` (one redundant planned "same as" had zeroed 6/6 items of a stamped session), spoken-number tokens to 120, and `reconcileConceptAnchors` (the ask names the thing explained, so a planned anchor "equal" leaked into every ask on fresh draw `…i08t`). **Confirm** (`/lesson-coverage confirm`, results in lesson-bench 26(a)/(b)): frozen `…ah5w`/`…f00i` re-judged unchanged (control); fresh full-pipeline draws — **6 of 10 explain-pinned spoken blocks shipped items, and every one of those 6 objectives landed `ASSESSED_SUFFICIENTLY`** (two lessons PASS 1.00); the 4 empties are draw variance (repro: `…xpnk` ships 2/2 on re-draw after a thin first attempt; `…57wg` 1/2). Gates: focused suites 224/224 + full vitest 4900/4901 (the 1 fail is `gemini-solar-system-explorer.eval-modes` SP-21, passes 2/2 alone — flaky, untouched), `typecheck:lumina` 0, tsc **770 = baseline**. Mic: **HUMAN-CHECKS #139**.
**Residuals (queued, not dropped):** (1) **~40% of explain-pinned blocks still ship empty on a fresh draw** — the reviewer rejecting vague or sibling-kind items (a growing pattern under a "repeating" objective, "alternating shapes" as an anchor) plus the 4-word anchor cap catching a legitimate AAB unit ("two stars and a triangle"). Levers, in order: a third attempt when the pool is non-empty but thin; anchor cap 4 → 5; the kind-of-thing prompt line (added at the end of this slice, not yet re-probed). Executor `/eval-fix di-spoken-practice`, gate = the explain probe ≥ 5/6 on THREE consecutive runs plus fresh `confirm` ×3. (2) A "create your own pattern and explain it" brief (`…dip9`) and a "balanced or not balanced" brief pinned `compare_choice` (`…b4i5`, refused — "not balanced" contains "balanced", the ear-separability drop) are different shapes, not this mode's; note for `/lesson-coverage diagnose`. (3) The 4-word cap + "≤ 4" in the class notes: if (1) raises it, update the record. (4) The digest `ITEM_KEYS` `gates` one-liner (21c) still separate.

### 35. ❌ **RETRACTED 2026-09-07 — MIS-ROUTED, not a DI-family item.** Originally opened as "`fast-fact[recognize]` needs a spoken port" off package `…l5no`'s G6 citation. **Wrong layer: SUPPLY was never the defect.** `phoneme-explorer` already ships Live-DI, answered-aloud beginning-sound identification (`answers: ['spoken']`, "ESSENTIAL for K-2 literacy") and was ALREADY the block selected for the same objective in the same package — fast-fact was a redundant SECOND, tap-only block for ground already spoken-covered. Porting fast-fact would have built a duplicate DI surface for no gain. Corrected as a SELECTION-layer catalog fix (one constraint line on `fast-fact`, no new primitive) — shipped and confirmed at `qa/lesson-bench/BACKLOG.md` **item 31**. Lesson for next routing: **before opening a DI-port item off a G6 citation, check whether another block already selected for the SAME objective already answers `spoken`** — a tap-only citation next to an already-spoken sibling block is redundancy (SELECTION), not absence (SUPPLY). Folded into `/lesson-coverage`'s SKILL.md diagnose table 2026-09-07.

### 34. ✅ **SHIPPED 2026-09-06 — `compare_choice`, the closed-set comparative mode. OPENED 2026-09-05 (`/lesson-coverage kindergarten`, compare-attributes package) — `di-spoken-practice` HAS NO SHAPE FOR A COMPARATIVE DESCRIPTIVE UTTERANCE — refuses honestly, twice, on independent draws.** Objective: "describe the size and weight of objects using words like longer, shorter, heavier, and lighter." Both `obj3-spoken-attribute-description` and `obj3-spoken-explanation` (fresh draws `…3rvk` and `…wxyu`) generated `items=[]` — `spokenPracticePlan.ts`'s task enum (`visual_naming` / `read_aloud` / `say_answer` / `count_and_say` / `unsupported`) and the generic pack's item schema (ONE `stimulusText`/`stimulusEmoji` per item) have no shape for "look at TWO objects and say which is longer/heavier" — a comparative production task needs two stimuli, and every current task assumes one. The empty-items refusal (`gemini-di-spoken-practice.ts`'s "no curated fallback... pedagogy over runnability") is working as designed, not a crash — this is a SUPPLY gap, not a bug. Same primitive already has the worst hit rate in the whole coverage set (`qa/lesson-coverage/evals.jsonl`: 9 rows, 4 fail / 4 warn, avg coverage 0.72) — this is that class, not a new one; worth checking whether other di-spoken-practice fails share the "needs 2+ stimuli" shape before scoping the fix. Executor: design a comparative `say_answer` variant (two `stimulusText`/`stimulusEmoji` slots, an ask that names both, `deriveResponseClass` extended for a comparative answer word) — `/add-eval-modes di-spoken-practice`, contract-first against any existing `docs/contracts/di-spoken-practice.md`. Gate: `/lesson-coverage confirm` on `…wxyu`, obj3 category improves past `ASSESSED_INDIRECTLY`. Routed from `qa/lesson-bench/BACKLOG.md` item 30(c). **Handoff prompt written 2026-09-06:** [HANDOFF-di-spoken-practice-comparative-2026-09-06.md](../HANDOFF-di-spoken-practice-comparative-2026-09-06.md) — scopes the fix to the CLOSED-SET comparative-word shape only (longer/shorter/heavier/lighter) and explicitly excludes the two OTHER di-spoken-practice `generation_failure` rows (`…-ah5w` obj2, `…-f00i` obj-speak-the-rule), which are open-ended verbal EXPLANATION — a different, harder problem, already flagged separately at lesson-bench item 21(c).

**SHIPPED 2026-09-06 — `compare_choice`, a fourth task identity (β 2.0, `closed_set_choice`).**
Report: [eval-reports/di-spoken-practice-compare-choice-2026-09-06.md](../eval-reports/di-spoken-practice-compare-choice-2026-09-06.md).
**The split:** CODE owns the menu, the model writes the pairs. `spokenPracticePlan.ts` extracts the
required words from the objective through the same token grounding that grounds a printed symbol
(a `compare_choice` target is a menu WORD, and `buildPlannedSpokenItems` refuses to build them —
there is no stimulus mapping to repeat); the item generator writes the object pairs around that
menu; `hasChoiceCoverage` re-checks after the gates that every named word was asked. Three of four
words ships NOTHING. Item schema gained a flat second stimulus (`stimulusText2`/`stimulusEmoji2`,
no nesting — the flash-lite-safe shape holds), the component a `pair` stimulus (two emoji, no
labels: the tutor names both aloud).
**The gate that makes it honest.** A comparative ask must read the menu, so it contains the answer
by construction and `findAnswerLeaks` had to exempt it. That exemption is safe ONLY paired with the
new `findChoiceMenuDefects`, which requires EVERY choice in EVERY ask — a complete menu tells the
child nothing; two-of-four is a two-way guess in a four-way costume, and it is what the model
reaches for unprompted (the wxyu intent proposed "is it longer or heavier?" verbatim). Four more
refusals ride it: <2 choices, an answer off the menu, choices the ear cannot separate, and the one
**found live on the first real generation** — flash-lite wrote **"a long pencil"** against a menu
holding "longer", and every whole-token gate passed it (`containsPhrase` is deliberately substring-
blind, which is what stops "cat" hitting "catalog"). Closed with a 4-character shared-stem rule
(catches long/longer, heavy/heavier, light/lighter, short/shorter; spares leaf/lighter). Zero stem
giveaways in the six sessions generated after the fix. `findUnspokenStimulus` grew the pair case
(the ask must NAME both things — two unnamed pictures are not a comparison for a pre-reader) and
stopped requiring ARTICLES, a latent false-drop on every mode ("the rock" vs a stimulus "a rock").
**Gates:** tsc **770 = baseline** (0 new), `typecheck:lumina` **0**, vitest **570/570** across 44
files (16 new cases), live probe `scripts/probe-di-spoken-practice-compare.mjs` **9/9** — SELECTION
(both frozen manifests, stripped of pins, route obj3 to `compare_choice` through the real
`resolveLessonEvalModes`), RESOLUTION (6 independent draws, all full-menu/leak-free), and REFUSAL
(the out-of-scope explain objective still ships nothing).
**Gate met, and it drew the scope line itself.** `/lesson-coverage` on both packages with a fresh
slot spliced in: **3rvk obj3 ("**Describe** … using words like longer, shorter, heavier, and
lighter") `ASSESSED_INSUFFICIENTLY` → `ASSESSED_SUFFICIENTLY`**, lesson coverage **0.67 → 0.83**,
1/3 → 2/3 sufficient, judge note "Spoken production tasks directly elicit all four target
descriptive words". **wxyu obj3 ("**Explain** … in your own words") stays `ASSESSED_INDIRECTLY`** —
its `generation_failure` is GONE (evidence 6→8, all four items cited) but the judge now names the
real reason: "closed-set selection… rather than an open explanation". That is the handoff's own
out-of-scope line, drawn by the judge rather than asserted.
**Residuals:** (a) **mic — HUMAN-CHECKS #137**, no live drive; the menu clause has never been heard
performed. (b) The EXPLAIN class is untouched by design (`…-ah5w` obj2, `…-f00i` speak-the-rule,
wxyu obj3) — stays with lesson-bench item 21(c). (c) NOT swept: per pilot-then-sweep, no other
`evals.jsonl` row was converted; "does any other lesson share this closed-set comparative shape" is
the next pull, not this slice.

### 33. 🎙️ **OPENED 2026-09-06 (`/lesson-coverage kindergarten rhyming`) — `poetry-lab[rhyme_hunt]` IS THE ONLY TAP-ONLY PRODUCTION BLOCK LEFT IN A K RHYMING LESSON.** The bench's G6 gate (K-2 literacy production answers out loud) cites it in 2/2 fresh packages (`kindergarten-rhyming-20260906033528-p5m6`, `kindergarten-rhyming-fun-with-words-20260906034157-t872`) and it sat in the user's own build-stream lesson `kindergarten-language-arts-rhyme-recognition-production-20260906033544-o2cg`. Layer: SUPPLY. The round is a proposition menu — four pictured line-ending words, "tap the two that rhyme" — so under the spoken-first ruling it is a `closed_set_choice`: the tutor reads the poem, the child SAYS the two words, the judge accepts either order, the tap stays as the no-mic fallback. `rhyme-studio` already carries the spoken recognition / identification / production trio in the same lesson, so the port is a contract check against `docs/contracts/poetry-lab.md` (rhyme_hunt = PRE, reader none, the tutor must not pair the words aloud) plus the runner. Executor: `/add-di-loop poetry-lab` (rhyme_hunt only; analysis/composition stay tap/type). Gate: `/lesson-coverage confirm` on `…p5m6` → G6 ✓, plus a `--di` headless drive. Media-player's `listen_and_look` is the other G6 citation in `…p5m6` and is owned by `qa/media-player-reimagining/BACKLOG.md`, not here.
...
### 23. 🧩 **OPENED 2026-08-18 (user directive) — KNOWLEDGE-CHECK RECALIBRATION: microstep item model + the composite judged-loop port.** Executor: slice 2 = `/add-di-loop knowledge-check`; slice 3 = catalog + floor edit (no skill). **Slice 3 re-scoped 2026-09-05:** the floor edit is now P2 of the KC redesign handoff (`qa/HANDOFF-knowledge-check-redesign-2026-09-05.md`) — three new stimulus insets + production kinds `say_it`/`point_to`/`how_many`, R2 forked, commit = affirmation; lesson-bench BACKLOG item 25 owns the design. **Slice 3 ⚙️ SHIPPED 2026-09-05 (machine gates + headless generator/oracle/script probe; browser + live drive owed):** `knowledgeCheckScript.ts` gains `say_it` (voice; class narrows per card — number word / `letter_name` / `shape_name`), `how_many` (voice, `number_word_to_20`; counting aloud = thinking, the number they stop on = the answer), `point_to` (gesture, `manipulation`; `tapVerdictCue` now judges TOKEN positions of the printed sentence). Each kind: how-to-play, ask, affirm/correction/close, judging contract with the blind-tutor stimulus description, harness answers with a named signature wrong (the START count on a take-away picture; the OTHER sign; the NEXT numeral). Production items are exempt from the answered-once rule (key = stimulus × answer; the generator's `used` set keeps stimuli apart). `KnowledgeCheck.tsx` renders the stimulus (static inset, or `NumberSentenceTokens` with the tap) — the one on-screen commit is the honest POINT. Tap surface: `ProductionProblem.tsx`. Pack gates + catalog contract green on a mixed set (`KnowledgeCheck.production.di-script.test.ts`, 15 pins). The user's `…pgr5` label ("says the answer, then has to tap") is addressed structurally for production items — a spoken kind never renders a menu on the judged surface. Owed: mic row **#135**, headless `run_tutor_live.py --component knowledge-check --di --runs 3` on a production set.
...
**2026-09-13 — Misconception backend is primitive-agnostic (USER RULING: production API code names no primitive).**
`misconception_opportunities.py` (tables) is gone. `learning_observations.py` owns scope resolution
(any subject), capture models, delivery and projections; `misconception_receipts.py` owns generic
receipt certification. The catalog declares `observationDelivery: 'server'` on the six math sources;
`captureMisconception.ts` relays `subject` + `delivery`; the backend stamps a canonical published
scope or refuses, and every later rule is a record property (`is_server_delivered`). Receipts bind a
signed plan (`primitive_type`, per-item `accepted_answers`, opaque provenance) to the hypothesis's own
scope; the reviewed-objective allowlist and place-value arithmetic moved to the TS compiler. Status
route is `GET /misconception-status?primitive_type=&skill_id=`. `retire_unstamped_hypotheses.py`
(script) covers the legacy-record ruling; dry run found 0 across 8 students. Verified: backend 76/76
misconception tests, full suite unchanged vs HEAD, `typecheck:lumina` 0, vitest 133 across touched
sets, `probe_place_value_authenticated.py` PASS end to end with cleanup.
CLOSED 2026-09-13 (ledger line above; catalog `learningObservations` + one generic branch) — frontend residual: `geminiService.ts` six `if (item.componentId === …)` delivery branches and
`learningObservationServer.ts` `subject: 'MATHEMATICS'` literal → catalog field (`learningObservations:
{ eligible(config) }`) + objective subject from config, one generic branch. Executor
`/add-misconception-loop` (consumer-contract step 2).
QUEUED — remaining primitive-specific backend code, separate slices under the same ruling:
`calibration/problem_type_registry.py` (193 IRT prior rows mirroring catalog `evalModes`),
`config/discrimination_priors.py`, `generators/content_schemas.py`, `curriculum_retrieval_service.py`
DI names, `composable_problem_generation.py`. Executor `/pm` to scope; the prior registry is the one
that grows per `/add-eval-modes`.

**2026-09-12 — place-value misconception opportunity contract implemented; production acceptance OPEN.**
**Latest continuation / supersedes the next-action ordering below:** one saved Place Value digit-worth hypothesis now targets both the chart and Base-Ten Blocks read mode at the same reviewed Grade 4 objective/medium tier. Blocks use a bounded selector and shared response ledger; no copied diagnosis or block-side resolution. Real LLM/auth/store/generation PASS for two draws per primitive, unadapted baselines and verified cleanup. Full frontend 6,009 passed / 10 skipped, final focused 9 passed, Lumina typecheck zero, backend 51 passed. Tester offers the second representation; review HTML embeds actual contrasts while retaining all 69 entries. Next: live teaching/fading and fresh learner evidence, general evaluator/observation context, canonical retry identity. Human #113/#63 and transfer remain OPEN. [Current handoff](../HANDOFF-learning-observations-2026-09-12.md), [bridge report](../misconception/place-value-base-ten-bridge-2026-09-12.md).
The backend now refuses score/tag-only resolution for place value (including legacy diagnoses). Server-certified compiled item plans bind to an atomic hypothesis revision; the existing runner submits ordered first-response, transcription and assistance observations through the canonical attempt path. Missing/invalid evidence leaves ordinary learning progress intact. Follow-up: Grade 4 `NBT004-01-b` reviewed at the exact publication for four-digit compare; two real targeted generation draws and real Firestore competing-consumer/stale-revision checks pass, with disposable records cleaned up. Grade 3 `NBT003-02-a` remains ineligible. **Next executor: `student-data-loop`** — fix submission-level retry identity/fan-out (receipt consumption alone is idempotent), then verify authenticated issuance → mounted response → canonical submission. Local signing and deployed Firestore client read/write denial now verified. Real Firebase capture through signed Next issuance passes; full published objective examples no longer incorrectly trigger the named-number veto. Dedicated Misconception Loop developer tester now pins the authenticated Grade 4 task, preserves instance/curriculum identity, uses normal evaluation/capture and displays backend status. Mounted microphone/canonical-submission acceptance remains open; automatic retries are disabled in this tester. #113/#63 stay human-owned. [Report](../misconception/place-value-opportunities-2026-09-12.md), [policy/ADR](../ADR-misconception-opportunities-2026-09-12.md). No broader family rollout or script changes.
