...
- **19h. THE JUDGED-LOOP TESTING STORY** (the "eval-test does not scale" answer).
  ...
  - **19h-i-b. IN PROGRESS — SWEEP THE ADAPTERS. 3 of 11 done (`counting-board` + `addition-subtraction-scene` ✅ 2026-08-15, `push-pull-arena` ✅ 2026-08-16).** ⚠️ **The remaining 8 are ALL in `literacy.ts`** — the collision the handoff warned about is now unavoidable, so rebase before each one. Next by the handoff's order: `picture-vocabulary`. One `DiPortAdapter` per remaining runner port; each needs its script module to export a cue surface (`<primitive>PackBase`, spread by the component) and its answer material including the **signature wrong** its `discriminationFor` clause claims to refuse. Serial, one port at a time, each with a `--di` drive. Executor: `/tutor-test --di` per port. **📍 START HERE: [`qa/HANDOFF-19h-i-b-adapter-sweep-2026-08-16.md`](../HANDOFF-19h-i-b-adapter-sweep-2026-08-16.md)** — the 8 remaining ports RE-CENSUSED per rung (eval modes, script size, build gate, 18d state), the three defect classes that have now repeated and are worth checking before spending a session, the drive arithmetic (**30 modes × 3 drills = 90 sessions**, and the one observation bearing on whether cap needs to be per-mode), and nine traps. ⭐ **Its headline: 18d in literacy is a DIFFERENT SHAPE — not a quoted hint but *"Say the question once more, then wait"* — and because `level3` already routes through the correction on all eight, a per-entry grep reports every one of them as already fixed. They are not.** *(Ports 1–2 record kept at [`…-2026-08-15.md`](../HANDOFF-19h-i-b-adapter-sweep-2026-08-15.md).)*
