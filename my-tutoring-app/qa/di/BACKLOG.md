...
- **19h. THE JUDGED-LOOP TESTING STORY** (the "eval-test does not scale" answer).
  ...
  - **19h-i-b. IN PROGRESS — SWEEP THE ADAPTERS. 3 of 11 done (`counting-board` + `addition-subtraction-scene` ✅ 2026-08-15, `push-pull-arena` ✅ 2026-08-16).** ⚠️ **The remaining 8 are ALL in `literacy.ts`** — the collision the handoff warned about is now unavoidable, so rebase before each one. Next by the handoff's order: `picture-vocabulary`. One `DiPortAdapter` per remaining runner port; each needs its script module to export a cue surface (`<primitive>PackBase`, spread by the component) and its answer material including the **signature wrong** its `discriminationFor` clause claims to refuse. Serial, one port at a time, each with a `--di` drive. Executor: `/tutor-test --di` per port. **📍 START HERE: [`qa/HANDOFF-19h-i-b-adapter-sweep-2026-08-15.md`](../HANDOFF-19h-i-b-adapter-sweep-2026-08-15.md)** — the settled per-port recipe, the 9 remaining ports with their eval modes and catalog files, the literacy.ts collision with the concurrent 18d session, and the six traps that cost time on ports 1–2.
