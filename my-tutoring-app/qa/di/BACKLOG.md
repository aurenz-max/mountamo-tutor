...
- **19h. THE JUDGED-LOOP TESTING STORY** (the "eval-test does not scale" answer).
  ...
  - **19h-i-b. IN PROGRESS — SWEEP THE ADAPTERS. 2 of 11 done (`counting-board` + `addition-subtraction-scene`, both ✅ 2026-08-15).** One `DiPortAdapter` per remaining runner port; each needs its script module to export a cue surface (`<primitive>PackBase`, spread by the component) and its answer material including the **signature wrong** its `discriminationFor` clause claims to refuse. Serial, one port at a time, each with a `--di` drive. Executor: `/tutor-test --di` per port.
    ...
    - **19h-i-f REPRODUCED HERE ON THE CAP DRILL, with the same fingerprint:** `di-no-verdict` ×2, and the spoken lines were this catalog's own rungs verbatim — *"…think about what happened in the story"* (level 2) and *"Take your time. Look at the picture. Then tell me."* (level 3). Fixed exactly as port 1 (the law before the branches + rungs routed through the correction), **re-driven: both HIGHs gone**, only the known-open 18c pair left. Two ports, two reproductions, one mechanism — 19h-i-f is confirmed, not suspected.
