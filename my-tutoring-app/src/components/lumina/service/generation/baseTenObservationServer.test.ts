...
// A blocks-origin observation, as the shared delivery endpoint returns it.
const observation = { id: 'observation-5e1f', summary: 'Asked what the blocks in a column are worth, the student says how many blocks there are.',
  evidence: '{"evalMode":"read_blocks","phases":[{"phase":"worth","expected":"forty","observed":"four"}]}' };
...
const run = async (patch: Record<string, unknown> = {}, authorization: string | null = 'Bearer owner') =>
  (await withGenerationRequest(authorization, () => generateComponentContent({ ...item, config: { ...item.config, ...patch } } as never, topic, 'Grade 4')) as { data: BaseTenBlocksData }).data;
...
it('a saved blocks observation reaches the real registry generator, changes the compiled mats, and stays private', async () => {
  ...
  expect(JSON.stringify(data)).not.toMatch(/how many blocks there are|observation-5e1f|learningObservations|misconceptionOpportunity/);
});

it('abstention leaves the baseline, and ineligible tasks or anonymous requests never fetch observations', async () => {
  stubEngines('abstain');
...
it('a client-supplied observation list is dropped before generation', async () => {
  ...
  const data = await run({ learningObservations: [observation] });
