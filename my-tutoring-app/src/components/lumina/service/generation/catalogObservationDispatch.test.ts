...
const { signed, delivered: observation } = signedObservation({ primitiveType: 'fraction-bar', scope: { subject: 'MATHEMATICS', grade: '3', skillId: 'NF001-03', subskillId: 'NF001-03-b' },
  summary: 'The student swaps the roles of the two numbers in a fraction, choosing the bottom number as the shaded count.' });
...
it('dispatches on the catalog declaration: fraction-bar reads its objective scope and shapes the bars', async () => {
  ...
  const planner = vi.mocked(ai.models.generateContent).mock.calls.map(([a]) => String(a.contents)).find(c => c.startsWith('Select ONE'));
  expect(planner).toContain(observation.id);
...
it('a declared consumer is a server-delivered skill-scoped source, and ineligible tasks or requests without a packet never plan', async () => {
  ...
  expect(fetch).not.toHaveBeenCalled();
  expect(vi.mocked(ai.models.generateContent).mock.calls.map(([a]) => String(a.contents)).filter(c => c.startsWith('Select ONE'))).toHaveLength(0);
});
