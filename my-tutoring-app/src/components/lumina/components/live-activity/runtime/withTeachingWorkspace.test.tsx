...
// The catalog decides: number-sequencer declares `teachingWorkspace`, number-line does not.
const Train = withTeachingWorkspace('number-sequencer', Teaching, Scripted);
const Line = withTeachingWorkspace('number-line', Teaching, Scripted);
...
it('mounts the scripted drill for a family the catalog does not declare, and whenever there is no runtime', () => {
  expect(mounted(Line, 'jump')).toBe('scripted');
