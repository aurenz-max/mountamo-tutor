...
describe('ordinary lesson workspace eligibility', () => {
  ...
  it('every family that binds the workspace admits every one of its modes; no mode is withheld from lessons', () => {
    ...
    expect(bound.map(([id]) => id).sort()).toEqual(['balance-scale', 'bar-model', 'compare-objects', 'comparison-builder', 'counting-board', 'di-letter-sounds', 'di-math-facts', 'di-sentence-reading', 'di-word-reading', 'fraction-circles', 'letter-sound-link', 'number-bond', 'number-line', 'number-sequencer', 'number-tracer', 'ordinal-line', 'place-value-chart', 'shape-sorter', 'sorting-station', 'ten-frame']);
