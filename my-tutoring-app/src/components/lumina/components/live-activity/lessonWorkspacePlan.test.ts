...
describe('ordinary lesson workspace eligibility', () => {
  ...
  it('every family that binds the workspace admits every one of its modes; no mode is withheld from lessons', () => {
    ...
    expect(bound.map(([id]) => id).sort()).toEqual(['balance-scale', 'bar-model', 'base-ten-blocks', 'compare-objects', 'comparison-builder',
      ...
      'place-value-chart', 'ramp-lab', 'rhyme-studio', 'shape-sorter', 'sorting-station', 'sound-swap', 'spatial-scene', 'syllable-clapper', 'ten-frame', 'word-builder', 'word-flip', 'word-workout', 'you-and-me']);
