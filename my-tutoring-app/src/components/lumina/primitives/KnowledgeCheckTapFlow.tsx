...
/**
 * KnowledgeCheckTapFlow — the DI-OFF surface of knowledge-check
 * (qa/di/BACKLOG.md item 23 slice 2).
 *
...
interface KnowledgeCheckTapFlowProps {
...
// ─── Main tap-flow component ─────────────────────────────────────────────────

export const KnowledgeCheckTapFlow: React.FC<KnowledgeCheckTapFlowProps> = ({ data }) => {
  ...
  // `challengeType`/`stimulus` are the catalog tutoring block's template keys
  // (shared with the judged surface, which pushes the same two).
  const aiPrimitiveData = useMemo(() => ({
    problemCount: problems.length,
    currentProblemIndex: activeIndex,
    challengeType: activeProblem?.type || 'unknown',
    stimulus: getQuestionText(activeProblem),
...
export function getQuestionText(problem: ProblemData | undefined): string {
  if (!problem) return '';
  switch (problem.type) {
