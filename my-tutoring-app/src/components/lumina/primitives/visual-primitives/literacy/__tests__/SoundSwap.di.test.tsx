 ...
 *     grade; tapping a sound asks the tutor for that SOUND, never a
 *     word.
...
/** SoundSwap runs only on the teaching workspace, so it is mounted the way a lesson mounts it. */
const bound = (data: SoundSwapData) => {
  const runtime = new LiveLessonRuntime('test', { allowSupportArtifacts: true, allowAnswerExposure: true, maxSupportLevel: 3 });
  return <LiveRuntimeContext.Provider value={runtime}><LiveRuntimeSurface runtime={runtime}>
    <SoundSwap data={data} runtimePlanItemId="plan-swap" runtimeEvalMode="addition" />
...
describe('SoundSwap @ PRE (gradeLevel K)', () => {
  ...
  it('tapping a sound asks the tutor for that SOUND', () => {
    render(bound(makeData('K')));
    fireEvent.click(screen.getByRole('button', { name: 'sound /k/' }));
    const spoken = tagged('The learner tapped a sound');
  ...
  it('tapping a sound never speaks a WORD — the new word is the answer', () => {
    ...
    expect(tagged('The learner tapped a sound')).toHaveLength(3);
...
describe('SoundSwap tier levers · what survived the verbal port', () => {
  ...
  it('showWordImage LIVE — easy shows the starting word’s picture cue, hard withdraws it', () => {
    const easy = render(bound(makeData('1', { showWordImage: true })));
    expect(screen.getByText('an orange cat sitting')).toBeTruthy();
    easy.unmount();

    render(bound(makeData('1', { showWordImage: false })));
  ...
  it('showTargetHighlight LIVE — easy marks the sound to change, hard leaves the child to find it', () => {
    const easy = render(bound(makeData('1', { showTargetHighlight: true })));
    ...
    render(bound(makeData('1', { showTargetHighlight: false })));
  ...
  it('NO tier withdraws tap-to-hear — a stuck child can always recover a sound', () => {
    for (const tier of [{}, { showWordImage: false, showTargetHighlight: false, nameTargetSound: false }]) {
      ...
      fireEvent.click(screen.getByRole('button', { name: 'sound /k/' }));
      expect(tagged('The learner tapped a sound')).toHaveLength(1);
      view.unmount();
    }
  });
  ...
  it('optionCount is DEAD — there are no answer buttons left for it to size', () => {
    render(bound(makeData('1', { optionCount: 5 })));
  ...
  it('nameTargetSound is DEAD — it governed a spoken walk that no longer exists', () => {
    ...
    const on = render(bound(makeData('1', { nameTargetSound: true })));
    const withLever = on.container.innerHTML;
    on.unmount();
    const off = render(bound(makeData('1', { nameTargetSound: false })));
