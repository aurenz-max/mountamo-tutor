  ...
  return (
    <div className="w-full max-w-4xl mx-auto my-8 animate-fade-in" style={{ animationDelay: `${index * 200}ms` }}>
      <div className="glass-panel rounded-2xl border border-white/10 overflow-hidden shadow-2xl relative">
        ...
        <div className={isTall ? '' : 'overflow-x-auto custom-scrollbar'}>
            ...
            <thead>
              <tr>
                {data.headers.map((header, i) => (
                  <th
                    key={i}
                    className={`px-6 py-4 text-xs font-bold text-slate-400 uppercase tracking-wider bg-black/20 border-b border-white/10 first:pl-8
                      ${isTall ? '' : 'whitespace-nowrap'}`}
                  >
                    {header}
                  </th>
                ))}
              ...
              {data.rows.map((row, rIndex) => (
                ...
                >
                  {row.map((cell, cIndex) => (
                    <td
                        key={cIndex}
                        className={`px-6 py-4 border-b border-white/5 group-last:border-0 first:pl-8
                            ${isTall ? '' : 'whitespace-nowrap'}
