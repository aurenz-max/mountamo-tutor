...
| **L5** | Polished | `/add-sound` (+ `/add-spoken-judge` for spoken-production primitives) | 2-4 tactile interaction sounds; spoken clip-judge ladder where the student produces speech | Interactions are silent (global navigate/correct/celebration sounds are automatic regardless) | component calls `SoundManager.*`; spoken: `useSpokenTurn` / spoken judge wiring |
