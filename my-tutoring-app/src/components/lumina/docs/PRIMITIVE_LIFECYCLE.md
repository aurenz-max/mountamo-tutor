...
| **L5** | Polished | `/add-sound` (+ `/add-voice-control` for spoken-production primitives; doctrine + push-to-talk: `/add-spoken-judge`) | 2-4 tactile interaction sounds; spoken clip-judge ladder where the student produces speech | Interactions are silent (global navigate/correct/celebration sounds are automatic regardless) | component calls `SoundManager.*`; spoken: `useVoiceAnswer` / `useVoiceChoice` wiring (`/add-voice-control`) |
