...
| **L5** | Polished | `/add-sound` (+ voice: `/add-live-tutor-tools` — the shared tutor/JEV teaching workspace, the STRONG form; it owns migrating the legacy `/add-di-loop` runner packs and edits them in place — or `/add-voice-control` as the interim non-DI rung; doctrine: `TEACHING_WORKSPACE.md`, `SPOKEN_INTERACTION_DOCTRINE.md` for task rules) | 2-4 tactile interaction sounds; the Live tutor teaches from the visible workspace, the shared observer judges the completed exchange and the runtime commits the outcome (workspace), or the spoken clip-judge ladder (interim) | Interactions are silent; advance rides a timer or a button instead of a judged outcome | component calls `SoundManager.*`; workspace: `useTeachingWorkspace` binding + adapter `bindsTeachingWorkspace` (`/add-live-tutor-tools`); legacy, being migrated: a `<primitive>Script.ts` pack over `useJudgedScriptRunner`; interim: `useVoiceAnswer` / `useVoiceChoice` wiring (`/add-voice-control`) |
