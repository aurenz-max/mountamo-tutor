...
| **L5** | Polished | `/add-sound` (+ voice: `/add-di-loop` — the tutor-owned DI judged loop, the STRONG form — or `/add-voice-control` as the interim non-DI rung; doctrine: `SPOKEN_INTERACTION_DOCTRINE.md`) | 2-4 tactile interaction sounds; the Live tutor asks/waits/judges in-band and its own verdict advances (DI), or the spoken clip-judge ladder (interim) | Interactions are silent; advance rides a timer or a button instead of the tutor's verdict | component calls `SoundManager.*`; DI: a `<primitive>Script.ts` pack over `useJudgedScriptRunner` (`/add-di-loop`); interim: `useVoiceAnswer` / `useVoiceChoice` wiring (`/add-voice-control`) |
