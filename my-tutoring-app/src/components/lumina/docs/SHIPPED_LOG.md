...
Total Bucket A backlog grows from 2 (percent-bar, double-number-line per §3a) to **10** after this audit:

| # | Primitive | Grade | Shape | Priority |
|---|-----------|-------|-------|----------|
| 1 | `percent-bar` | 5-8 | Classic Bucket A | K-3 adjacent (PRD-listed) |
| 2 | `double-number-line` | 5-7 | Classic Bucket A | K-3 adjacent (PRD-listed) |
...
K-3 priority remains entries #1-#2 (already in §7). Mid-elementary (entries #3-#5) join Workstream 3 ahead of upper-grade entries. Upper-grade Bucket A entries (#6-#10) are worth tracking but lower priority for the same reason the K-3 framing was used originally — content density compounds where the engine actually routes students today. Same pool-service / orchestrator decision rule from §6a #1 applies to all 10.
