...
Still required: actual audible cues on supported devices; human review of manuscript forms and alternate stroke conventions; child finger/stylus trials; empirical tolerances/IRT; backend replay once copy/write becomes eligible for adaptive submission. The tutoring scaffold is implemented and passes three-mode prompt probes plus a real live connection; full live coaching behavior remains to verify. Support and structural tiers are now implemented: trace progressively hides arrows and starts; all modes withdraw line labels and reminders. Copy retains its model; write retains only its audible target cue. A code-owned stroke/curve/corner/extension score selects low/middle/high available forms within each scoped case. The anchor score is the tier; it widens to the neighbouring band only when the anchor alone cannot supply one distinct form per challenge, so a tier never repeats a single letter through a session that its scope could vary. Only scope saturates: a one-letter scope stays that letter at every tier. No-tier behavior and scorer thresholds are unchanged. Tutor context and trace retry feedback honor hidden aids. See [tier verification](../../../../../qa/eval-reports/letter-workshop-tiers-2026-09-07.md). Withdrawing arrows from trace must never relabel it copy or independent write.
