...
- **Property:** At `gradeBand==='K'` no numeric keyboard input is offered.
  ...
  - **`act_out` @ Grade 1 — FORKED BY OPERATION (item 12, 2026-08-06):** the ANSWER stays the typed numeral + Check at both operations (this requirement is intact). The SCENE forks: **subtraction** is direct manipulation (seeded at `startCount`; taps send objects away, capped at `changeCount`; then the child counts the survivors and types the count — no auto-judge), **addition** keeps count-the-scene (start+change painted, `groupedReveal` shows the join). A static scene can show a join but not a departure, so subtraction had to be enacted at every band.
...
- **Property:** For a pinned mode + tier, `difficulty` withdraws on-screen aids and hardens the problem SHAPE within the same number band:
  - `act_out` → AXIS 1 only: `showCountBadges` (ordinal-on-tap) + `groupedReveal` (change group animates separately). **These aids serve the Grade-1 count-the-scene model; at K, act_out is direct manipulation and the count-badge aid is moot (superseded by R3's enactment) — see Conflicts.** At Grade-1 **subtraction** (item 12) both aids keep a consumer: `showCountBadges` moves to the post-enactment taps (count the survivors — the group the answer is actually about), and `groupedReveal` was already inert for subtraction (every object is in the start group).
...
- 2026-08-06 — item 12 (Grade-1 `act_out` wires crossed, browser-reported). The mode promised manipulation at every band (evalMode "Manipulate objects in scene"; the LLM-authored instruction read "Tap 2 frogs to send them away") while Grade 1 implemented counting: the scene painted `startCount` frogs that never left and each tap stamped an ordinal badge, so the aid rehearsed 1…6 when the answer was 4. Assessed **COMPATIBLE / fork-by-operation**: (a) G1 subtraction becomes an enacted scene — seeded at `startCount`, tap removes, capped at `changeCount`, then taps fall through to the count aid on the survivors; (b) the G1 answer channel is UNCHANGED (typed numeral + Check — no auto-judge, R3 intact); (c) G1 addition untouched (count-the-scene + `groupedReveal`, R6 intact); (d) the act-out instruction is now code-owned at every band (K strings byte-identical), so it can never again name an action the band doesn't implement; (e) the ten frame mirrors the on-screen count, closing an answer leak that also affected K. Verified: 12/12 jsdom (7 K regression + 5 new G1) + a 10-check real-Chrome run (taps remove: 6→5→4, cap holds at 4, badge on the 3rd tap, ten frame 4, typed 4 completes, addition still 7-and-non-removing).
