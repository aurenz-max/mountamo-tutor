...
- **Grading (2026-09-13):** a landing is correct only at the exact snapped grid point (`isSnappedPlacementExact`, |placed − expected| < snap/2). Before this, integer tolerance equalled the snap step, so a landing one hop short or past was accepted. Every generated landing is on the grid, so reachability (the oracle's check) is unchanged. `plot_point` still accepts ±1 snap step — queued as NL-2, not changed here.
- **Text (2026-09-13, NL-3):** jump instructions and hints may name the first start and the hop sizes, never a position the hops pass through or land on (`jumpTextNamesPosition`); offending LLM text falls back to the code template.
...
### R13 — Observation-driven jump contrast · OBSERVED (2026-09-13)
- **Property:** a saved, same-skill observation reaches the jump generator only through the backend-signed launch packet (verified in `generationRequest.ts`, joined in `learningObservationPacket.ts`; no backend call during generation), only for Grade 1 `OPS001-03-a` / `OPS001-04-a` at `targetEvalMode: jump` + `difficulty: medium` (`numberLineObservationScope`). The generator makes one shared-planner call when the task is eligible (grade 1–2, jump, medium, no named equation or start in topic/intent). A validated `contrast_start_positions` replaces at most ONE tuple before any text is written, with the zero-anchored twin of another challenge (same hop and direction; add from 0 or subtract to 0). Count 4, every other tuple, the `[1..5]` table, range, uniqueness, R4 arithmetic and tier scaffolds hold. No zero in range → `insufficient-capacity`, baseline kept. `learningAdaptation` = move, status, comparisonCount, and `source` stamped only by the delivery server. No receipt and no resolution: `number-line` declares `observationDelivery: 'server'` in the catalog, so its hypotheses are scope-stamped and the backend (which names no primitive) refuses score/tag resolution for them.
- **Capture:** every jump Check is kept in `student_work.jumpResponses` (start, op, expected and placed landings, try number, arc shown). Evidence attaches only when a try was wrong and carries `firstResponseScore`, because sessions advance only when correct and always submit 100.
- **Demanded by:** misconception loop (plans.json number-line / jump).
- **Evidence:** `numberLineRemediation.test.ts`, `gemini-number-line.remediation.test.ts` (seeded causal; fails with the selector removed), `numberLineObservationServer.test.ts` (registry path), `numberLineEvidence.test.ts`, backend `test_number_line_observation_scope.py`; real probe `qa/misconception/number-line-jump-2026-09-13.md`.
- **Probe:** eval-test `evalMode=jump&grade=1&difficulty=medium&topic=Add within 20 by counting on&remediationFocus=<start-counting observation>` → `learningAdaptation.move = contrast_start_positions`, one challenge anchored at zero paired with a same-hop challenge; an unrelated focus → no `learningAdaptation`.
...
- 2026-09-13 — misconception loop, jump start contrast (`/add-misconception-loop`). R4 grading tightened to the exact landing (a one-hop miss had been accepted, which hid the very error the loop targets). R13 added. R1–R3, R5–R12 untouched: plot/order/between code paths unchanged; the selector runs only on single-step jump tuples at medium; existing grade-band, reader-fit 14k, session-distinctness and oracle suites pass. `--check` COMPATIBLE by rule: the only behavior change to an existing requirement is stricter jump grading, which no consumer depends on being lenient.
