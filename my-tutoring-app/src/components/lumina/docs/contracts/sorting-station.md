...
- **Status:** ACTIVE (no open conflicts; 3 resolved on record; 4 gap requirements OPEN, G3 LANDED 2026-07-21). ⚠️ **DI JUDGED-LOOP PORT 2026-08-18 — read the changelog before editing:** R6 and R7 are RE-BASED (the controls they named are gone; what they PROTECT is satisfied structurally), R5 is superseded in kind by the scripted ask, and **G2 + G3(a) are now UNBLOCKED** — their shortfall was the written/numeric medium, which the spoken port removed. **R3's K band floor is deliberately UNMOVED and pinned by a test**; moving it is a `/reader-fit` re-audit, never a catalog edit.
...
- **Property:** object window K 4–6 / G1 5–8; bins K ≤3 / G1 ≤4. **Enforcement split (precision, 2026-07-15 baseline check):** the bin cap is a hard code clamp (`maxCategories`); the object window is **tier-conditioned prompt guidance** — `normalizeSupportTier` is a strict lookup, so a draw with no valid `config.difficulty` gets NO window prompt line and the LLM may undershoot the floor (observed: one untiered G1 draw at 4 objects). The demanding consumer always passes a tier, and tiered draws honor the window. **Amended 2026-09-08 (K atlas slice 6), sort family only:** the object FLOOR is now code-enforced too — `sortChallengeFault` rejects a challenge under `gradeObjectWindow(band).min` and it is redrawn once. The bin cap is unmoved and now binds a second producer: when an objective NAMES its groups, that set replaces the tier's bin ramp, but **only while it fits the band cap**; a named set larger than the cap is dropped, logged, and left to the tier, because the cap is a band contract rather than an arbitrary ceiling and overriding it is its own decision. Difficulty moves STRUCTURE — bin ramp 2→3→4, `compareGap` 3→1, `oddSharedAttrs` 0→2, near-miss ratio — never raw magnitude past the grade cap. Support tiers withdraw `showCounts` badges and the `showModelExample` worked item across easy→hard.
...
### R11 — a card's picture is the object its label names · OBSERVED (added 2026-09-09)
- **Property:** every object card's emoji depicts the object its own label names. `cardPictureFault` faults a card whose picture is a KNOWN one and whose label contains a KNOWN noun the picture does not depict, with no other word in the label agreeing ("Red Banana" 🍓 goes; "Red Apple" 🍎 and "Rain Hat" 👒 stay). Enforced on the sort family (via `sortChallengeFault`), on `two_attributes`, and on `sort-variety` — where the set is shared by every round, so the SET is rejected rather than the card, since dropping one card thins the set below the band's object floor.
- **Demanded by:** PRE band (R4's consumer). At the pre-reader band the picture IS the object, so a mismatched emoji makes the item unanswerable by eye — the exact demand the 2026-09-08 floor move assumed was absent.
- **Evidence:** `qa/reader-fit/k-band-floor-2026-09-08.md` residual 3 (a `two_attributes` card labelled "Red Banana" carrying 🍓). `two_attributes` had NO draw gate at all before this.
- **Deliberately low-recall:** the noun→picture table is a known-answer list, not a classifier, and anything absent from it passes. A false reject costs the child a whole challenge; a miss costs one card.
- **Probe:** two fresh K draws per mode at the asked count — every card's emoji agrees with its label. Unit: the named-bins fixture may not give different objects the same emoji.

### R12 — the mode ships the count the lesson asked for · OBSERVED (added 2026-09-09)
- **Property:** a pinned single-mode session generates `config.challengeCount` challenges (2..10), defaulting to 4 only when the caller sends none. `sort_variety`'s rounds are capped at **3 per object set** (`MAX_ROUNDS_PER_SET`) and a lesson asking for more is served by additional on-topic sets, one draw each.
- **Demanded by:** mastery sessions ([[mastery-over-demo]]: a single-mode session needs 3-6+ instances). MEAS001-02-G / PTRN001-02-D routed to draws of 2.
- **Evidence:** `qa/reader-fit/k-band-floor-2026-09-08.md` residual 2 (`sort_variety` returned 2 where 5 were asked). Two caps stacked: the orchestrator hardcoded 4 regardless of the ask, and variety capped rounds at 3.
...
- **2026-08-18 note:** the port did NOT close this. Spoken justification is open-set production (`open_set_word`), which is BLOCKED in `judgedScriptContract.ts` — so this needs a judge capability, not a medium. Recorded so the next reader does not mistake the spoken port for having answered it.
...
### G2 — two-attribute sorting reachable at K · OPEN — ⚠️ **THE BLOCKER IS GONE (2026-08-18): the medium named below no longer exists.** The DI port asks the compound one object at a time as a SPOKEN yes/no, machine-verified (`two_attributes` signature drive 8/8 — the half-answered compound refused every time). What remains is exactly the step this entry demands and the port deliberately did not take: the **reader-fit re-audit at PRE**. `/reader-fit sorting-station`.
...
- **Follow-ups:** (a) ⚠️ **UNBLOCKED 2026-08-18** — the DI port makes every `sort_variety` rule VOICED by construction, so the reading demand this follow-up names is gone; what is left is the re-audit. (a, original text) K voiced-rule variant (unfloor via reader-fit re-audit, mirroring G2 — NOT a simple floor removal); (b) extend the R1 exemption to the `/topic-fidelity` automated probe.
...
- 2026-08-18 — **DI JUDGED-LOOP PORT (`/add-di-loop`), all seven eval modes. `--check` verdict: COMPATIBLE.** Report: `qa/primitive-contracts/sorting-station-check-2026-08-18.md`. **R7 and R6 are RE-BASED, not overridden** — R7 protects the COMMIT STEP for multi-part construction, and the judged loop removes the *multi-part construction*, not the commit: one object is now one atomic judged turn whose commit is the child's spoken answer plus the tutor's verdict. (C3 pre-warned "the tempting over-general edit a future declutter pass would reach for"; this is not that pass, because nothing here grades partial work.) R6 the same way — the child SAYS which card does not belong, so there is no tap left to latch. **R4 preserved verbatim** (the live risk of a whole-file rewrite): K trays stay `bucketEmoji`-primary with the word as caption, cards emoji-primary and enlarged, adult chrome hidden, `isPreReader` the one gate, Grade-1 control pinned. R5 superseded IN KIND and strengthened — the scripted ask names the groups aloud by construction and the K band floor forces it at every tier, so the `{{instruction}}`/`{{categories}}` keys that could "render SILENT" no longer exist; `contextKeys` are now the family's `['challengeType','stimulus']`. One deliberate change at BOTH bands: the printed instruction is gone (the tutor speaks it). R1/R2/R8/R9 hold; **R10 tightens** — counts must now land in the benched 1–20 spoken range and an EMPTY group is dropped rather than asked, since zero has no benched spoken form. **R3 — THE K BAND FLOOR — DELIBERATELY UNMOVED**, and pinned by a test: G2's own path is "a reader-fit re-audit … **Not** a simple unflooring", so this slice ships the MEDIUM that makes the audit possible and leaves the floor to the audit. **G2 and G3(a) are now unblocked and are the top follow-up (`/reader-fit sorting-station`)** — their shortfall was the written/numeric medium, which is gone. Four pre-existing CONTENT faults surfaced by writing the spoken ask, all fixed: the per-tray count badge printed the answer to every count ask (pixel leak, now per-item); the compare ask contained its own answer by construction (re-shaped to take a group LABEL); a prompt contradiction had silently killed `sort_attribute`'s metacognitive choice (objects carried one attribute, so the chooser had nothing to choose — a one-button chooser in the click era too); and `pick_rule`'s correct spoken answer was the adult word "category" (now "kind" via `spokenAxisName`, spoken side only — `ruleName` keeps the key). Evidence: live 7-probe run 27/27 kept / 0 drops / 69 items / pack gates clean; judged drives 20/20 plain + 20/20 signature on `sort_one`, 8/8 `two_attributes`, 9/9 `count_compare`, 4/4 `odd_one_out`; cap drill 0 HIGH (3/3 byte-identical corrections, zero `di-no-verdict`); `typecheck:lumina` 0; own suites 59/59 + 13/13 (reader-fit suite REWRITTEN onto the judged surface, no pinned intent dropped). Not verified: any browser run.
- 2026-09-08 — **K atlas slice 6 (`/eval-fix sorting-station`, SS-6).** Filed as draw
  variance; it was not. The easy tier's `resolveProblemShape` asked for "about 2 groups"
  while MEAS001-06-B named three (hot, warm, cold), so one draw obeyed the tier and shipped
  a two-bin sort in 4/4 challenges. Fix: the groups an objective names are resolved through
  a SCHEMA extraction (`resolveNamedCategories`) rather than a regex — "into hot, warm and
  cold" and "by color or shape" are not separable by pattern, and the second names an axis,
  not a bin set — and they set the bin count the tier would otherwise ramp. The set is
  stated in the prompt (`buildNamedCategoriesSection`) and enforced after parse: bins are
  DERIVED from object attribute values, so a draw that never says "warm" is a two-bin sort
  and is rejected, then redrawn once. **R8 amended, not overridden** — a named set only
  binds while it fits the band bin cap. Two further faults fold into the same gate, both
  caught by the redraw: an object count under the band floor (a 3-object K challenge) and a
  tile whose "emoji" was the text `Ic`. Degrade is explicit: if neither draw is clean, the
  first ships with a log rather than emptying the lesson. Evidence: 3 consecutive live
