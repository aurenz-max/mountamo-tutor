...
class StructuralValidator:
    ...
    VALID_VISUAL_TYPES = {
        ...
        "object-collection", "comparison-panel", "card-grid",
        # Math visuals
        ...
        "letter-tracing", "letter-picture", "letter-card", "word-card", "alphabet-sequence", "rhyming-pairs",
    ...
    def __init__(self, schemas_dir: Optional[Path] = None):
        ...
        if schemas_dir is None:
            # Default to schemas directory inside evaluation directory
    ...
    def _detect_problem_type(self, problem: Dict[str, Any]) -> Optional[str]:
        ...
        elif "prompt" in problem and "interaction_config" in problem:
            # Live interaction: has AI tutor prompt and interaction configuration
            prompt = problem.get("prompt", {})
            interaction_config = problem.get("interaction_config", {})
            if isinstance(prompt, dict) and "system" in prompt and "instruction" in prompt:
                if isinstance(interaction_config, dict) and "mode" in interaction_config:
    ...
    def _validate_visual_intent(self, visual_intent: Any) -> List[str]:
        ...
        1. Simple: { "needs_visual": bool, "visual_type": str, ... }
        2. Composite (live_interaction): { "display_visual_intent": {...}, "interaction_visual_intent": {...} }
        """
        issues = []
        ...
        # Check if this is a COMPOSITE structure (live_interaction)
        # Composite has display_visual_intent and/or interaction_visual_intent instead of needs_visual
        is_composite = ("display_visual_intent" in visual_intent or
                       "interaction_visual_intent" in visual_intent)

        if is_composite:
            # Validate composite structure (used by live_interaction)
            # Both sub-intents are optional, but at least one should be present
            if "display_visual_intent" in visual_intent:
                display_intent = visual_intent["display_visual_intent"]
                if display_intent is not None:  # null is allowed
                    sub_issues = self._validate_simple_visual_intent(display_intent, "display_visual_intent")
                    issues.extend(sub_issues)

            if "interaction_visual_intent" in visual_intent:
                interaction_intent = visual_intent["interaction_visual_intent"]
                if interaction_intent is not None:  # null is allowed
                    sub_issues = self._validate_simple_visual_intent(interaction_intent, "interaction_visual_intent")
                    issues.extend(sub_issues)

            return issues

        # Otherwise, validate as SIMPLE structure
        return self._validate_simple_visual_intent(visual_intent, "visual_intent")

    def _validate_simple_visual_intent(self, visual_intent: Any, field_name: str = "visual_intent") -> List[str]:
        """Validate a simple (non-composite) visual intent structure"""
        issues = []
        ...
        if "needs_visual" not in visual_intent:
            issues.append(f"{field_name} missing 'needs_visual' field")
            return issues

        if not isinstance(visual_intent["needs_visual"], bool):
            issues.append(f"{field_name}.needs_visual must be a boolean")

        # If needs_visual is true, check additional fields
        if visual_intent.get("needs_visual"):
            if "visual_type" not in visual_intent:
                issues.append(f"{field_name} must have 'visual_type' when needs_visual=true")
            ...
            if "visual_id" not in visual_intent:
                issues.append(f"{field_name} must have 'visual_id' when needs_visual=true")

            if "visual_purpose" not in visual_intent:
                issues.append(f"{field_name} must have 'visual_purpose' when needs_visual=true")
