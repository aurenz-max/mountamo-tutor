...
from app.models.visual_schemas import VISUAL_TYPE_TO_SCHEMA
...
class GeminiJudge:
    ...
    def _extract_visual_types(self, problem: Dict[str, Any]) -> List[str]:
        """
        Extract all visual types referenced in the problem.

        Args:
            problem: Problem dict

        Returns:
            List of visual type strings found in the problem
        """
        visual_types = set()

        def extract_from_dict(d: Any):
            """Recursively extract visual_type fields"""
            if isinstance(d, dict):
                if 'visual_type' in d and d.get('needs_visual'):
                    visual_types.add(d['visual_type'])
    ...
    def _build_evaluation_prompt(
        ...
        # Extract visual types and build visual schema documentation
        visual_types = self._extract_visual_types(problem)
        visual_schema_docs = ""
        if visual_types:
            visual_schema_docs = "\n\nVISUAL SCHEMA REFERENCE:\n"
            visual_schema_docs += "The problem uses the following visual types. Here's what each visual schema provides:\n\n"
            for visual_type in visual_types:
                if visual_type in VISUAL_TYPE_TO_SCHEMA:
                    schema = VISUAL_TYPE_TO_SCHEMA[visual_type]
                    visual_schema_docs += f"• {visual_type}:\n"
                    visual_schema_docs += f"  Schema: {json.dumps(schema, indent=4)}\n"
                    visual_schema_docs += f"  This visual type renders with the following data structure and will display accordingly in the UI.\n\n"
...
1. PEDAGOGICAL APPROACH & EFFECTIVENESS (NEW & MOST IMPORTANT)
   ...
   - If visual schemas are used, evaluate whether the CHOSEN VISUAL TYPE is appropriate. For example:
     * For phonics/rhyming: Does it use visual schemas that show WORDS prominently (word-card, sight-word-card) vs. just images?
     * For counting: Does it use object-collection or comparison-panel vs. abstract representations?
     * For literacy: Does the visual type match whether the skill is auditory (letter sounds) vs. visual (letter recognition)?
   - Consider alternative formats (e.g., interactive, image-based, hands-on activity description, or DIFFERENT visual schema types). Would another approach be significantly better?
   - Score reflects how well the *method* matches the *skill*. A perfectly executed but inappropriate problem format or visual schema choice should receive a low score here.
...
"""
    ...
    async def evaluate_problem(
        ...
        try:
            ...
            # Call Gemini with sufficient tokens for complete JSON response
            response = await self.client.aio.models.generate_content(
                ...
                config=GenerateContentConfig(
                    ...
                    max_output_tokens=8192  # Increased to ensure complete JSON responses
                )
            )
            ...
            # Already logged in the JSON parsing block
            raise
