...
Edge and version collections remain flat:
  curriculum_edges/{edge_id}
...
class FirestoreCurriculumReader:
    """Read-only queries against curriculum draft/published docs and flat edge collections.

    Curriculum hierarchy (subjects, units, skills, subskills) is read from
    the hierarchical draft doc. Edges, prerequisites, versions, and primitives
    remain in flat collections.
    """
    ...
    def _c(self) -> Dict[str, Any]:
        """Flat collection references (edges, versions, etc.)."""
    ...
    async def get_unit(self, unit_id: str) -> Optional[Dict[str, Any]]:
        """Find a unit across all subjects (searches drafts then published)."""
        for collection_name in ("curriculum_drafts", "curriculum_published"):
            for grade_doc in self._client.collection(collection_name).stream():
    ...
    async def get_skills_by_unit(self, unit_id: str, include_drafts: bool = False) -> List[Dict[str, Any]]:
        """Get skills for a unit (searches for the unit across subjects)."""
        # We need the parent subject — search for unit_id
        unit_doc = await self.get_unit(unit_id)
        if not unit_doc:
            return []
    ...
    async def get_skills_by_unit_ids(self, unit_ids: List[str], include_drafts: bool = False) -> List[Dict[str, Any]]:
        """Get skills for multiple units."""
        result = []
        for uid in unit_ids:
            result.extend(await self.get_skills_by_unit(uid, include_drafts))
        return result

    async def get_skill(self, skill_id: str) -> Optional[Dict[str, Any]]:
        """Find a skill across all subjects."""
        for collection_name in ("curriculum_drafts", "curriculum_published"):
            for grade_doc in self._client.collection(collection_name).stream():
                for subject_doc in grade_doc.reference.collection("subjects").stream():
                    d = subject_doc.to_dict()
    ...
    async def get_subskills_by_skill(self, skill_id: str, include_drafts: bool = False) -> List[Dict[str, Any]]:
        """Get subskills for a skill."""
        skill_doc = await self.get_skill(skill_id)
        if not skill_doc:
            return []
    ...
    async def get_subskills_by_skill_ids(self, skill_ids: List[str], include_drafts: bool = False) -> List[Dict[str, Any]]:
        """Get subskills for multiple skills."""
        result = []
        for sid in skill_ids:
            result.extend(await self.get_subskills_by_skill(sid, include_drafts))
        return result

    async def get_subskill(self, subskill_id: str) -> Optional[Dict[str, Any]]:
        """Find a subskill via the subskill_index (O(1) per subject doc)."""
        for collection_name in ("curriculum_drafts", "curriculum_published"):
            for grade_doc in self._client.collection(collection_name).stream():
                for subject_doc in grade_doc.reference.collection("subjects").stream():
                    d = subject_doc.to_dict()
    ...
    # ==================== EDGE READS (flat collections) ====================

    async def get_edges_for_subject(self, subject_id: str, include_drafts: bool = False) -> List[Dict[str, Any]]:
        query = self._c["edges"].where("subject_id", "==", subject_id)
        if not include_drafts:
            query = query.where("is_draft", "==", False)
        return [doc.to_dict() for doc in query.stream()]

    async def get_entity_edges(self, entity_id: str, entity_type: str, include_drafts: bool = False) -> Dict[str, List[Dict[str, Any]]]:
        out_query = self._c["edges"].where("source_entity_id", "==", entity_id)
        if not include_drafts:
            out_query = out_query.where("is_draft", "==", False)
        outgoing = [doc.to_dict() for doc in out_query.stream()]

        in_query = self._c["edges"].where("target_entity_id", "==", entity_id)
        if not include_drafts:
    ...
    async def get_prerequisite_edges(self, subject_id: str) -> List[Dict[str, Any]]:
        query = self._c["edges"].where("subject_id", "==", subject_id).where("is_prerequisite", "==", True)
        return [doc.to_dict() for doc in query.stream()]

    async def get_edge(self, edge_id: str) -> Optional[Dict[str, Any]]:
        doc = self._c["edges"].document(edge_id).get()
        return doc.to_dict() if doc.exists else None
    ...
    async def get_draft_entities(self, subject_id: str) -> Dict[str, List[Dict[str, Any]]]:
        ...
        # Edges and prerequisites remain flat
        for doc in self._c["edges"].where("subject_id", "==", subject_id).where("is_draft", "==", True).stream():
